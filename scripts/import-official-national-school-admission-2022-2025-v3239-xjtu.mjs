...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2022-2025-v3239-xjtu-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2022-2025-v3239-xjtu";
const OFFICIAL_NAV_URL = "https://zs.xjtu.edu.cn/";
const INDEX_URL = "https://zswxxcx.xjtu.edu.cn/public/zsdata/lqxx/";
const SPA_URL = "https://zswxxcx.xjtu.edu.cn/public/zsdata/lqxx/#/lnfs";
const BASE_API_URL = "https://zswxxcx.xjtu.edu.cn/lqxx/s";
...
const SOURCE = {
  id: "official-xjtu-national-2022-2025-school-admission",
  quality: "official-school-xjtu-2022-2025-national-api-score-rank",
  schoolCode: "10698",
  schoolName: "西安交通大学",
  city: "陕西西安",
  publisher: "西安交通大学招生办公室",
  tags: ["陕西", "西安", "西安交通大学", "985", "211", "双一流", "C9"],
};

function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2022-2025-v3239-xjtu.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2022-2025-v3239-xjtu.mjs --use-cache --concurrency 4",
    "",
    "Imports 西安交通大学招生办公室 2022-2025 历年分数 official API data.",
  ].join("\n");
}
...
async function postJsonRaw(rawRoot, rawFile, url, body, useCache) {
  ...
  const text = await requestText(url, {
    ...
    headers: {
      ...
      origin: "https://zswxxcx.xjtu.edu.cn",
    },
...
function queryRawRel(query, index) {
  ...
  return `xjtu-${query.nf}-${slugify(query.sf)}-${slugify(query.klmc)}-${slugify(query.xqmc)}-${slugify(query.zslb)}-${index}-${hash}.json`;
}
...
function buildRecord(row, rawRel, query, pageIndex, rowIndex, dataType) {
  ...
  const idPrefix = dataType === "institution-admission" ? "xjtu-summary" : "xjtu";
  const record = {
    ...
    sourcePageKey: `xjtu-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
function parseQuery(raw, rawRel, query, pageIndex) {
  ...
  if (payload.code !== 200 && payload.code !== "200") {
    return {
      records,
      summary: {
        pageKey: `xjtu-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
async function main() {
  ...
  await getTextRaw(rawRoot, "xjtu-official-admission-site.html", OFFICIAL_NAV_URL, args.useCache, "text/html,*/*;q=0.9");
  const indexHtml = await getTextRaw(rawRoot, "xjtu-lqxx-index.html", INDEX_URL, args.useCache, "text/html,*/*;q=0.9");
  const jsFiles = [];
  ...
  const themeRaw = await postJsonRaw(rawRoot, "xjtu-getTheme.json", THEME_URL, {}, args.useCache);
  const theme = bodyFromRaw(themeRaw).homeTheme || {};
  if (normalizeText(theme.xxmc) !== SOURCE.schoolName || !/zs\.xjtu\.edu\.cn/.test(String(theme.zsxxLink || ""))) {
    throw new Error("Official query system theme does not identify 西安交通大学 and zs.xjtu.edu.cn; refusing to import without that evidence.");
  }

  const getTypeRaw = await postJsonRaw(rawRoot, "xjtu-getType-lnfs.json", GET_TYPE_URL, { type: "lnfs" }, args.useCache);
  const globalCfgRaw = await postJsonRaw(rawRoot, "xjtu-getGlobalCfg.json", GLOBAL_CFG_URL, {}, args.useCache);
  const displayCfgRaw = await postJsonRaw(rawRoot, "xjtu-getlqxsgz-lnfs.json", DISPLAY_CFG_URL, { type: "lnfs" }, args.useCache);
  ...
  const rawFiles = [
    `${RAW_DIR}/xjtu-official-admission-site.html`,
    `${RAW_DIR}/xjtu-lqxx-index.html`,
    ...jsFiles,
    `${RAW_DIR}/xjtu-getTheme.json`,
    `${RAW_DIR}/xjtu-getType-lnfs.json`,
    `${RAW_DIR}/xjtu-getGlobalCfg.json`,
    `${RAW_DIR}/xjtu-getlqxsgz-lnfs.json`,
    ...pageSummaries.map((summary) => summary.rawFile),
  ];

  const configSha256 = {};
  for (const rawFile of rawFiles.filter((file) => /xjtu-(official-admission-site|lqxx-index|getTheme|getType|getGlobalCfg|getlqxsgz|.*\.js)/.test(file))) {
  ...
  const sourceNotes = [
    {
      ...
      title: "西安交通大学招生办公室历年分数 API（2022-2025）",
      url: SPA_URL,
      officialNavigationUrl: OFFICIAL_NAV_URL,
      quality: SOURCE.quality,
      usage:
        "学校官网单校分专业录取分数边界；可用于西安交通大学候选边界复核、985/211/双一流/C9和理工医学方向分数段趋势及全国单校分数加厚，不替代任何省级教育考试院全量投档/录取表。",
      rawDir: RAW_DIR,
      rawFiles,
      parsedRecords: records.length,
  ...
  const output = {
    dataset: "official-national-school-admission-2022-2025-v3239-xjtu",
