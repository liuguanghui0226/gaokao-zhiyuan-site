...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3298.json");
const expectedCovered = ["安徽", "北京", "重庆", "福建", "甘肃", "广东", "广西", "贵州", "海南", "河北", "河南", "黑龙江", "湖北", "湖南", "江西", "吉林", "内蒙古", "陕西", "山东", "上海", "四川", "天津", "新疆", "西藏", "浙江"];
...
assert(manifest.modelVersion === "local-deterministic-v3.298-heilongjiang-control-lines2026-and-rank-provenance-847051records", "Unexpected v3.298 model version");
assert(manifest.recordCount === 847051, "Unexpected v3.298 record count");
assert(core.admissionScoreLayer.coverage.dataTypes["control-line"] === 1405, "Unexpected v3.298 control-line count");
const coverage = [];
...
assert(coveredProvinces.length === 25 && missingProvinces.length === 6, "Expected 25 covered and 6 missing provinces after Heilongjiang v3.298");
...
assert(!heilongjiang.boundaries.some((row) => [466, 464, 269, 255, 238, 180, 170, 140, 70].includes(row.minScore)), "Heilongjiang special, art or sports lines leaked into ordinary coverage");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3298",
