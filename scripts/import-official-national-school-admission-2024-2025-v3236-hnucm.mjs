...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2024-2025-v3236-hnucm-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2024-2025-v3236-hnucm";
const OFFICIAL_NAV_URL = "https://zhaosheng.hnucm.edu.cn/fsf_login-jg.jsp?wbtreeid=1062";
const INDEX_URL = "https://zsdata.hnucm.edu.cn/zsdata/lqxx/";
const SPA_URL = "https://zsdata.hnucm.edu.cn/zsdata/lqxx/#/lnfs";
const BASE_API_URL = "https://zsdata.hnucm.edu.cn/lqxx/s";
...
const SOURCE = {
  id: "official-hnucm-national-2024-2025-school-admission",
  quality: "official-school-hnucm-2024-2025-national-api-score-rank",
  schoolCode: "10541",
  schoolName: "湖南中医药大学",
  city: "湖南长沙",
  publisher: "湖南中医药大学本科招生办公室",
  tags: ["湖南", "长沙", "湖南中医药大学", "中医药", "医学类"],
};

function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2024-2025-v3236-hnucm.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2024-2025-v3236-hnucm.mjs --use-cache --concurrency 4",
    "",
    "Imports 湖南中医药大学本科招生网 2024-2025 历年分数 official API data.",
  ].join("\n");
}
...
async function postJsonRaw(rawRoot, rawFile, url, body, useCache) {
  ...
  const text = await requestText(url, {
    ...
    headers: {
      ...
      origin: "https://zsdata.hnucm.edu.cn",
    },
...
function queryRawRel(query, index) {
  ...
  return `hnucm-${query.nf}-${slugify(query.sf)}-${slugify(query.klmc)}-${slugify(query.xqmc)}-${slugify(query.zslb)}-${index}-${hash}.json`;
}
...
function buildRecord(row, rawRel, query, pageIndex, rowIndex, dataType) {
  ...
  const idPrefix = dataType === "institution-admission" ? "hnucm-summary" : "hnucm";
  const record = {
    ...
    sourcePageKey: `hnucm-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
  ...
  if (province === "西藏") {
    record.cautions.push("西藏行仅为湖南中医药大学官网单校分数；汉族/少数民族候选类别保留，不参与省级全量闭合。");
  }
  return record;
}
...
function parseQuery(raw, rawRel, query, pageIndex) {
  ...
  if (payload.code !== 200 && payload.code !== "200") {
    return {
      records,
      summary: {
        pageKey: `hnucm-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
async function main() {
  ...
  const navHtml = await getTextRaw(rawRoot, "hnucm-official-navigation-page.html", OFFICIAL_NAV_URL, args.useCache, "text/html,*/*;q=0.9");
  const indexHtml = await getTextRaw(rawRoot, "hnucm-lqxx-index.html", INDEX_URL, args.useCache, "text/html,*/*;q=0.9");
  const jsFiles = [];
  ...
  if (!/zsdata\.hnucm\.edu\.cn\/zsdata\/lqxx/.test(navHtml)) {
    throw new Error("Official navigation page no longer links to zsdata.hnucm.edu.cn/zsdata/lqxx; refusing to import without that evidence.");
  }

  const getTypeRaw = await postJsonRaw(rawRoot, "hnucm-getType-lnfs.json", GET_TYPE_URL, { type: "lnfs" }, args.useCache);
  const globalCfgRaw = await postJsonRaw(rawRoot, "hnucm-getGlobalCfg.json", GLOBAL_CFG_URL, {}, args.useCache);
  const displayCfgRaw = await postJsonRaw(rawRoot, "hnucm-getlqxsgz-lnfs.json", DISPLAY_CFG_URL, { type: "lnfs" }, args.useCache);
  ...
  const rawFiles = [
    `${RAW_DIR}/hnucm-official-navigation-page.html`,
    `${RAW_DIR}/hnucm-lqxx-index.html`,
    ...jsFiles,
    `${RAW_DIR}/hnucm-getType-lnfs.json`,
    `${RAW_DIR}/hnucm-getGlobalCfg.json`,
    `${RAW_DIR}/hnucm-getlqxsgz-lnfs.json`,
    ...pageSummaries.map((summary) => summary.rawFile),
  ];

  const configSha256 = {};
  for (const rawFile of rawFiles.filter((file) => /hnucm-(official-navigation-page|lqxx-index|getType|getGlobalCfg|getlqxsgz|.*\.js)/.test(file))) {
  ...
  const sourceNotes = [
    {
      ...
      title: "湖南中医药大学本科招生网历年分数 API（2024-2025）",
      url: SPA_URL,
      officialNavigationUrl: OFFICIAL_NAV_URL,
      quality: SOURCE.quality,
      usage:
        "学校官网单校分专业录取分数、最低位次和录取概况边界；可用于湖南中医药大学候选边界复核、中医药/医学方向分数段趋势和西藏/新疆单校分数加厚，不替代任何省级教育考试院全量投档/录取表。",
      rawDir: RAW_DIR,
      rawFiles,
      parsedRecords: records.length,
  ...
  const output = {
    dataset: "official-national-school-admission-2024-2025-v3236-hnucm",
