...
const BASE_VERSION = "local-deterministic-v3.323-fujian-official-rank2025-published-floor-aligned-868426records";
const NEXT_VERSION = "local-deterministic-v3.324-heilongjiang-official-rank2025-no-policy-bonus-published-floor-aligned-868426records";
const SOURCE_ID = "official-heilongjiang-rank-2025-v3324";
const PROVINCE = "黑龙江";
const YEAR = 2025;
const BASE_RANKS = 126945;
const ADDED_RANKS = 1091;
const NEXT_RANKS = 128036;
const RECORDS = 868426;
const LINKED_RECORDS = 7098;
const EXCLUDED_SPECIAL_RECORDS = 84;
const LINKED_SOURCE_NOTES = 111;
const SCORE_FLOORS = { 历史类: 130, 物理类: 130 };

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-heilongjiang-rank-conversion-2025-v3324-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-heilongjiang-rank-conversion-2025-v3324-runtime-manifest.json",
  };
...
function scoreDerivedRankBoundary(note, heilongjiangCount) {
  const fields = [
  ...
  for (const [province, field] of fields) {
    const count = field === "heilongjiang2025ScoreDerivedRankRecords" ? heilongjiangCount : Number(note[field] || 0);
    if (count > 0) scopes.push(`${province}2025年${count}条`);
  }
  return `${scopes.join("、")}普通类整数最低分记录按同年一分一段表换算省级位次区间；广西严格按目标院校区内/区外选用对应加分表；山西仅覆盖本科线及以上，湖北使用含政策性加分的普通类全分段表，福建覆盖官方已公布至215分的分数档，黑龙江使用不含照顾政策分且公开至130分的文化课表；其他年份和特殊路径保持原证据状态。`;
}
...
function verifyImport(payload) {
  assert(payload.dataset === "official-heilongjiang-rank-conversion-2025-v3324-import", `Unexpected dataset ${payload.dataset}`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "Heilongjiang source note mismatch");
  assert(payload.rankConversions?.length === ADDED_RANKS, `Expected ${ADDED_RANKS} rank rows`);
  assert(payload.audit?.parsedRecords === ADDED_RANKS && payload.audit?.duplicateIds === 0, "Heilongjiang import audit drifted");
  assert(payload.audit?.officialXlsTableRows === 1093 && payload.audit?.officialPositiveRows === ADDED_RANKS, "Heilongjiang XLS row audit drifted");
  assert(payload.audit?.eolTableRows === 1094 && payload.audit?.rowComparisons === 1091 && payload.audit?.cellComparisons === 3273, "Heilongjiang cross-source audit drifted");
  assert(payload.audit?.sourceDifferences === 0 && payload.audit?.rawCountCellAnomalies === 2, "Heilongjiang source-difference audit drifted");
  assert(payload.audit?.allDerivedCountsClose === true && payload.audit?.allCumulativeRanksContinuous === true, "Heilongjiang arithmetic audit drifted");
  assert(payload.audit?.scoreBasis === "gaokao-cultural-score-excluding-policy-bonus", "Heilongjiang score basis drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "历史类").length === 528, "History row count drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "物理类").length === 563, "Physics row count drifted");
  assert(!payload.rankConversions.some((row) => (
    (row.subjectType === "历史类" && [654, 144].includes(row.score))
    || (row.subjectType === "物理类" && [136, 134].includes(row.score))
  )), "Zero-candidate scores must remain unmapped");
...
function mapAdmissionRecord(record, rankIndex, topBySubject) {
  ...
  const replacementCaution = "该位次由黑龙江2025官方普通类文化课一分段表按最低分换算，口径不含照顾政策分，非原录取/投档表直接公布。";
...
function main() {
  ...
  if (core.modelVersion === NEXT_VERSION) {
    ...
    assert(shard.rankConversions.filter((row) => row.sourceId === SOURCE_ID).length === ADDED_RANKS, "Already-applied Heilongjiang rank rows drifted");
  ...
  const item = manifest.shards[PROVINCE];
  assert(item, "Heilongjiang runtime shard is missing");
  ...
  assert(shard.records.length === 15910 && shard.records.length === item.records, "Heilongjiang record count drifted before merge");
  assert(shard.rankConversions.length === 1071 && shard.rankConversions.length === item.rankConversions, "Heilongjiang rank count drifted before merge");
  assert(!shard.rankConversions.some((row) => row.sourceId === SOURCE_ID), "Heilongjiang v3.324 ranks already exist");
  ...
  assert(officialLinked === 5706 && thirdPartyLinked === 1392, "Official/third-party linked split drifted");
  assert(schoolOfficialScopeLinked === 493, "School-official-only linked count drifted");
  ...
  assert(linkedByType.get("major-group-admission") === 4016, "Major-group linked count drifted");
  assert(linkedByType.get("major-admission") === 1722, "Major linked count drifted");
  assert(linkedByType.get("institution-admission") === 150, "Institution linked count drifted");
  assert(linkedByType.get("vocational-admission") === 1206, "Vocational linked count drifted");
  ...
  assert(shard.rankConversions.length === 2162, "Heilongjiang rank count drifted after merge");
  for (const subjectType of Object.keys(SCORE_FLOORS)) {
    ...
    assert(firstAt600?.sourceId === SOURCE_ID, `${subjectType} runtime lookup does not prioritize the official Heilongjiang source`);
  }
  ...
  for (const [sourceId, count] of linkedBySource) {
    ...
    note.heilongjiang2025ScoreDerivedRankRecords = count;
    note.heilongjiang2025RankScoreBasis = "gaokao-cultural-score-excluding-policy-bonus";
    if (Number.isFinite(Number(note.rankUnavailableRecords))) {
      assert(Number(note.rankUnavailableRecords) >= count, `${sourceId} rank-unavailable source count is smaller than the Heilongjiang linked count`);
    ...
    if (Number.isFinite(Number(note.scoreOnlyRecords))) {
      assert(Number(note.scoreOnlyRecords) >= count, `${sourceId} score-only source count is smaller than the Heilongjiang linked count`);
  ...
  layer.currentFinding = "新增黑龙江2025年历史类528条、物理类563条官方文化课一分段记录；1091个正人数分数档与考试院XLS、黑龙江省政府发布页及EOL完整表交叉核验，3273个分数/人数/累计人数单元零差异。由此为7098条黑龙江2025普通类整数最低分补充不含照顾政策分口径的省级位次区间，其中5706条来自官方来源；84条特殊路径继续隔离。";
  layer.downgradeReason = `当前数据层 ${NEXT_VERSION} 严格区分院校原生最低位次与最低分换算位次；黑龙江2025文化课一分段表明确不含照顾政策分，官方公开至130分，历史类654/144分和物理类136/134分为零人数缺口。艺术体育综合分、科类不明、非整数分、低于公开分数档和特殊路径不混入普通类自动推荐。`;
  ...
  manifest.runtimeProfile = { ...(manifest.runtimeProfile || {}), version: "v3.324", initialCore: "knowledge-core-lite.json.gz", fullEvidenceCore: "knowledge-core.json.gz" };
  ...
  const tempDir = path.join(releaseDir, `.v3324-${process.pid}`);
  ...
  const runtimeManifest = {
    dataset: "official-heilongjiang-rank-conversion-2025-v3324-runtime",
    generatedAt: payload.generatedAt,
    sourceId: SOURCE_ID,
    importFile: args.importFile,
    releaseDir: args.releaseDir,
    before: { modelVersion: BASE_VERSION, records: RECORDS, rankConversions: BASE_RANKS, provinceRankConversions: 1071 },
    after: {
