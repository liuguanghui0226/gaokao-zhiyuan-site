...
const DEFAULT_MANIFEST = "data/admissions/runtime-core-lite-v3336-manifest.json";
const EXPECTED_VERSION = "local-deterministic-v3.336-xizang-rank-attestation-input-binding-required-868426records";
const SOURCE_NOTE_FIELDS = [
  ...
  "scoreBasis",
  "rankPolicyBonusIncluded",
  "policyBonusStatus",
  "automaticAdmissionScoreAlignmentAllowed",
  "alignmentBlockReason",
  "scoreBasisAuditEvidenceId",
  "rankUsageRequired",
  "lowerAggregateEstimateGuard",
  "lowerAggregateGuardedRecords",
  "rankPublicationStatus",
  "publicRankConversionAvailable",
  "candidateCategoryRequired",
  "candidateCategoryDefinitions",
  "reviewedOfficialChannels",
  "manualRankSourceConfirmationRequired",
  "acceptedManualRankSources",
  "manualRankSourceLabel",
  "manualRankAttestationBoundFields",
  "staleRecommendationInvalidationRequired",
  "officialPublicDisclosureAudit",
];

function parseArgs(argv) {
  const args = { releaseDir: DEFAULT_RELEASE_DIR, manifest: DEFAULT_MANIFEST };
  for (let index = 0; index < argv.length; index += 1) {
    if (argv[index] === "--release-dir") args.releaseDir = argv[++index];
...
const auditFile = path.resolve(PROJECT_ROOT, args.manifest);
...
if (fullCore.modelVersion !== EXPECTED_VERSION) throw new Error(`Unexpected model version ${fullCore.modelVersion}`);
...
const liteCore = {
  ...fullCore,
  admissionScoreLayer: { ...fullCore.admissionScoreLayer, sourceNotes: compactNotes },
  browserRuntime: {
...
const audit = {
  dataset: "runtime-core-lite-v3336",
  generatedAt: new Date().toISOString(),
  releaseDir: path.relative(PROJECT_ROOT, releaseDir),
  modelVersion: fullCore.modelVersion,
  profile: "core-lite-v1",
  sourceNotes: { full: fullNotes.length, lite: compactNotes.length, fields: SOURCE_NOTE_FIELDS },
  fullCore: {
    file: path.relative(PROJECT_ROOT, coreFile),
    bytes: fullRaw.byteLength,
    compressedBytes: fs.statSync(coreFile).size,
    sha256: sha256(fullRaw),
  },
  liteCore: {
    file: path.relative(PROJECT_ROOT, liteFile),
    bytes: liteWritten.raw.byteLength,
    compressedBytes: liteWritten.compressed.byteLength,
    sha256: sha256(liteWritten.raw),
  ...
  runtimeManifest: {
    file: path.relative(PROJECT_ROOT, manifestFile),
  ...
  boundary: "The full evidence core remains published; the browser initial core retains only source-note fields used by the UI.",
};
