...
const auditFile = path.join(projectRoot, "data/admissions/runtime-core-lite-v3315-manifest.json");
...
assert.equal(lite.modelVersion, "local-deterministic-v3.315-hebei-official-rank2025-aligned-868426records");
...
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 118702);
assert.equal(liteNotes.length, fullNotes.length);
assert.equal(liteNotes.length, 5119);
...
const rankSource = liteNotes.find((note) => note.id === "official-hebei-rank-2025-v3315");
assert.ok(rankSource);
assert.equal(rankSource.province, "河北");
assert.equal(rankSource.year, 2025);
assert.equal(rankSource.quality, "official-hebei-rank-conversion-pdf-ocr-full-table-cross-verified");
assert.equal(rankSource.url, "http://www.hebeea.edu.cn/html/xxgl/tzgg/2025/0624-194513-559.html");
...
assert.equal(manifest.coreLite.sourceNotes, 5119);
assert.equal(manifest.runtimeProfile.version, "v3.315");
...
assert.equal(audit.dataset, "runtime-core-lite-v3315");
