...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3286.json");
const expectedCovered = ["安徽", "北京", "广东", "河南", "湖南", "江西", "吉林", "陕西", "山东", "四川", "新疆", "西藏", "浙江"];
...
assert(coveredProvinces.length === 13 && missingProvinces.length === 18, "Expected 13 covered and 18 missing provinces after Beijing v3.286");
const beijing = coverage.find((row) => row.province === "北京");
assert(beijing?.ordinaryRecords === 2, "Expected two Beijing ordinary boundaries");
assert(beijing.boundaries.some((row) => row.routeKind === "ordinary-bachelor" && row.minScore === 429 && row.scoreBasis === "gaokao-total"), "Beijing bachelor boundary drifted");
assert(beijing.boundaries.some((row) => row.routeKind === "ordinary-vocational" && row.minScore === 120 && row.scoreBasis === "chinese-math-foreign-450"), "Beijing vocational score basis drifted");

const report = {
  dataset: "official-control-line-coverage-2026-v3286",
  generatedAt: core.generatedAt,
  modelVersion: core.modelVersion,
  definition: "运行分片中2026年普通类本科/专科或普通类一段/二段省级控制线；特殊类型、艺体、军警、专项、定向、预科、少数民族和对口路径不计入。不同控制线分数口径保持分列。",
