...
const TMP_ROOT = path.join(os.homedir(), ".codex", "tmp", "gaokao-xinjiang-undergraduate2-2025");
const DEFAULT_URL = "https://www.xjzk.gov.cn/c/2025-08-06/494702.shtml";
const DEFAULT_OUT = "data/admissions/official-xinjiang-undergraduate2-filing-2025-import.json";
const VISION_HELPER = path.join(PROJECT_ROOT, "scripts", "vision-table-row-ocr.swift");
const YEAR = 2025;
const PROVINCE = "新疆";
const BATCH = "本科二批";
const SOURCE_ID = "official-xinjiang-undergraduate2-filing-2025";
...
const ROW_CORRECTIONS = new Map([
  ["29677|1265", { schoolName: "新疆天山职业技术大学" }],
  ["29677|3746", { schoolName: "河北石油职业技术大学" }],
  ["29677|3794", { schoolName: "河北工业职业技术大学" }],
  ["29677|3838", { schoolName: "兰州石化职业技术大学" }],
  ["29679|3010", { schoolName: "甘肃政法大学" }],
  ["29679|7794", { schoolName: "甘肃林业职业技术大学" }],
  ["29679|7841", { schoolName: "江西应用科技学院" }],
  ["29681|2689", {
    minScore: 295,
    maxScore: 322,
    avgScore: 301.71,
    tieBreakScores: { totalScore: 295, chinese: 63, comprehensive: 161, math: 101 },
  }],
  ["29681|4392", { schoolName: "新疆政法学院" }],
  ["29681|6966", { schoolName: "新疆交通职业技术大学" }],
  ["29681|8117", { schoolCode: "3128", schoolName: "吉林铁道职业技术大学" }],
]);
...
function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-xinjiang-undergraduate2-filing-2025.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-xinjiang-undergraduate2-filing-2025.mjs --use-cache",
    "",
...
async function downloadText(url) {
  const response = await fetch(url, {
    headers: {
      "user-agent": "Mozilla/5.0 gaokao-xinjiang-undergraduate2-importer/1.0",
...
function visionItemsForImage(file, cacheDir, dimensions, visionBinary) {
  ...
  const imageId = path.basename(file, ".png");
...
function schoolTags(record) {
  ...
  if (Number(record.minScore) < 350) tags.push("本科二批低分边界");
  return [...new Set(tags)];
}
...
function columnsFor(width) {
  ...
  return {
    ...
    filed: [s(455), s(535)],
    high: [s(535), s(625)],
    minTotal: [s(625), s(705)],
    chinese: [s(705), s(790)],
    comprehensive: [s(790), s(870)],
    math: [s(870), s(950)],
    avg: [s(950), width + 10],
  };
}
...
function parseImageRows({ image, items, dimensions }) {
  ...
  for (const anchor of anchors) {
    ...
    const base = {
      ...
      sourceId: SOURCE_ID,
      sourceQuality: "official-xinjiang-2025-undergraduate2-institution-filing-image-ocr-score-only",
    ...
    base.id = `${YEAR}-xinjiang-undergrad2-filing-${hash(idBase, 18)}`;
...
async function main() {
  ...
  fs.writeFileSync(outPath, JSON.stringify({
    ...
    sourceNotes: [{
      ...
      quality: "official-xinjiang-2025-undergraduate2-institution-filing-image-ocr-score-only",
      usage: `官方普通类本科二批次院校投档图片表，经本地 OCR 抽取 ${records.length} 条院校投档线；无最低位次，按 score-only 本科二批进档边界使用。`,
