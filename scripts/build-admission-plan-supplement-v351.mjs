...
const DEFAULT_OUTPUT = "site/data/release-v3.275/admission-plan-supplement-v351.json.gz";
const INPUTS = [
  ...
  "data/admissions/official-national-school-plan-2026-v350-maotai-import.json",
  "data/admissions/official-national-school-plan-2026-v351-xmu-import.json",
];
const VERSION = "v3.351";
const RECORD_FIELDS = [
...
function usage() {
  return `Usage:\n  node scripts/build-admission-plan-supplement-v351.mjs --out ${DEFAULT_OUTPUT}`;
}
...
function main() {
  ...
  const records = inputs.flatMap((input) => input.parsed.records.map(compactRecord));
  ...
  const aggregateSource = {
    ...
    title: "官方院校 2026 分省分专业计划补充（苏州大学、暨南大学、茅台学院、厦门大学）",
    publisher: "苏州大学、暨南大学、茅台学院、厦门大学招生办公室",
    url: "https://zs.xmu.edu.cn/info/1044/36612.htm",
    quality: "official-school-national-plan-api-html-2026-multi-source",
    usage: "运行时独立补充层，汇总四所高校官网 2026 年分省分专业计划；只用于当年专业池、计划数、科类、选科和路径约束，不替代省级考试院计划、投档线或录取概率。",
  };
  const payload = {
    version: VERSION,
