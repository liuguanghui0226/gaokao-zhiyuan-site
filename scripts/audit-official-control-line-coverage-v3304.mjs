...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3304.json");
...
assert(manifest.modelVersion === "local-deterministic-v3.304-yunnan-control-lines2026-art-thresholds-and-rank-image-provenance-847238records", "Unexpected v3.304 model version");
assert(manifest.recordCount === 847238, "Unexpected v3.304 record count");
assert(core.admissionScoreLayer.coverage.dataTypes["control-line"] === 1592, "Unexpected v3.304 control-line count");
...
assert(coveredProvinces.length === 31 && missingProvinces.length === 0, "Expected all 31 provinces covered after Yunnan v3.304");
...
assert(yunnan?.ordinaryRecords === 4, "Expected four Yunnan ordinary boundaries");
for (const expected of [
  ["历史类", "ordinary-bachelor", 465],
  ["历史类", "ordinary-vocational", 180],
  ["物理类", "ordinary-bachelor", 435],
  ["物理类", "ordinary-vocational", 180],
]) {
  assert(yunnan.boundaries.some((row) => row.subjectType === expected[0] && row.routeKind === expected[1] && row.minScore === expected[2] && row.scoreBasis === "gaokao-total"), `Yunnan boundary drifted: ${expected.join("/")}`);
}
assert(!yunnan.boundaries.some((row) => [545, 505, 380, 365, 345, 325].includes(row.minScore)), "Yunnan special, art or sports lines leaked into ordinary coverage");
assert(yunnan.sourceIds.includes("official-yunnan-control-lines-2026"), "Yunnan official control source is missing");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3304",
