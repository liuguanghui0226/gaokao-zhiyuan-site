...
const BASE_VERSION = "local-deterministic-v3.306-national-school-official-nchu2021-2025-derived-rank-852098records";
const NEXT_VERSION = "local-deterministic-v3.307-national-school-official-jxust2023-2025-native-rank-855003records";
const SOURCE_ID = "official-jxust-national-2023-2025-school-major-admission";
const EXPECTED_ADDED_RECORDS = 2905;
const EXPECTED_NATIVE_RANKS = 2704;
const EXPECTED_BASE_RECORDS = 852098;
const EXPECTED_NEXT_RECORDS = 855003;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-national-school-admission-2023-2025-v3307-jxust-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-national-school-admission-2023-2025-v3307-jxust-runtime-manifest.json",
  };
...
function verifyImport(payload) {
  assert(payload.dataset === "official-national-school-admission-2023-2025-v3307-jxust", `Unexpected import dataset ${payload.dataset}`);
  assert(payload.records?.length === EXPECTED_ADDED_RECORDS, `Expected ${EXPECTED_ADDED_RECORDS} records`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "JXUST source note mismatch");
  assert(new Set(payload.records.map((record) => record.id)).size === EXPECTED_ADDED_RECORDS, "JXUST import contains duplicate ids");
  assert(payload.records.every((record) => record.sourceId === SOURCE_ID && record.schoolName === "江西理工大学" && record.dataType === "major-admission"), "JXUST import contains out-of-scope rows");
  assert(payload.records.filter((record) => record.minRankEnd).length === EXPECTED_NATIVE_RANKS, "JXUST native-rank count drifted");
  assert(payload.records.filter((record) => record.rankDerivedFromScore).length === 0, "JXUST must not contain derived ranks");
  assert(payload.records.filter((record) => record.formalScoreScope === "school-official-only").length === 2596, "JXUST ordinary count drifted");
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 309, "JXUST special-path count drifted");
  assert(payload.records.filter((record) => record.minRankEnd).every((record) => record.rankEvidenceScope === "school-recorded-min-score-rank" && record.nativeAdmissionRankUnavailable === false), "JXUST rank evidence semantics drifted");
}
...
function alreadyApplied(core, manifest, releaseDir) {
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(rows.every((record) => record.province === province), `JXUST province drift in ${province}`);
    count += rows.length;
  }
  assert(count === EXPECTED_ADDED_RECORDS, `Already-applied JXUST row count drifted: ${count}`);
...
function main() {
  ...
  assert(byProvince.size === 31 && byProvince.has("西藏") && byProvince.has("宁夏"), "JXUST province coverage drifted");
  const tempDir = path.join(releaseDir, `.v3307-${process.pid}`);
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(!shard.records.some((record) => record.sourceId === SOURCE_ID), `${province} already contains JXUST rows`);
  ...
  layer.currentFinding = "新增江西理工大学2023-2025年31省2905条官方分专业最低/平均/最高分和录取人数；2704条直接保留官网表列最低分位次，201条位次未公开记录保持rankUnavailable。普通school-official-only与艺体、预科、专项、定向、征集、中外合作等special-path-only保持隔离，学校单校证据最高只到A-候选复核。";
  ...
  coverage.schools = sortedUnique([...(coverage.schools || []), "江西理工大学"]);
  coverage.cities = sortedUnique([...(coverage.cities || []), "赣州"]);
  coverage.schoolTags = sortedUnique([...(coverage.schoolTags || []), "公办", "理工", "江西省属"]);
  ...
  for (const [yearText, count] of Object.entries(payload.audit.yearCounts)) {
    ...
    if (!existingSchoolNames.has("江西理工大学")) row.schools = Number(row.schools || 0) + 1;
  }
  ...
  const runtimeManifest = {
    dataset: "official-national-school-admission-2023-2025-v3307-jxust-runtime",
