...
const auditFile = path.join(projectRoot, "data/admissions/runtime-core-lite-v3316-manifest.json");
...
assert.equal(lite.modelVersion, "local-deterministic-v3.316-chongqing-authority-linked-rank2025-aligned-868426records");
...
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 119677);
assert.equal(liteNotes.length, fullNotes.length);
assert.equal(liteNotes.length, 5120);
...
const rankSource = liteNotes.find((note) => note.id === "official-chongqing-rank-2025-v3316");
assert.ok(rankSource);
assert.equal(rankSource.province, "重庆");
assert.equal(rankSource.year, 2025);
assert.equal(rankSource.quality, "official-content-mirror-eol-chongqing-exam-authority-linked-full-table-dxsbb-cross-verified");
assert.equal(rankSource.url, "https://gaokao.eol.cn/chong_qing/dongtai/202506/t20250624_2676752.shtml");
...
assert.equal(manifest.coreLite.sourceNotes, 5120);
...
assert.equal(audit.dataset, "runtime-core-lite-v3316");
