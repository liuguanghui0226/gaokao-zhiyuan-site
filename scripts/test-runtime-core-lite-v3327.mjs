...
const audit = JSON.parse(fs.readFileSync(
  path.join(projectRoot, "data/admissions/runtime-core-lite-v3327-manifest.json"),
  "utf8",
));

assert.equal(lite.modelVersion, "local-deterministic-v3.327-tianjin-official-rank2025-policy-bonus-inclusive-full-table-aligned-868426records");
assert.equal(lite.modelVersion, full.modelVersion);
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 128972);
...
const tianjin = lite.admissionScoreLayer.sourceNotes.find((note) => note.id === "official-tianjin-rank-2025-v3327");
assert.equal(tianjin.scoreBasis, "gaokao-total-including-policy-bonus");
assert.equal(tianjin.rankPolicyBonusIncluded, true);
...
assert.equal(manifest.coreLite.sourceNotes, 5131);
assert.equal(manifest.runtimeProfile.version, "v3.327");
assert.equal(audit.dataset, "runtime-core-lite-v3327");
...
console.log("Runtime core-lite v3.327 tests passed");
