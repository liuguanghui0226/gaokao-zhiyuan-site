...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2021-2025-v3238-uestc-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2021-2025-v3238-uestc";
const OFFICIAL_NAV_URL = "https://zs.uestc.edu.cn/";
const INDEX_URL = "https://chaxun.uestc.edu.cn/public/zsdata/lqxx/";
const SPA_URL = "https://chaxun.uestc.edu.cn/public/zsdata/lqxx/#/lnfs";
const BASE_API_URL = "https://chaxun.uestc.edu.cn/lqxx/s";
...
const SOURCE = {
  id: "official-uestc-national-2021-2025-school-admission",
  quality: "official-school-uestc-2021-2025-national-api-score-only",
  schoolCode: "10614",
  schoolName: "电子科技大学",
  city: "四川成都",
  publisher: "电子科技大学本科招生办公室",
  tags: ["四川", "成都", "电子科技大学", "985", "211", "双一流", "电子信息"],
};

function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2021-2025-v3238-uestc.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2021-2025-v3238-uestc.mjs --use-cache --concurrency 4",
    "",
    "Imports 电子科技大学本科招生网 2021-2025 历年分数 official API data.",
  ].join("\n");
}
...
async function postJsonRaw(rawRoot, rawFile, url, body, useCache) {
  ...
  const text = await requestText(url, {
    ...
    headers: {
      ...
      origin: "https://chaxun.uestc.edu.cn",
    },
...
function classifyAdmission(row) {
  ...
  if (/专项|预科|内高班|西藏班|单列|南疆|定向|援疆|民族班|优师/.test(text)) {
...
function queryRawRel(query, index) {
  ...
  return `uestc-${query.nf}-${slugify(query.sf)}-${slugify(query.klmc)}-${slugify(query.xqmc)}-${slugify(query.zslb)}-${index}-${hash}.json`;
}
...
function buildRecord(row, rawRel, query, pageIndex, rowIndex, dataType) {
  ...
  const idPrefix = dataType === "institution-admission" ? "uestc-summary" : "uestc";
  const record = {
    ...
    sourcePageKey: `uestc-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
function parseQuery(raw, rawRel, query, pageIndex) {
  ...
  if (payload.code !== 200 && payload.code !== "200") {
    return {
      records,
      summary: {
        pageKey: `uestc-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
async function main() {
  ...
  const navHtml = await getTextRaw(rawRoot, "uestc-official-navigation-page.html", OFFICIAL_NAV_URL, args.useCache, "text/html,*/*;q=0.9");
  const indexHtml = await getTextRaw(rawRoot, "uestc-lqxx-index.html", INDEX_URL, args.useCache, "text/html,*/*;q=0.9");
  const jsFiles = [];
  ...
  if (!/chaxun\.uestc\.edu\.cn\/public\/zsdata\/lqxx/.test(navHtml)) {
    throw new Error("Official navigation page no longer links to chaxun.uestc.edu.cn/public/zsdata/lqxx; refusing to import without that evidence.");
  }

  const getTypeRaw = await postJsonRaw(rawRoot, "uestc-getType-lnfs.json", GET_TYPE_URL, { type: "lnfs" }, args.useCache);
  const globalCfgRaw = await postJsonRaw(rawRoot, "uestc-getGlobalCfg.json", GLOBAL_CFG_URL, {}, args.useCache);
  const displayCfgRaw = await postJsonRaw(rawRoot, "uestc-getlqxsgz-lnfs.json", DISPLAY_CFG_URL, { type: "lnfs" }, args.useCache);
  ...
  const rawFiles = [
    `${RAW_DIR}/uestc-official-navigation-page.html`,
    `${RAW_DIR}/uestc-lqxx-index.html`,
    ...jsFiles,
    `${RAW_DIR}/uestc-getType-lnfs.json`,
    `${RAW_DIR}/uestc-getGlobalCfg.json`,
    `${RAW_DIR}/uestc-getlqxsgz-lnfs.json`,
    ...pageSummaries.map((summary) => summary.rawFile),
  ];

  const configSha256 = {};
  for (const rawFile of rawFiles.filter((file) => /uestc-(official-navigation-page|lqxx-index|getType|getGlobalCfg|getlqxsgz|.*\.js)/.test(file))) {
  ...
  const sourceNotes = [
    {
      ...
      title: "电子科技大学本科招生网历年分数 API（2021-2025）",
      url: SPA_URL,
      officialNavigationUrl: OFFICIAL_NAV_URL,
      quality: SOURCE.quality,
      usage:
        "学校官网单校分专业录取分数边界；可用于电子科技大学候选边界复核、985/211/双一流和电子信息方向分数段趋势及全国单校分数加厚，不替代任何省级教育考试院全量投档/录取表。",
      rawDir: RAW_DIR,
      rawFiles,
      parsedRecords: records.length,
  ...
  const output = {
    dataset: "official-national-school-admission-2021-2025-v3238-uestc",
