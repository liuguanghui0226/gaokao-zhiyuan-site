...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3288.json");
const expectedCovered = ["安徽", "北京", "广东", "河南", "湖南", "江西", "吉林", "内蒙古", "陕西", "山东", "四川", "天津", "新疆", "西藏", "浙江"];
...
assert(coveredProvinces.length === 15 && missingProvinces.length === 16, "Expected 15 covered and 16 missing provinces after Inner Mongolia v3.288");
...
const neimenggu = coverage.find((row) => row.province === "内蒙古");
assert(neimenggu?.ordinaryRecords === 4, "Expected four Inner Mongolia ordinary boundaries");
for (const expected of [
  ["物理类", "ordinary-bachelor", 363],
  ["物理类", "ordinary-vocational", 160],
  ["历史类", "ordinary-bachelor", 403],
  ["历史类", "ordinary-vocational", 160],
]) {
  assert(neimenggu.boundaries.some((row) => row.subjectType === expected[0] && row.routeKind === expected[1] && row.minScore === expected[2] && row.scoreBasis === "gaokao-total"), `Inner Mongolia boundary drifted: ${expected.join("/")}`);
}
assert(!neimenggu.boundaries.some((row) => [488, 512, 323, 326].includes(row.minScore)), "Inner Mongolia special or project lines leaked into ordinary coverage");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3288",
