...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3300.json");
const expectedCovered = ["安徽", "北京", "重庆", "福建", "甘肃", "广东", "广西", "贵州", "海南", "河北", "河南", "黑龙江", "湖北", "湖南", "吉林", "江苏", "江西", "辽宁", "内蒙古", "山东", "陕西", "上海", "四川", "天津", "西藏", "新疆", "浙江"];
...
assert(manifest.modelVersion === "local-deterministic-v3.300-liaoning-control-lines2026-and-rank-provenance-847095records", "Unexpected v3.300 model version");
assert(manifest.recordCount === 847095, "Unexpected v3.300 record count");
assert(core.admissionScoreLayer.coverage.dataTypes["control-line"] === 1449, "Unexpected v3.300 control-line count");
...
assert(coveredProvinces.length === 27 && missingProvinces.length === 4, "Expected 27 covered and 4 missing provinces after Liaoning v3.300");
assert(JSON.stringify([...missingProvinces].sort()) === JSON.stringify(["宁夏", "青海", "山西", "云南"].sort()), `Unexpected missing provinces: ${missingProvinces.join("、")}`);
...
assert(jiangsu.boundaries.some((row) => row.subjectType === "历史类" && row.routeKind === "ordinary-bachelor" && row.minScore === 484), "Jiangsu history boundary drifted");
assert(jiangsu.boundaries.some((row) => row.subjectType === "物理类" && row.routeKind === "ordinary-bachelor" && row.minScore === 456), "Jiangsu physics boundary drifted");
...
for (const expected of [
  ...
  assert(liaoning.boundaries.some((row) => row.subjectType === expected[0] && row.routeKind === expected[1] && row.minScore === expected[2] && row.scoreBasis === "gaokao-total"), `Liaoning boundary drifted: ${expected.join("/")}`);
}
assert(!liaoning.boundaries.some((row) => [527, 508, 331, 258, 250, 221, 172].includes(row.minScore)), "Liaoning special, art or sports lines leaked into ordinary coverage");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3300",
