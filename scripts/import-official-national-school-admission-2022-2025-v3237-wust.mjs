...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2022-2025-v3237-wust-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2022-2025-v3237-wust";
const OFFICIAL_NAV_URL = "https://zs.wust.edu.cn/";
const INDEX_URL = "https://zsdata.wust.edu.cn/zsdata/lqxx/";
const SPA_URL = "https://zsdata.wust.edu.cn/zsdata/lqxx/#/lnfs";
const BASE_API_URL = "https://zsdata.wust.edu.cn/lqxx/s";
...
const SOURCE = {
  id: "official-wust-national-2022-2025-school-admission",
  quality: "official-school-wust-2022-2025-national-api-score-rank",
  schoolCode: "10488",
  schoolName: "武汉科技大学",
  city: "湖北武汉",
  publisher: "武汉科技大学本科招生办公室",
  tags: ["湖北", "武汉", "武汉科技大学", "工科", "冶金", "医工交叉"],
};

function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2022-2025-v3237-wust.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2022-2025-v3237-wust.mjs --use-cache --concurrency 4",
    "",
    "Imports 武汉科技大学本科招生网 2022-2025 历年分数 official API data.",
  ].join("\n");
}
...
async function postJsonRaw(rawRoot, rawFile, url, body, useCache) {
  ...
  const text = await requestText(url, {
    ...
    headers: {
      ...
      origin: "https://zsdata.wust.edu.cn",
    },
...
function queryRawRel(query, index) {
  ...
  return `wust-${query.nf}-${slugify(query.sf)}-${slugify(query.klmc)}-${slugify(query.xqmc)}-${slugify(query.zslb)}-${index}-${hash}.json`;
}
...
function buildRecord(row, rawRel, query, pageIndex, rowIndex, dataType) {
  ...
  const idPrefix = dataType === "institution-admission" ? "wust-summary" : "wust";
  const record = {
    ...
    sourcePageKey: `wust-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
function parseQuery(raw, rawRel, query, pageIndex) {
  ...
  if (payload.code !== 200 && payload.code !== "200") {
    return {
      records,
      summary: {
        pageKey: `wust-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
async function main() {
  ...
  const navHtml = await getTextRaw(rawRoot, "wust-official-navigation-page.html", OFFICIAL_NAV_URL, args.useCache, "text/html,*/*;q=0.9");
  const indexHtml = await getTextRaw(rawRoot, "wust-lqxx-index.html", INDEX_URL, args.useCache, "text/html,*/*;q=0.9");
  const jsFiles = [];
  ...
  if (!/zsdata\.wust\.edu\.cn\/zsdata\/lqxx/.test(navHtml)) {
    throw new Error("Official navigation page no longer links to zsdata.wust.edu.cn/zsdata/lqxx; refusing to import without that evidence.");
  }

  const getTypeRaw = await postJsonRaw(rawRoot, "wust-getType-lnfs.json", GET_TYPE_URL, { type: "lnfs" }, args.useCache);
  const globalCfgRaw = await postJsonRaw(rawRoot, "wust-getGlobalCfg.json", GLOBAL_CFG_URL, {}, args.useCache);
  const displayCfgRaw = await postJsonRaw(rawRoot, "wust-getlqxsgz-lnfs.json", DISPLAY_CFG_URL, { type: "lnfs" }, args.useCache);
  ...
  const rawFiles = [
    `${RAW_DIR}/wust-official-navigation-page.html`,
    `${RAW_DIR}/wust-lqxx-index.html`,
    ...jsFiles,
    `${RAW_DIR}/wust-getType-lnfs.json`,
    `${RAW_DIR}/wust-getGlobalCfg.json`,
    `${RAW_DIR}/wust-getlqxsgz-lnfs.json`,
    ...pageSummaries.map((summary) => summary.rawFile),
  ];

  const configSha256 = {};
  for (const rawFile of rawFiles.filter((file) => /wust-(official-navigation-page|lqxx-index|getType|getGlobalCfg|getlqxsgz|.*\.js)/.test(file))) {
  ...
  const sourceNotes = [
    {
      ...
      title: "武汉科技大学本科招生网历年分数 API（2022-2025）",
      url: SPA_URL,
      officialNavigationUrl: OFFICIAL_NAV_URL,
      quality: SOURCE.quality,
      usage:
        "学校官网单校分专业录取分数、最低位次和录取概况边界；可用于武汉科技大学候选边界复核、湖北/工科/医工交叉方向分数段趋势和全国单校分数加厚，不替代任何省级教育考试院全量投档/录取表。",
      rawDir: RAW_DIR,
      rawFiles,
      parsedRecords: records.length,
  ...
  const output = {
    dataset: "official-national-school-admission-2022-2025-v3237-wust",
