...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2022-2025-v3260-cdu-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2022-2025-v3260-cdu";
const OFFICIAL_HOME_URL = "https://zhaosheng.cdu.edu.cn/";
const INDEX_URL = "https://zhaosheng.cdu.edu.cn/zsdata/lnfs";
const SPA_URL = "https://zhaosheng.cdu.edu.cn/zsdata/lnfs";
const BASE_API_URL = "https://zhaosheng.cdu.edu.cn/zsdata/api/lqxx/s";
...
const SOURCE = {
  id: "official-cdu-national-2022-2025-school-admission",
  quality: "official-school-cdu-2022-2025-national-api-score-rank",
  schoolCode: "11079",
  schoolName: "成都大学",
  city: "成都",
  publisher: "成都大学招生办公室",
  tags: ["四川", "成都", "成都大学", "综合", "公办"],
};
...
function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2022-2025-v3260-cdu.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2022-2025-v3260-cdu.mjs --use-cache --concurrency 4",
    "",
    "Imports 成都大学 2022-2025 历年分数 official API data.",
  ].join("\n");
}
...
function rawAssetFile(url) {
  return `cdu-${path.basename(new URL(url).pathname)}`;
}
...
function classifyAdmission(row) {
  ...
  if (/乡村振兴/.test(text)) return { admissionType: "乡村振兴", formalScoreScope: "special-path-only" };
  if (/帮扶计划/.test(text)) return { admissionType: "帮扶计划", formalScoreScope: "special-path-only" };
  if (/中外高水平大学学生交流计划/.test(text)) return { admissionType: "特殊路径", formalScoreScope: "special-path-only" };
  if (/中职|对口|旅游服务类|职教本科|单独考试|普通中高计划/.test(text)) return { admissionType: "特殊路径", formalScoreScope: "special-path-only" };
  if (/中外合作|普通中外|合作办学|专项|预科|一类模式|八协|内高班|西藏班|西藏区内|内地西藏|内地新疆|藏线|单列|南疆|新疆协作|定向|援疆|民族班|新疆班|优师|运动队|综合评价|提前批|公费师范/.test(text)) {
...
function queryRawRel(query, index) {
  ...
  return `cdu-${query.nf}-${slugify(query.sf)}-${slugify(query.klmc)}-${slugify(query.xqmc)}-${slugify(query.zslb)}-${index}-${hash}.json`;
}
...
function buildRecord(row, rawRel, query, pageIndex, rowIndex, dataType) {
  ...
  const idPrefix = dataType === "institution-admission" ? "cdu-summary" : "cdu";
  const record = {
    ...
    sourcePageKey: `cdu-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
function parseQuery(raw, rawRel, query, pageIndex) {
  ...
  if (payload.code !== 200 && payload.code !== "200") {
    return {
      records,
      summary: {
        pageKey: `cdu-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
async function main() {
  ...
  const officialHomeRawFile = "cdu-lqxx-spa-page.html";
  ...
  try {
    ...
    const warningRawFile = "cdu-lqxx-spa-fetch-warning.json";
    ...
    fs.writeFileSync(path.join(rawRoot, warningRawFile), `${JSON.stringify({
      ...
      boundary: "The importer does not infer data from this unavailable shell page; records are sourced from the official zhaosheng.cdu.edu.cn SPA/API responses below.",
    }, null, 2)}\n`);
  }
  const officialHomeHtml = fs.existsSync(path.join(rawRoot, officialHomeRawFile))
    ...
    : "";
  const indexHtml = await getTextRaw(rawRoot, "cdu-lqxx-index.html", INDEX_URL, args.useCache, "text/html,*/*;q=0.9");
  const jsFiles = [];
  ...
  if (officialHomeHtml && !/lqxx|lqcx|历年|分数|录取|招生/.test(officialHomeHtml)) {
    throw new Error("CDU historical-score SPA shell no longer matches the expected admission-query surface; refusing to import without that evidence.");
  }

  const getTypeRaw = await postJsonRaw(rawRoot, "cdu-getType-lnfs.json", GET_TYPE_URL, { type: "lnfs" }, args.useCache);
  const themeCfgRaw = await postJsonRaw(rawRoot, "cdu-getTheme.json", THEME_CFG_URL, {}, args.useCache);
  const displayCfgRaw = await postJsonRaw(rawRoot, "cdu-getlqxsgz-lnfs.json", DISPLAY_CFG_URL, { type: "lnfs" }, args.useCache);
  const campusCfgRaw = await postJsonRaw(rawRoot, "cdu-xqlxXsCfg.json", CAMPUS_CFG_URL, {}, args.useCache);
  ...
  const rawFiles = [
    ...
    `${RAW_DIR}/cdu-lqxx-index.html`,
    ...jsFiles,
    `${RAW_DIR}/cdu-getType-lnfs.json`,
    `${RAW_DIR}/cdu-getTheme.json`,
    `${RAW_DIR}/cdu-getlqxsgz-lnfs.json`,
    `${RAW_DIR}/cdu-xqlxXsCfg.json`,
    ...pageSummaries.map((summary) => summary.rawFile),
  ];

  const configSha256 = {};
  for (const rawFile of rawFiles.filter((file) => /cdu-(lqxx-spa-page|lqxx-spa-fetch-warning|lqxx-index|getType|getTheme|getlqxsgz|xqlxXsCfg|.*\.js)/.test(file))) {
  ...
  const sourceNotes = [
    {
      ...
      title: "成都大学历年分数 API（2022-2025）",
      url: SPA_URL,
      officialHomeUrl: OFFICIAL_HOME_URL,
      quality: SOURCE.quality,
      usage:
        "学校官网单校分专业录取最高分/平均分/最低分/最低位次边界；可用于成都大学候选边界复核、成都/四川综合类院校方向分数段趋势和全国单校分数加厚，不替代任何省级教育考试院全量投档/录取表。源表公开最低位次的行保留最低位次；缺失位次的行保持 rankUnavailable=true 且不生成假位次。",
      rawDir: RAW_DIR,
      rawFiles,
      parsedRecords: records.length,
      ...
      boundaryNotes: [
        ...
        "艺术体育、国家/地方专项、中外合作、普通中高计划、职教本科、对口/单独考试、公费师范、一类模式预科、单列类等特殊路径按 special-path-only 隔离。",
        "源系统 typeMap 当前未列西藏，本包不生成西藏假记录；西藏省级正式全量投档/录取表缺口不因本源改变。",
        "新疆及其他特殊路径行作为学校官网单校候选边界保留，不当作省级正式全量表。",
      ],
    },
  ];

  const output = {
    dataset: "official-national-school-admission-2022-2025-v3260-cdu",
