...
const BASE_VERSION = "local-deterministic-v3.317-liaoning-official-mirror-rank2025-aligned-868426records";
const NEXT_VERSION = "local-deterministic-v3.318-hunan-education-department-rank2025-aligned-868426records";
const SOURCE_ID = "official-hunan-rank-2025-v3318";
const PROVINCE = "湖南";
const YEAR = 2025;
const BASE_RANKS = 120750;
const ADDED_RANKS = 1139;
const NEXT_RANKS = 121889;
const RECORDS = 868426;
const LINKED_RECORDS = 9376;
const EXCLUDED_SPECIAL_RECORDS = 388;
const LINKED_SOURCE_NOTES = 116;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-hunan-rank-conversion-2025-v3318-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-hunan-rank-conversion-2025-v3318-runtime-manifest.json",
  };
...
function scoreDerivedRankBoundary(note, hunanCount) {
  const scopes = [];
  const fields = [
  ...
  for (const [province, field] of fields) {
    const count = field === "hunan2025ScoreDerivedRankRecords" ? hunanCount : Number(note[field] || 0);
...
function verifyImport(payload) {
  assert(payload.dataset === "official-hunan-rank-conversion-2025-v3318-import", `Unexpected dataset ${payload.dataset}`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "Hunan source note mismatch");
  assert(payload.rankConversions?.length === ADDED_RANKS, `Expected ${ADDED_RANKS} rank rows`);
  assert(payload.audit?.parsedRecords === ADDED_RANKS && payload.audit?.duplicateIds === 0, "Hunan import audit drifted");
  assert(payload.audit?.rowComparisons === ADDED_RANKS && payload.audit?.cellComparisons === 3417 && payload.audit?.sourceDifferences === 0, "Hunan evidence comparison drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "历史类").length === 550, "History row count drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "物理类").length === 589, "Physics row count drifted");
...
function mapAdmissionRecord(record, rankIndex, topBySubject) {
  ...
  const replacementCaution = "该位次由湖南2025成绩统计表按最低分换算，非原录取/投档表直接公布。";
...
function main() {
  ...
  const item = manifest.shards[PROVINCE];
  assert(item, "Hunan runtime shard is missing");
  ...
  assert(shard.records.length === 32686 && shard.records.length === item.records, "Hunan record count drifted before merge");
  assert(shard.rankConversions.length === 1137 && shard.rankConversions.length === item.rankConversions, "Hunan rank count drifted before merge");
  assert(!shard.rankConversions.some((row) => row.sourceId === SOURCE_ID), "Hunan v3.318 ranks already exist");
  ...
  assert(officialLinked === 8337 && thirdPartyLinked === 1039, "Official/third-party linked split drifted");
  assert(schoolOfficialScopeLinked === 635, "School-official-only linked count drifted");
  assert(topBucketLinked === 5, `Expected 5 top-bucket linked rows, got ${topBucketLinked}`);
  assert(linkedBySource.size === LINKED_SOURCE_NOTES, `Expected ${LINKED_SOURCE_NOTES} linked source IDs, got ${linkedBySource.size}`);
  assert(linkedByType.get("major-admission") === 1483, "Major linked count drifted");
  assert(linkedByType.get("institution-admission") === 176, "Institution linked count drifted");
  assert(linkedByType.get("vocational-admission") === 2330, "Vocational linked count drifted");
  assert(linkedByType.get("major-group-admission") === 5383, "Major-group linked count drifted");
  ...
  assert(shard.rankConversions.length === 2276, "Hunan rank count drifted after merge");
  ...
  for (const [sourceId, count] of linkedBySource) {
    ...
    note.hunan2025ScoreDerivedRankRecords = count;
    if (Number.isFinite(Number(note.rankUnavailableRecords))) {
      assert(Number(note.rankUnavailableRecords) >= count, `${sourceId} rank-unavailable source count is smaller than the Hunan linked count`);
    ...
    if (Number.isFinite(Number(note.scoreOnlyRecords))) {
      assert(Number(note.scoreOnlyRecords) >= count, `${sourceId} score-only source count is smaller than the Hunan linked count`);
  ...
  layer.currentFinding = "新增湖南2025年历史类550档、物理类589档一分一段记录；湖南省教育厅来源标注的两份完整表共1152行，独立完整镜像1139行、3417个分数/人数/累计人数单元逐项一致，阳光高考同时保存湖南考试招生来源原图。由此为9376条湖南2025历史/物理类普通整数最低分记录补充省级位次区间，其中8337条来自官方来源、1039条来自待复核第三方来源；388条特殊路径继续隔离。";
  layer.downgradeReason = `当前数据层 ${NEXT_VERSION} 严格区分院校原生最低位次与最低分换算的省级分数段位次；湖南档分含优惠加分，只用于同年同科类文化课档分换算，艺体综合分、科类不明、非整数分和特殊路径不混入普通类推荐。`;
  ...
  const tempDir = path.join(releaseDir, `.v3318-${process.pid}`);
  ...
  const runtimeManifest = {
    dataset: "official-hunan-rank-conversion-2025-v3318-runtime",
    generatedAt: payload.generatedAt,
    sourceId: SOURCE_ID,
    importFile: args.importFile,
    releaseDir: args.releaseDir,
    before: { modelVersion: BASE_VERSION, records: RECORDS, rankConversions: BASE_RANKS, provinceRankConversions: 1137 },
    after: {
