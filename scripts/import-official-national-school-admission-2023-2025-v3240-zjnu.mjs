...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2023-2025-v3240-zjnu-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2023-2025-v3240-zjnu";
const OFFICIAL_NAV_URL = "http://zs.zjnu.edu.cn/";
const INDEX_URL = "https://lqcx.zjnu.edu.cn/zsdata/lqxx/";
const SPA_URL = "https://lqcx.zjnu.edu.cn/zsdata/lqxx/#/lnfs";
const BASE_API_URL = "https://lqcx.zjnu.edu.cn/lqxx/s";
...
const SOURCE = {
  id: "official-zjnu-national-2023-2025-school-admission",
  quality: "official-school-zjnu-2023-2025-national-api-score-rank",
  schoolCode: "10345",
  schoolName: "浙江师范大学",
  city: "浙江金华",
  publisher: "浙江师范大学招生办公室",
  tags: ["浙江", "金华", "浙江师范大学", "师范"],
};
...
function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2023-2025-v3240-zjnu.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2023-2025-v3240-zjnu.mjs --use-cache --concurrency 4",
    "",
    "Imports 浙江师范大学招生办公室 2023-2025 历年分数 official API data.",
  ].join("\n");
}
...
function extractJsAssets(indexHtml) {
  ...
  const pattern = /(?:src|href)=["']?(\/public\/zsdata\/lqxx\/js\/[^"' >]+\.js)/g;
...
function classifyAdmission(row) {
  ...
  const artMajor = `${row.zymc || ""}`;
  if (
    /艺术|美术|音乐|舞蹈|播音|书法|表演|戏剧/.test(artContext)
    || /视觉传达设计|环境设计|产品设计|服装与服饰设计|艺术设计|数字媒体艺术|工艺美术|动画/.test(artMajor)
  ) {
  ...
  if (/提前批|提前文|专项|预科|内高班|西藏班|单列|南疆|定向|援疆|民族班|少数民族|优师/.test(text)) {
...
function queryRawRel(query, index) {
  ...
  return `zjnu-${query.nf}-${slugify(query.sf)}-${slugify(query.klmc)}-${slugify(query.xqmc)}-${slugify(query.zslb)}-${index}-${hash}.json`;
}
...
function buildQueries(typeMap) {
  ...
  for (const key of keys) {
    const [sf, nf, klmc, xqmc] = key.split("_");
    if (!sf || !nf || !klmc || xqmc === undefined) {
      skippedAllQueries.push({ key, issue: "bad_typeMap_key" });
      continue;
    }
    if (!MAINLAND_PROVINCES.has(sf)) {
      skippedAllQueries.push({ key, issue: "skip_non_mainland_gaokao_region" });
      continue;
    }
...
function buildRecord(row, rawRel, query, pageIndex, rowIndex, dataType) {
  ...
  const idPrefix = dataType === "institution-admission" ? "zjnu-summary" : "zjnu";
  const record = {
    ...
    sourcePageKey: `zjnu-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
function parseQuery(raw, rawRel, query, pageIndex) {
  ...
  if (payload.code !== 200 && payload.code !== "200") {
    return {
      records,
      summary: {
        pageKey: `zjnu-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
async function main() {
  ...
  await getTextRaw(rawRoot, "zjnu-official-admission-site.html", OFFICIAL_NAV_URL, args.useCache, "text/html,*/*;q=0.9");
  const indexHtml = await getTextRaw(rawRoot, "zjnu-lqxx-index.html", INDEX_URL, args.useCache, "text/html,*/*;q=0.9");
  const jsFiles = [];
  ...
  const themeRaw = await postJsonRaw(rawRoot, "zjnu-getTheme.json", THEME_URL, {}, args.useCache);
  const theme = bodyFromRaw(themeRaw).homeTheme || {};
  if (normalizeText(theme.xxmc) !== SOURCE.schoolName || !/zs\.zjnu\.edu\.cn/.test(String(theme.zsxxLink || ""))) {
    throw new Error("Official query system theme does not identify 浙江师范大学 and zs.zjnu.edu.cn; refusing to import without that evidence.");
  }

  const getTypeRaw = await postJsonRaw(rawRoot, "zjnu-getType-lnfs.json", GET_TYPE_URL, { type: "lnfs" }, args.useCache);
  const globalCfgRaw = await postJsonRaw(rawRoot, "zjnu-getGlobalCfg.json", GLOBAL_CFG_URL, {}, args.useCache);
  const displayCfgRaw = await postJsonRaw(rawRoot, "zjnu-getlqxsgz-lnfs.json", DISPLAY_CFG_URL, { type: "lnfs" }, args.useCache);
  ...
  const rawFiles = [
    `${RAW_DIR}/zjnu-official-admission-site.html`,
    `${RAW_DIR}/zjnu-lqxx-index.html`,
    ...jsFiles,
    `${RAW_DIR}/zjnu-getTheme.json`,
    `${RAW_DIR}/zjnu-getType-lnfs.json`,
    `${RAW_DIR}/zjnu-getGlobalCfg.json`,
    `${RAW_DIR}/zjnu-getlqxsgz-lnfs.json`,
    ...pageSummaries.map((summary) => summary.rawFile),
  ];

  const configSha256 = {};
  for (const rawFile of rawFiles.filter((file) => /zjnu-(official-admission-site|lqxx-index|getTheme|getType|getGlobalCfg|getlqxsgz|.*\.js)/.test(file))) {
  ...
  const sourceNotes = [
    {
      ...
      title: "浙江师范大学招生办公室历年分数 API（2023-2025）",
      url: SPA_URL,
      officialNavigationUrl: OFFICIAL_NAV_URL,
      quality: SOURCE.quality,
      usage:
        "学校官网单校分专业录取分数边界；可用于浙江师范大学候选边界复核、师范类和浙江省属重点高校方向分数段趋势及全国单校分数加厚，不替代任何省级教育考试院全量投档/录取表。",
      rawDir: RAW_DIR,
      rawFiles,
      parsedRecords: records.length,
      ...
      pageSummaries,
      configSha256,
      theme: bodyFromRaw(themeRaw).homeTheme || null,
      ...
      boundaryNotes: [
        "源表公开最低分位次的行保留 minRank；源表用 / 或 0 表示位次未公开的行统一标记 rankUnavailable=true。",
  ...
  const output = {
    dataset: "official-national-school-admission-2023-2025-v3240-zjnu",
