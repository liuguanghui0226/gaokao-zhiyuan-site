...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3291.json");
const expectedCovered = ["安徽", "北京", "福建", "广东", "河北", "河南", "湖北", "湖南", "江西", "吉林", "内蒙古", "陕西", "山东", "四川", "天津", "新疆", "西藏", "浙江"];
...
assert(coveredProvinces.length === 18 && missingProvinces.length === 13, "Expected 18 covered and 13 missing provinces after Hubei v3.291");
...
assert(!hubei.boundaries.some((row) => [529, 532, 326, 321, 210, 387, 375, 217, 120, 588, 598].includes(row.minScore)), "Hubei special, art, sports or skill-gaokao lines leaked into ordinary coverage");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3291",
