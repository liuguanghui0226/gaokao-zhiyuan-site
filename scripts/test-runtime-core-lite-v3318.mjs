...
const auditFile = path.join(projectRoot, "data/admissions/runtime-core-lite-v3318-manifest.json");
...
assert.equal(lite.modelVersion, "local-deterministic-v3.318-hunan-education-department-rank2025-aligned-868426records");
...
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 121889);
assert.equal(liteNotes.length, fullNotes.length);
assert.equal(liteNotes.length, 5122);
...
assert.equal(manifest.coreLite.sourceNotes, 5122);
...
assert.equal(audit.dataset, "runtime-core-lite-v3318");
