...
function readAdmissionData() {
  ...
  files.forEach((name, index) => {
    ...
    try {
      ...
      const payloadRecords = splitAdmissionPayloadRecords(payload);
      for (const record of payloadRecords.admissionRecords) {
        const recordWithSource = compactAdmissionRuntimeRecord(record);
        records.push(recordWithSource);
      }
      for (const record of payloadRecords.rankRecords) {
        rankRecords.push(compactAdmissionRuntimeRecord(record));
      }
    } catch (error) {
...
function refreshAdmissionsOnly() {
  ...
  const version = `local-deterministic-v3.187-national-school-official-njust2023-2026-${admissionScoreLayer.structuredRecords}records`;
