...
assert.equal(sourceNote.derivedRankRecords, 338);
assert.equal(sourceNote.rankUnavailableRecords, 7125);
...
assert.equal(sourceNote.tianjin2025ScoreDerivedRankRecords, 16);
assert.equal(sourceNote.shanghai2025ScoreDerivedRankRecords, 10);
assert.equal(sourceNote.shanghai2025RankScoreBasis, "gaokao-total-including-policy-bonus");
assert.equal(sourceNote.shanghai2025RankPolicyBonusIncluded, true);
assert.equal(sourceNote.anhui2025ScoreDerivedRankRecords, 25);
assert.equal(sourceNote.anhui2025RankScoreBasis, "gaokao-total-including-policy-bonus");
assert.equal(sourceNote.anhui2025RankPolicyBonusIncluded, true);
assert.equal(sourceNote.jiangxi2025ScoreDerivedRankRecords, 24);
assert.equal(sourceNote.jiangxi2025RankScoreBasis, "gaokao-total-including-policy-bonus");
assert.equal(sourceNote.jiangxi2025RankPolicyBonusIncluded, true);
assert.equal(sourceNote.guangdong2025ScoreDerivedRankRecords, 24);
assert.equal(sourceNote.guangdong2025RankUsage, "本科/专科层次加分分别匹配");
assert.match(sourceNote.rankAlignmentBoundary, /河北2025年28条、重庆2025年23条、辽宁2025年26条、湖南2025年22条、江苏2025年17条、广西2025年18条、山西2025年19条、湖北2025年22条、福建2025年24条、黑龙江2025年15条、海南2025年8条、天津2025年16条、上海2025年10条、安徽2025年25条、江西2025年24条/);
...
assert.match(sourceNote.rankAlignmentBoundary, /天津使用含政策加分且公开至300分的普通高考总成绩表/);
assert.match(sourceNote.rankAlignmentBoundary, /上海使用含政策加分且公开至402分本科线的高考总成绩表/);
assert.match(sourceNote.rankAlignmentBoundary, /安徽使用含加分且公开至200分的完整分档表/);
assert.match(sourceNote.rankAlignmentBoundary, /江西使用含政策加分的档案分表并公开至100分/);
assert.match(sourceNote.rankAlignmentBoundary, /广东2025年24条普通类整数最低分记录/);
...
assert.equal(rankUnavailable, 7125);
assert.equal(derived, 338);
...
assert.equal(jiangxiComputer.rankUnavailable, false);
assert.equal(jiangxiComputer.rankDerivedFromScore, true);
assert.equal(jiangxiComputer.rankEvidenceScope, "score-derived-provincial-segment");
assert.equal(jiangxiComputer.rankSourceId, "official-jiangxi-rank-2025-v3330");
assert.deepEqual([jiangxiComputer.minRankStart, jiangxiComputer.minRankEnd], [7159, 7442]);
