...
const auditFile = path.join(projectRoot, "data/admissions/runtime-core-lite-v3322-manifest.json");
...
assert.equal(lite.modelVersion, "local-deterministic-v3.322-hubei-official-rank2025-full-cohort-aligned-868426records");
...
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 126013);
assert.equal(liteNotes.length, fullNotes.length);
assert.equal(liteNotes.length, 5126);
...
assert.equal(manifest.coreLite.sourceNotes, 5126);
assert.equal(manifest.runtimeProfile.version, "v3.322");
assert.equal(audit.dataset, "runtime-core-lite-v3322");
