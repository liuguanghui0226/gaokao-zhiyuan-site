...
const BASE_VERSION = "local-deterministic-v3.308-national-school-official-wtu2021-2025-native-rank-857225records";
const NEXT_VERSION = "local-deterministic-v3.309-national-school-official-qlu2021-2025-native-rank-859382records";
const SOURCE_ID = "official-qlu-national-2021-2025-school-major-admission";
const EXPECTED_ADDED_RECORDS = 2157;
const EXPECTED_NATIVE_RANKS = 2074;
const EXPECTED_BASE_RECORDS = 857225;
const EXPECTED_NEXT_RECORDS = 859382;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-national-school-admission-2021-2025-v3309-qlu-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-national-school-admission-2021-2025-v3309-qlu-runtime-manifest.json",
  };
...
function verifyImport(payload) {
  assert(payload.dataset === "official-national-school-admission-2021-2025-v3309-qlu", `Unexpected import dataset ${payload.dataset}`);
  assert(payload.records?.length === EXPECTED_ADDED_RECORDS, `Expected ${EXPECTED_ADDED_RECORDS} records`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "QLU source note mismatch");
  assert(new Set(payload.records.map((record) => record.id)).size === EXPECTED_ADDED_RECORDS, "QLU import contains duplicate ids");
  assert(payload.records.every((record) => record.sourceId === SOURCE_ID && record.schoolName === "齐鲁工业大学" && record.dataType === "major-admission"), "QLU import contains out-of-scope rows");
  assert(payload.audit.skippedRows.length === 0 && payload.audit.sourceRows === EXPECTED_ADDED_RECORDS, "QLU source row reconciliation drifted");
  assert(payload.records.filter((record) => record.minRankEnd).length === EXPECTED_NATIVE_RANKS, "QLU native-rank count drifted");
  assert(payload.records.filter((record) => record.rankDerivedFromScore).length === 0, "QLU must not contain derived ranks");
  assert(payload.records.filter((record) => record.formalScoreScope === "school-official-only").length === 1849, "QLU ordinary count drifted");
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 308, "QLU special-path count drifted");
  assert(payload.records.filter((record) => record.minRankEnd).every((record) => record.rankEvidenceScope === "school-recorded-min-score-rank" && record.nativeAdmissionRankUnavailable === false), "QLU rank evidence semantics drifted");
}
...
function alreadyApplied(core, manifest, releaseDir) {
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(rows.every((record) => record.province === province), `QLU province drift in ${province}`);
    count += rows.length;
  }
  assert(count === EXPECTED_ADDED_RECORDS, `Already-applied QLU row count drifted: ${count}`);
...
function main() {
  ...
  assert(byProvince.size === 28 && !byProvince.has("西藏") && !byProvince.has("青海") && !byProvince.has("宁夏"), "QLU official non-empty province coverage drifted");
  const tempDir = path.join(releaseDir, `.v3309-${process.pid}`);
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(!shard.records.some((record) => record.sourceId === SOURCE_ID), `${province} already contains QLU rows`);
    ...
    shardStats[province] = { recordsAdded: additions.length, recordsAfter: shard.records.length, nativeRanksAdded: additions.filter((record) => record.minRankEnd && !record.rankDerivedFromScore).length };
  }
  ...
  layer.currentFinding = "新增齐鲁工业大学2021-2025年28省2157条官方分专业录取记录；2074条直接保留官网最低录取位次，83条位次未公开记录保持rankUnavailable。718份官方原始响应覆盖297个科类/类型分数查询，2157条源行零跳过、零重复；1849条普通school-official-only与308条艺体、中外合作、地方专项、民族班、校企合作、菏泽校区等special-path-only继续隔离。官网列出西藏、青海、宁夏入口但五年内无分数记录，明确保持来源空缺。";
  ...
  coverage.schools = sortedUnique([...(coverage.schools || []), "齐鲁工业大学"]);
  coverage.cities = sortedUnique([...(coverage.cities || []), "济南", "菏泽"]);
  coverage.schoolTags = sortedUnique([...(coverage.schoolTags || []), "公办", "理工", "山东省属"]);
  ...
  for (const [yearText, count] of Object.entries(payload.audit.yearCounts)) {
    ...
    if (!existingSchoolNames.has("齐鲁工业大学")) row.schools = Number(row.schools || 0) + 1;
  }
  ...
  const runtimeManifest = {
    dataset: "official-national-school-admission-2021-2025-v3309-qlu-runtime",
    ...
    after: {
      ...
      ordinaryRecords: 1849,
      specialPathRecords: 308,
      scoreDerivedRankRecords: 0,
      nativeAdmissionRankRecords: EXPECTED_NATIVE_RANKS,
      rankUnavailableRecords: 83,
  ...
  console.log(JSON.stringify({ status: "applied", modelVersion: NEXT_VERSION, recordCount: EXPECTED_NEXT_RECORDS, recordsAdded: EXPECTED_ADDED_RECORDS, scoreDerivedRankRecords: 0, nativeAdmissionRankRecords: EXPECTED_NATIVE_RANKS, provinceCount: byProvince.size, coreSha256: runtimeManifest.after.coreSha256, manifestSha256: runtimeManifest.after.manifestSha256 }, null, 2));
}
