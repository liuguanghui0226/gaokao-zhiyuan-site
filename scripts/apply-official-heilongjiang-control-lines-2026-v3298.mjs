...
const BASE_VERSION = "local-deterministic-v3.297-hainan-control-lines2026-and-rank-provenance-847033records";
const NEXT_VERSION = "local-deterministic-v3.298-heilongjiang-control-lines2026-and-rank-provenance-847051records";
...
const EXPECTED_RECORDS = 18;
const EXPECTED_RANK_ROWS = 1071;
const EXPECTED_NEW_RECORD_COUNT = 847051;
const EXPECTED_NEW_SHARD_RECORDS = 15413;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-heilongjiang-control-lines-2026-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-heilongjiang-control-lines-2026-v3298-runtime-manifest.json",
  };
...
function refreshReadiness(container, records, rankConversions) {
  const row = container?.rows?.find((item) => item.province === "黑龙江");
  assert(row, "Heilongjiang province-readiness row is missing");
...
function rankUrlFor(record) {
  ...
  throw new Error(`Unexpected Heilongjiang rank subject: ${record.subjectType}`);
}
...
function patchRankSourceNote(core, sourceEvidence, sourceRankRows) {
  ...
  assert(note, "Official Heilongjiang rank source note is missing");
  assert(note.parsedRecords === EXPECTED_RANK_ROWS, `Unexpected Heilongjiang parsed rank rows: ${note.parsedRecords}`);
  assert(note.url === RANK_PAGE_URL, `Unexpected Heilongjiang rank page URL: ${note.url}`);
  assert(note.quality === "official-heilongjiang-rank-conversion-xls", `Heilongjiang rank quality drifted: ${note.quality}`);
  assert(note.attachmentUrls?.includes(RANK_HISTORY_URL) && note.attachmentUrls?.includes(RANK_PHYSICS_URL), "Heilongjiang official XLS URLs drifted");
  assert(note.parsedSubjects?.find((row) => row.subjectType === "历史类")?.parsedRecords === 520, "Heilongjiang history rank inventory drifted");
  assert(note.parsedSubjects?.find((row) => row.subjectType === "物理类")?.parsedRecords === 551, "Heilongjiang physics rank inventory drifted");
  ...
  for (const [subject, checkpoints] of Object.entries(crossCheck)) {
    ...
    for (const key of ["vocational", "bachelor", "special"]) {
      ...
      assert(rankAt(sourceRankRows, subjectType, score)?.rankEnd === rankEnd, `Heilongjiang ${subjectType}/${score} rank cross-check drifted`);
    }
  }
  ...
  note.provenanceRevision = {
    verifiedAt: "2026-07-17",
    finding: "重新下载黑龙江省教育厅正式页面和历史、物理两份XLS，使用LibreOffice结构化转换后逐行复核1071条位次，与运行层分数、同分人数、累计人数和起止名次零差异。本轮只给每条位次补对应官方XLS URL；历史664分为官方表省略的零人数档，不补假行。",
    rankRowsLinked: EXPECTED_RANK_ROWS,
    rowsFullCrossChecked: sourceEvidence.rankEvidence.fullRowCrossCheck.reduce((sum, row) => sum + row.rowsCompared, 0),
    checkpointCount: 6,
    valueChanges: 0,
    directOfficialRedownloadStatus: "success",
    officialFiles: {
      page: { url: RANK_PAGE_URL, sha256: sourceEvidence.evidence.rankPage.sha256 },
      history: { url: RANK_HISTORY_URL, sha256: sourceEvidence.evidence.rankHistory.sha256, rows: 520 },
      physics: { url: RANK_PHYSICS_URL, sha256: sourceEvidence.evidence.rankPhysics.sha256, rows: 551 },
    },
  };
}
...
function verifyAlreadyApplied({ core, manifest, shard }) {
  ...
  assert(sourceRecords.length === EXPECTED_RECORDS, `Expected ${EXPECTED_RECORDS} Heilongjiang control records`);
  assert(sourceRankRows.length === EXPECTED_RANK_ROWS, `Expected ${EXPECTED_RANK_ROWS} Heilongjiang rank rows`);
  assert(sourceRankRows.every((record) => record.sourceUrl === rankUrlFor(record)), "Heilongjiang rank source URL repair drifted");
  assert(rankNote?.provenanceRevision?.rankRowsLinked === EXPECTED_RANK_ROWS, "Heilongjiang rank provenance drifted");
...
function main() {
  ...
  const shardFile = path.join(releaseDir, "heilongjiang.json.gz");
  ...
  assert(payload.dataset === "official-heilongjiang-control-lines-2026-import", `Unexpected import dataset: ${payload.dataset}`);
  ...
  assert(payload.records.every((record) => record.province === "黑龙江" && record.year === 2026 && record.dataType === "control-line" && record.sourceId === SOURCE_ID), "Import contains an out-of-scope record");
  assert(new Set(payload.records.map((record) => record.id)).size === EXPECTED_RECORDS, "Import contains duplicate ids");
  assert(payload.records.filter((record) => record.formalScoreScope === "control-line-only").length === 4, "Expected four ordinary Heilongjiang records");
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 14, "Expected 14 Heilongjiang special-path records");
  assert(payload.records.filter((record) => Number.isFinite(record.professionalMinScore)).length === 9, "Expected nine Heilongjiang professional thresholds");
  assert(payload.records.filter((record) => record.professionalQualification).length === 3, "Expected three Heilongjiang professional qualification rows");
  assert(payload.records.every((record) => record.scoreMaximum === 750), "Heilongjiang control records must retain the 750-point score scale");
  ...
  assert(manifest.recordCount === 847033, `Unexpected base record count ${manifest.recordCount}`);
  assert(manifest.rankConversionCount === 116656, `Unexpected rank-conversion count ${manifest.rankConversionCount}`);
  assert(manifest.shards?.["黑龙江"]?.records === 15395, `Unexpected Heilongjiang base count ${manifest.shards?.["黑龙江"]?.records}`);
  assert(manifest.shards?.["黑龙江"]?.rankConversions === EXPECTED_RANK_ROWS, "Unexpected Heilongjiang rank-conversion manifest count");
  assert(shard.rankConversions?.length === EXPECTED_RANK_ROWS, `Unexpected Heilongjiang rank-conversion count ${shard.rankConversions?.length}`);
  ...
  const sourceRankRows = rankRows(shard);
  assert(sourceRankRows.length === EXPECTED_RANK_ROWS, `Expected ${EXPECTED_RANK_ROWS} official Heilongjiang rank rows`);
  assert(sourceRankRows.every((record) => !record.sourceUrl), "Expected all Heilongjiang rank rows to need URL repair");
  ...
  const before = {
    ...
    heilongjiangRecords: shard.records.length,
    ...
    heilongjiangSha256: sha256(zlib.gunzipSync(fs.readFileSync(shardFile))),
  };
  ...
  assert(JSON.stringify(beforeRankValues) === JSON.stringify(sourceRankRows.map((record) => [record.id, record.score, record.scoreRange, record.rankStart, record.rankEnd, record.sameRankScore, record.sourceQuality])), "Heilongjiang rank values changed during provenance repair");
  ...
  layer.currentFinding = "黑龙江2026普通历史本科/专科385/150分、物理本科/专科340/150分进入普通资格路由；特殊类型、体育和艺术14条保持特殊路径。9条本科艺体记录分列文化与专业门槛，3条专科艺体记录不补造专业分。1071条官方XLS位次逐行零差异并补齐科类附件URL。";
  ...
  const provinceBreakdown = coverage.provinceBreakdown.find((row) => row.province === "黑龙江");
  assert(provinceBreakdown, "Heilongjiang province coverage row is missing");
  ...
  manifest.shards["黑龙江"].records = shard.records.length;
  manifest.shards["黑龙江"].bytes = shardBytes.byteLength;
  manifest.shards["黑龙江"].sha256 = sha256(shardBytes);
  ...
  const runtimeManifest = {
    dataset: "official-heilongjiang-control-lines-2026-v3298-runtime",
    ...
    after: {
      ...
      heilongjiangBytes: shardBytes.byteLength,
      heilongjiangSha256: sha256(shardBytes),
      manifestBytes: manifestBytes.byteLength,
      manifestSha256: sha256(manifestBytes),
    },
  };
  assert(runtimeManifest.after.heilongjiangRecords === EXPECTED_NEW_SHARD_RECORDS, `Unexpected Heilongjiang merged count ${runtimeManifest.after.heilongjiangRecords}`);
