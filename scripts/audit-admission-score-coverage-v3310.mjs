...
const DEFAULT_OUT = path.join(PROJECT_ROOT, "data/admissions/admission-score-coverage-v3310.json");
const SCORE_TYPES = new Set(["major-admission", "major-group-admission", "institution-admission", "vocational-admission"]);

function parseArgs(argv) {
  const args = { out: DEFAULT_OUT };
  for (let index = 2; index < argv.length; index += 1) {
    if (argv[index] === "--out") args.out = path.resolve(PROJECT_ROOT, argv[++index]);
...
function summarizeProvince(province, records) {
  ...
  return {
    ...
    years: [...new Set(scoreRecords.map((record) => Number(record.year)).filter(Number.isFinite))].sort((left, right) => right - left),
  };
}
...
function main() {
  ...
  provinceRows.sort((left, right) => left.records - right.records || left.province.localeCompare(right.province, "zh-CN"));
  const payload = {
    dataset: "admission-score-coverage-v3310",
