...
const YEAR = 2022;
const PROVINCE = "吉林";
const SOURCE_ID = "official-jilin-rank-2022";
const DEFAULT_OUT = "data/admissions/official-jilin-rank-conversion-2022-import.json";
const RAW_DIR = "data/admissions/raw/official-jilin-rank-2022";
const SOURCE_QUALITY = "official-chsi-jilin-rank-conversion-xls";
const PAGE_URL = "https://gaokao.chsi.com.cn/gkxx/zc/ss/202206/20220624/2197637116.html";
...
const SUBJECTS = [
  {
    ...
    url: "https://t2.chei.com.cn/news/getfile/2197637117-2197637116-ddcae55295bce67a74a3011501826d89.xls",
    mappingNote: "2022年吉林仍为旧文理口径，站内将理工类映射到物理类以便与新高考普通类位次层衔接。",
  },
  {
    subjectType: "历史类",
    ...
    localName: "arts.xls",
    url: "https://t1.chei.com.cn/news/getfile/2197637118-2197637116-1e817fe820368b38d9aeabb135c9a1d3.xls",
    mappingNote: "2022年吉林仍为旧文理口径，站内将文史类映射到历史类以便与新高考普通类位次层衔接。",
  },
];
...
function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-jilin-rank-conversion-2022.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-jilin-rank-conversion-2022.mjs --use-cache",
    "",
    "Options:",
    ...
    "",
    "Imports Jilin 2022 old science/arts score-segment XLS files as ordinary rank-conversion records.",
  ].join("\n");
}
...
async function download(url, accept = "*/*") {
  const response = await fetch(url, {
    ...
    headers: {
      "user-agent": "Mozilla/5.0 gaokao-jilin-rank-2022-importer/1.0",
      accept,
    },
  });
...
async function ensureRawFiles(rawDir, useCache) {
  ...
  const pageFile = path.join(rawDir, "chsi-jilin-rank-2022.html");
...
function validateRows(allRows, subject) {
  ...
  if (errors.length) throw new Error(`Invalid Jilin 2022 rank rows for ${subject.subjectType}: ${JSON.stringify(errors.slice(0, 5))}`);
...
function parseCsv(file, subject) {
  ...
  const match = /2022年吉林省普通高校招生考试(理工|文史)1分段表/.exec(title);
  if (!match) throw new Error(`Could not parse Jilin 2022 rank title in ${file}: ${title}`);
...
async function main() {
  ...
  const sourceNotes = [
    {
      id: SOURCE_ID,
      title: "吉林省2022年高考成绩一分段表（理工/文史，含照顾分）",
      publisher: "吉林省教育考试院",
      ...
      usage: "抽取阳光高考页面中来源为吉林省教育考试院的吉林2022理工/文史1分段XLS表，生成同年同原始科类分数到位次换算记录。",
  ...
  const payload = {
    dataset: "official-jilin-rank-conversion-2022-import",
    ...
    notes: [
      "本文件由 scripts/import-official-jilin-rank-conversion-2022.mjs 自动生成。",
      "来源为阳光高考页面，页面标注来源：吉林省教育考试院。",
      "仅导入理工类/文史类普通1分段表；同页艺术类、体育类表未混入普通类。",
      "2022旧文理口径映射为站内物理类/历史类时必须保留原始科类字段。",
