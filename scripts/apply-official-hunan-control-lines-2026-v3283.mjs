...
const BASE_VERSION = "local-deterministic-v3.282-zhejiang-control-lines2026-and-segment-routing-846519records";
const NEXT_VERSION = "local-deterministic-v3.283-hunan-control-lines2026-and-boundary-routing-846556records";
const SOURCE_ID = "official-hunan-control-lines-2026";
const EXPECTED_RECORDS = 37;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-hunan-control-lines-2026-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-hunan-control-lines-2026-v3283-runtime-manifest.json",
  };
...
function refreshHunanReadiness(container, records, rankConversions) {
  const row = container?.rows?.find((item) => item.province === "湖南");
  assert(row, "Hunan province-readiness row is missing");
...
function verifyAlreadyApplied({ core, manifest, shard }) {
  ...
  assert(sourceRecords.length === EXPECTED_RECORDS, `Expected ${EXPECTED_RECORDS} prior Hunan records, got ${sourceRecords.length}`);
  assert(manifest.recordCount === 846556, "Manifest record count drift after prior apply");
...
function main() {
  ...
  const hunanFile = path.join(releaseDir, "hunan.json.gz");
  ...
  const shard = readGzipJson(hunanFile);

  assert(payload.dataset === "official-hunan-control-lines-2026-import", `Unexpected import dataset: ${payload.dataset}`);
  ...
  assert(payload.records.every((record) => record.province === "湖南" && record.year === 2026 && record.dataType === "control-line" && record.sourceId === SOURCE_ID), "Import contains an out-of-scope record");
  ...
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 33, "Expected 33 special-path records");
  ...
  assert(manifest.recordCount === 846519, `Refusing unexpected base record count ${manifest.recordCount}`);
  assert(manifest.shards?.["湖南"]?.records === 31877, `Refusing unexpected Hunan base count ${manifest.shards?.["湖南"]?.records}`);
  assert(shard.rankConversions?.length === 1137, `Refusing unexpected Hunan rank-conversion count ${shard.rankConversions?.length}`);
  ...
  assert(newCount === 846556, `Unexpected merged record count ${newCount}`);
  ...
  layer.currentFinding = "湖南2026官方普通类历史本科446分、专科200分，物理本科400分、专科200分，已与同年1137条位次换算联动；特殊类型、体育、艺术和职高对口33条继续隔离。";
  ...
  const provinceBreakdown = coverage.provinceBreakdown.find((row) => row.province === "湖南");
  assert(provinceBreakdown, "Hunan province coverage row is missing");
  ...
  refreshHunanReadiness(layer.provinceReadiness, shard.records, shard.rankConversions);
  refreshHunanReadiness(coverage.provinceReadiness, shard.records, shard.rankConversions);

  const shardBytes = encodeJson(shard);
  atomicWriteGzip(hunanFile, shardBytes);
  ...
  manifest.shards["湖南"].records = shard.records.length;
  manifest.shards["湖南"].bytes = shardBytes.byteLength;
  manifest.shards["湖南"].sha256 = sha256(shardBytes);
  ...
  const runtimeManifest = {
    dataset: "official-hunan-control-lines-2026-v3283-runtime",
    ...
    after: {
      ...
      hunanBytes: shardBytes.byteLength,
      hunanSha256: sha256(shardBytes),
