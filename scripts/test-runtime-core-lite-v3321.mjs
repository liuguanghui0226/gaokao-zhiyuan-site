...
const auditFile = path.join(projectRoot, "data/admissions/runtime-core-lite-v3321-manifest.json");
...
assert.equal(lite.modelVersion, "local-deterministic-v3.321-shanxi-official-rank2025-bachelor-floor-aligned-868426records");
...
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 124700);
assert.equal(liteNotes.length, fullNotes.length);
assert.equal(liteNotes.length, 5125);
...
const rankSource = liteNotes.find((note) => note.id === "official-shanxi-rank-2025-v3321");
assert.equal(rankSource.province, "山西");
assert.equal(rankSource.year, 2025);
assert.equal(rankSource.quality, "official-source-attributed-shanxi-rank-images-html-government-cross-verified");
assert.equal(rankSource.url, "https://gaokao.chsi.com.cn/gkxx/zc/ss/202506/20250626/2293390947.html");
...
assert.equal(manifest.coreLite.sourceNotes, 5125);
assert.equal(manifest.runtimeProfile.version, "v3.321");
assert.equal(audit.dataset, "runtime-core-lite-v3321");
