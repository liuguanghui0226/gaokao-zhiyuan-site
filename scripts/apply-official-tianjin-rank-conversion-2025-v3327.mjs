...
const BASE_VERSION = "local-deterministic-v3.326-xinjiang-rank2025-score-basis-conflict-blocked-868426records";
const NEXT_VERSION = "local-deterministic-v3.327-tianjin-official-rank2025-policy-bonus-inclusive-full-table-aligned-868426records";
...
const BASE_RANKS = 128591;
const ADDED_RANKS = 381;
const NEXT_RANKS = 128972;
const RECORDS = 868426;
const LINKED_RECORDS = 3195;
const EXCLUDED_SPECIAL_RECORDS = 20;
const LINKED_SOURCE_NOTES = 69;
const SCORE_FLOOR = 300;
const SCORE_MAXIMUM = 750;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-tianjin-rank-conversion-2025-v3327-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-tianjin-rank-conversion-2025-v3327-runtime-manifest.json",
  };
...
function scoreDerivedRankBoundary(note, tianjinCount) {
  const fields = [
  ...
  for (const [province, field] of fields) {
    const count = field === "tianjin2025ScoreDerivedRankRecords" ? tianjinCount : Number(note[field] || 0);
    if (count > 0) scopes.push(`${province}2025年${count}条`);
  }
  return `${scopes.join("、")}普通类整数最低分记录按同年一分一段表换算省级位次区间；广西严格按目标院校区内/区外选用对应加分表；山西仅覆盖本科线及以上，湖北使用含政策性加分的普通类全分段表，福建覆盖官方已公布至215分的分数档，黑龙江使用不含照顾政策分且公开至130分的文化课表，海南使用含照顾加分且公开至246分的全体考生综合投档分表，天津使用含政策加分且公开至300分的普通高考总成绩表；其他年份和特殊路径保持原证据状态。`;
}
...
function verifyImport(payload) {
  assert(payload.dataset === "official-tianjin-rank-conversion-2025-v3327-import", `Unexpected dataset ${payload.dataset}`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "Tianjin source note mismatch");
  assert(payload.rankConversions?.length === ADDED_RANKS, `Expected ${ADDED_RANKS} rank rows`);
  assert(payload.audit?.parsedRecords === ADDED_RANKS && payload.audit?.duplicateIds === 0, "Tianjin import audit drifted");
  assert(payload.audit?.pdfRows === 381 && payload.audit?.htmlRows === 381, "Tianjin row audit drifted");
  assert(payload.audit?.comparedCells === 1143 && payload.audit?.valueDifferences === 0, "Tianjin comparison audit drifted");
  assert(payload.audit?.allScoresContinuous === true && payload.audit?.allCumulativeCountsClose === true, "Tianjin arithmetic audit drifted");
  assert(payload.audit?.policyBonusTitleVerified === true && payload.audit?.policyBonusFilingRuleVerified === true, "Tianjin policy-bonus evidence drifted");
  assert(payload.audit?.scoreBasis === SCORE_BASIS && payload.audit?.rankPolicyBonusIncluded === true, "Tianjin score basis drifted");
  assert(payload.audit?.topMergedCandidates === 656 && payload.audit?.publishedFloorRankEnd === 71938, "Tianjin boundary audit drifted");
...
function mapAdmissionRecord(record, rankIndex, topBySubject) {
  ...
  assert(rank, `No Tianjin rank mapping for ${record.id} at score ${record.minScore}`);
  const rankRangeText = rank.rankStart === rank.rankEnd ? `${rank.rankEnd}` : `${rank.rankStart}-${rank.rankEnd}`;
  const replacementCaution = "该位次由天津2025官方普通高考总成绩分数档按最低分换算，口径含政策加分，非原录取/投档表直接公布。";
  ...
  if (rank.rankStart === 1) cautions.push(`680分及以上合并档仅可确定为${rankRangeText}名，不生成档内伪精确位次。`);
  return {
    ...record,
    scoreOnly: false,
...
function main() {
  ...
  if (core.modelVersion === NEXT_VERSION) {
    ...
    assert(shard.rankConversions.filter((row) => row.sourceId === SOURCE_ID).length === ADDED_RANKS, "Already-applied Tianjin rank rows drifted");
    assert(shard.records.filter((row) => row.rankSourceId === SOURCE_ID).length === LINKED_RECORDS, "Already-applied Tianjin admission links drifted");
  ...
  const item = manifest.shards[PROVINCE];
  assert(item, "Tianjin runtime shard is missing");
  ...
  assert(shard.records.length === 10037 && shard.records.length === item.records, "Tianjin record count drifted before merge");
  assert(shard.rankConversions.length === 381 && shard.rankConversions.length === item.rankConversions, "Tianjin rank count drifted before merge");
  assert(!shard.rankConversions.some((row) => row.sourceId === SOURCE_ID), "Tianjin v3.327 ranks already exist");
  ...
  assert(officialLinked === 2910 && thirdPartyLinked === 285, "Official/third-party linked split drifted");
  assert(schoolOfficialScopeLinked === 380, "School-official-only linked count drifted");
  assert(topBucketLinked === 33, `Expected 33 top-bucket linked rows, got ${topBucketLinked}`);
  assert(linkedBySource.size === LINKED_SOURCE_NOTES, `Expected ${LINKED_SOURCE_NOTES} linked source IDs, got ${linkedBySource.size}`);
  assert(linkedByType.get("major-group-admission") === 2535, "Major-group linked count drifted");
  assert(linkedByType.get("major-admission") === 605, "Major linked count drifted");
  assert(linkedByType.get("institution-admission") === 55, "Institution linked count drifted");
  ...
  assert(shard.rankConversions.length === 762, "Tianjin rank count drifted after merge");
  const firstAt600 = shard.rankConversions.find((row) => row.year === YEAR && row.subjectType === SUBJECT_TYPE && row.score === 600);
  assert(firstAt600?.sourceId === SOURCE_ID, "Runtime lookup does not prioritize the official Tianjin 2025 source");
  ...
  for (const [sourceId, count] of linkedBySource) {
    ...
    note.tianjin2025ScoreDerivedRankRecords = count;
    note.tianjin2025RankScoreBasis = SCORE_BASIS;
    note.tianjin2025RankPolicyBonusIncluded = true;
    if (Number.isFinite(Number(note.rankUnavailableRecords))) {
      assert(Number(note.rankUnavailableRecords) >= count, `${sourceId} rank-unavailable source count is smaller than the Tianjin linked count`);
    ...
    if (Number.isFinite(Number(note.scoreOnlyRecords))) {
      assert(Number(note.scoreOnlyRecords) >= count, `${sourceId} score-only source count is smaller than the Tianjin linked count`);
  ...
  layer.currentFinding = "海南2025年555条官方综合投档分位次已为4241条海南2025综合普通类整数最低分完成同口径换算。新疆2025因政策加分口径未闭合，4234条无原生位次记录全部禁止自动套表。新增天津2025年381条官方总成绩分数档，PDF与完整HTML共1143个数值格零差异，为3195条天津2025综合普通类整数最低分补充含政策加分口径的省级位次区间，其中2910条来自官方来源；20条特殊路径继续隔离。";
  layer.downgradeReason = `当前数据层 ${NEXT_VERSION} 严格区分院校原生最低位次、同口径最低分换算位次和口径未闭合分数；海南2025表使用全体考生综合投档成绩，口径含照顾加分并公开至246分，800分及以上仅保留1-105名合并档。历史/物理类标签、艺术体育综合分、科类不明、非整数分、低于公开分数档和特殊路径不混入海南综合普通类自动推荐。新疆2025镜像表与官方含政策加分投档排序分出现9个0人分数冲突，4234条录取记录继续保持缺位次。天津2025表口径含政策加分并公开至300分，680分及以上仅保留1-656名合并档；299分及以下、物理/历史标签、非整数分和特殊路径不自动套表。`;
  layer.sourceNotes.push(sourceNote);
  assert(layer.sourceNotes.length === 5131, `Expected 5131 source notes, got ${layer.sourceNotes.length}`);
  ...
  manifest.runtimeProfile = { ...(manifest.runtimeProfile || {}), version: "v3.327", initialCore: "knowledge-core-lite.json.gz", fullEvidenceCore: "knowledge-core.json.gz" };
  ...
  const tempDir = path.join(releaseDir, `.v3327-${process.pid}`);
  ...
  const runtimeManifest = {
    dataset: "official-tianjin-rank-conversion-2025-v3327-runtime",
    generatedAt: payload.generatedAt,
    sourceId: SOURCE_ID,
    importFile: args.importFile,
    releaseDir: args.releaseDir,
    before: { modelVersion: BASE_VERSION, records: RECORDS, rankConversions: BASE_RANKS, provinceRankConversions: 381 },
    after: {
