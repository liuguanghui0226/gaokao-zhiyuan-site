...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3292.json");
const expectedCovered = ["安徽", "北京", "福建", "广东", "河北", "河南", "湖北", "湖南", "江西", "吉林", "内蒙古", "陕西", "山东", "上海", "四川", "天津", "新疆", "西藏", "浙江"];
...
assert(coveredProvinces.length === 19 && missingProvinces.length === 12, "Expected 19 covered and 12 missing provinces after Shanghai v3.292");
...
assert(shanghai.boundaries.some((row) => row.subjectType === "综合" && row.routeKind === "ordinary-bachelor" && row.minScore === 403 && row.scoreBasis === "gaokao-total"), "Shanghai bachelor boundary drifted");
assert(!shanghai.boundaries.some((row) => row.routeKind === "ordinary-vocational"), "Shanghai must not invent a 2026 vocational line");
assert(shanghai.ordinaryVocationalStatus === "pending-official-release", "Shanghai vocational status must remain pending");
assert(shanghai.ordinaryVocationalExpectedPublicationAt === "2026-07-29", "Shanghai vocational publication date drifted");
assert(!shanghai.boundaries.some((row) => [504, 302, 282, 220].includes(row.minScore)), "Shanghai special, art or sports lines leaked into ordinary coverage");

const hubei = coverage.find((row) => row.province === "湖北");
assert(hubei?.ordinaryRecords === 4, "Expected four Hubei ordinary boundaries");
for (const expected of [
  ["物理类", "ordinary-bachelor", 435],
  ["物理类", "ordinary-vocational", 200],
  ["历史类", "ordinary-bachelor", 443],
  ["历史类", "ordinary-vocational", 200],
]) {
  assert(hubei.boundaries.some((row) => row.subjectType === expected[0] && row.routeKind === expected[1] && row.minScore === expected[2] && row.scoreBasis === "gaokao-total"), `Hubei boundary drifted: ${expected.join("/")}`);
}
assert(!hubei.boundaries.some((row) => row.minScore === 150), "Hubei limited-school line leaked into general ordinary coverage");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3292",
