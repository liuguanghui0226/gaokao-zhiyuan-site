...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3299.json");
const expectedCovered = ["安徽", "北京", "重庆", "福建", "甘肃", "广东", "广西", "贵州", "海南", "河北", "河南", "黑龙江", "湖北", "湖南", "江苏", "江西", "吉林", "内蒙古", "陕西", "山东", "上海", "四川", "天津", "新疆", "西藏", "浙江"];
...
assert(manifest.modelVersion === "local-deterministic-v3.299-jiangsu-first-stage-control-lines2026-pending-vocational-and-rank-provenance-847079records", "Unexpected v3.299 model version");
assert(manifest.recordCount === 847079, "Unexpected v3.299 record count");
assert(core.admissionScoreLayer.coverage.dataTypes["control-line"] === 1433, "Unexpected v3.299 control-line count");
...
assert(coveredProvinces.length === 26 && missingProvinces.length === 5, "Expected 26 covered and 5 missing provinces after Jiangsu v3.299");
assert(JSON.stringify([...missingProvinces].sort()) === JSON.stringify(["辽宁", "宁夏", "青海", "山西", "云南"].sort()), `Unexpected missing provinces: ${missingProvinces.join("、")}`);
...
assert(hainan.boundaries.some((row) => row.subjectType === "综合" && row.routeKind === "ordinary-bachelor" && row.minScore === 479 && row.scoreBasis === "gaokao-total"), "Hainan ordinary undergraduate boundary drifted");
...
const heilongjiang = coverage.find((row) => row.province === "黑龙江");
assert(heilongjiang?.ordinaryRecords === 4, "Expected four Heilongjiang ordinary boundaries");
for (const expected of [
  ["历史类", "ordinary-bachelor", 385],
  ["历史类", "ordinary-vocational", 150],
  ["物理类", "ordinary-bachelor", 340],
  ["物理类", "ordinary-vocational", 150],
]) {
  assert(heilongjiang.boundaries.some((row) => row.subjectType === expected[0] && row.routeKind === expected[1] && row.minScore === expected[2] && row.scoreBasis === "gaokao-total"), `Heilongjiang boundary drifted: ${expected.join("/")}`);
}

const jiangsu = coverage.find((row) => row.province === "江苏");
assert(jiangsu?.ordinaryRecords === 2, "Expected two Jiangsu ordinary undergraduate boundaries");
for (const expected of [
  ["历史类", "ordinary-bachelor", 484],
  ["物理类", "ordinary-bachelor", 456],
]) {
  assert(jiangsu.boundaries.some((row) => row.subjectType === expected[0] && row.routeKind === expected[1] && row.minScore === expected[2] && row.scoreBasis === "gaokao-total"), `Jiangsu boundary drifted: ${expected.join("/")}`);
}
assert(!jiangsu.boundaries.some((row) => row.routeKind === "ordinary-vocational"), "Jiangsu must not invent a 2026 ordinary vocational line");
assert(!jiangsu.boundaries.some((row) => [532, 513, 413, 390, 363, 330, 279, 242, 228, 210, 190, 185, 180, 110].includes(row.minScore)), "Jiangsu special, art or sports lines leaked into ordinary coverage");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3299",
