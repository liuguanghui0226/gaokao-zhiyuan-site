...
const auditFile = path.join(projectRoot, "data/admissions/runtime-core-lite-v3320-manifest.json");
...
assert.equal(lite.modelVersion, "local-deterministic-v3.320-guangxi-dual-bonus-scope-rank2025-aligned-868426records");
...
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 124183);
assert.equal(liteNotes.length, fullNotes.length);
assert.equal(liteNotes.length, 5124);
...
const rankSource = liteNotes.find((note) => note.id === "official-guangxi-rank-2025-v3320");
assert.equal(rankSource.province, "广西");
...
assert.equal(manifest.coreLite.sourceNotes, 5124);
assert.equal(manifest.runtimeProfile.version, "v3.320");
assert.equal(audit.dataset, "runtime-core-lite-v3320");
