...
const BASE_VERSION = "local-deterministic-v3.285-anhui-control-lines2026-and-rank-provenance-846657records";
const NEXT_VERSION = "local-deterministic-v3.286-beijing-control-lines2026-and-score-basis-846666records";
const SOURCE_ID = "official-beijing-control-lines-2026";
const RANK_SOURCE_ID = "official-beijing-rank-2026";
...
const RANK_PAGE_BYTES = 12943;
const RANK_PAGE_SHA256 = "b140391d70126bf52e82b5d1818443edafd144017555293f49be174e9fc2c009";
const RANK_PDF_BYTES = 134541;
const RANK_PDF_SHA256 = "39f1e77097c56cbd7e1cd2971793e6231ba2ca9230811ba502a830153c4556a8";
const EXPECTED_RECORDS = 9;
const EXPECTED_RANK_ROWS = 341;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-beijing-control-lines-2026-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-beijing-control-lines-2026-v3286-runtime-manifest.json",
  };
...
function refreshReadiness(container, records, rankConversions) {
  const row = container?.rows?.find((item) => item.province === "北京");
  assert(row, "Beijing province-readiness row is missing");
...
function patchRankSourceNote(core) {
  ...
  assert(note, "Official Beijing rank source note is missing");
  assert(note.parsedRecords === EXPECTED_RANK_ROWS, `Unexpected Beijing parsed rank rows: ${note.parsedRecords}`);
  assert(note.pdfPages === 10, `Unexpected Beijing rank PDF pages: ${note.pdfPages}`);
  assert(note.attachmentUrls?.includes(RANK_PDF_URL), "Beijing rank PDF URL drifted");
  ...
  note.provenanceRevision = {
    ...
    finding: "重新下载北京教育考试院正式来源页和10页官方PDF；既有341条综合改革位次记录保持不变，运行分片逐条补齐来源页URL。",
...
function verifyAlreadyApplied({ core, manifest, shard }) {
  ...
  assert(manifest.recordCount === 846666, "Manifest record count drift");
  assert(sourceRecords.length === EXPECTED_RECORDS, `Expected ${EXPECTED_RECORDS} Beijing control records`);
  assert(sourceRankRows.length === EXPECTED_RANK_ROWS, `Expected ${EXPECTED_RANK_ROWS} Beijing rank rows`);
  assert(sourceRankRows.every((record) => record.sourceUrl === RANK_SOURCE_URL), "Beijing rank source URL repair drifted");
  assert(rankNote?.pageHtmlSha256 === RANK_PAGE_SHA256, "Beijing rank page provenance drifted");
  assert(rankNote?.pdfSha256 === RANK_PDF_SHA256, "Beijing rank PDF provenance drifted");
...
function main() {
  ...
  const beijingFile = path.join(releaseDir, "beijing.json.gz");
  ...
  const shard = readGzipJson(beijingFile);

  assert(payload.dataset === "official-beijing-control-lines-2026-import", `Unexpected import dataset: ${payload.dataset}`);
  ...
  assert(payload.records.every((record) => record.province === "北京" && record.year === 2026 && record.dataType === "control-line" && record.sourceId === SOURCE_ID), "Import contains an out-of-scope record");
  assert(new Set(payload.records.map((record) => record.id)).size === EXPECTED_RECORDS, "Import contains duplicate ids");
  assert(payload.records.filter((record) => record.formalScoreScope === "control-line-only").length === 2, "Expected two ordinary records");
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 7, "Expected seven special-path records");
  ...
  assert(manifest.recordCount === 846657, `Unexpected base record count ${manifest.recordCount}`);
  assert(manifest.rankConversionCount === 116656, `Unexpected rank-conversion count ${manifest.rankConversionCount}`);
  assert(manifest.shards?.["北京"]?.records === 6481, `Unexpected Beijing base count ${manifest.shards?.["北京"]?.records}`);
  assert(manifest.shards?.["北京"]?.rankConversions === 688, "Unexpected Beijing rank-conversion manifest count");
  assert(shard.rankConversions?.length === 688, `Unexpected Beijing rank-conversion count ${shard.rankConversions?.length}`);
  ...
  const sourceRankRows = rankRows(shard);
  assert(sourceRankRows.length === EXPECTED_RANK_ROWS, `Expected ${EXPECTED_RANK_ROWS} official Beijing rank rows`);
  assert(sourceRankRows.every((record) => !record.sourceUrl), "Expected all Beijing rank rows to need URL repair");
  ...
  const before = {
    ...
    beijingRecords: shard.records.length,
    ...
    beijingSha256: sha256(zlib.gunzipSync(fs.readFileSync(beijingFile))),
  };
  ...
  assert(JSON.stringify(beforeRankValues) === JSON.stringify(sourceRankRows.map((record) => [record.id, record.score, record.rankStart, record.rankEnd, record.sameRankScore])), "Beijing rank values changed during provenance repair");
  ...
  assert(newCount === 846666, `Unexpected merged record count ${newCount}`);
  ...
  layer.currentFinding = "北京2026普通本科控制线429分按高考总分判断，普通专科控制线120分按语数外三科总分判断；341条同年综合改革位次记录已补齐官方来源。其余7条特殊类型、艺体和高职单考单招控制线不进入普通推荐。";
  ...
  const provinceBreakdown = coverage.provinceBreakdown.find((row) => row.province === "北京");
  assert(provinceBreakdown, "Beijing province coverage row is missing");
  ...
  atomicWriteGzip(beijingFile, shardBytes);
  ...
  manifest.shards["北京"].records = shard.records.length;
  manifest.shards["北京"].bytes = shardBytes.byteLength;
  manifest.shards["北京"].sha256 = sha256(shardBytes);
  ...
  const runtimeManifest = {
    dataset: "official-beijing-control-lines-2026-v3286-runtime",
    ...
    after: {
      ...
      scoreBasisCounts: payload.diagnostics.scoreBasisCounts,
      coreBytes: coreBytes.byteLength,
      coreSha256: sha256(coreBytes),
      beijingBytes: shardBytes.byteLength,
      beijingSha256: sha256(shardBytes),
