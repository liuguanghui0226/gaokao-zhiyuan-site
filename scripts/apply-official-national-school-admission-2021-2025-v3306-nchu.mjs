...
const BASE_VERSION = "local-deterministic-v3.305-pending-vocational-schedule-audit-and-ui-847238records";
const NEXT_VERSION = "local-deterministic-v3.306-national-school-official-nchu2021-2025-derived-rank-852098records";
const SOURCE_ID = "official-nchu-national-2021-2025-school-major-admission";
const EXPECTED_ADDED_RECORDS = 4860;
const EXPECTED_DERIVED_RANKS = 4804;
const EXPECTED_BASE_RECORDS = 847238;
const EXPECTED_NEXT_RECORDS = 852098;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-national-school-admission-2021-2025-v3306-nchu-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-national-school-admission-2021-2025-v3306-nchu-runtime-manifest.json",
  };
...
function verifyImport(payload) {
  assert(payload.dataset === "official-national-school-admission-2021-2025-v3306-nchu", `Unexpected import dataset ${payload.dataset}`);
  assert(payload.records?.length === EXPECTED_ADDED_RECORDS, `Expected ${EXPECTED_ADDED_RECORDS} records`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "NCHU source note mismatch");
  assert(new Set(payload.records.map((record) => record.id)).size === EXPECTED_ADDED_RECORDS, "NCHU import contains duplicate ids");
  assert(payload.records.every((record) => record.sourceId === SOURCE_ID && record.schoolName === "南昌航空大学" && record.dataType === "major-admission"), "NCHU import contains out-of-scope rows");
  assert(payload.records.filter((record) => record.rankDerivedFromScore).length === EXPECTED_DERIVED_RANKS, "NCHU derived-rank count drifted");
  assert(payload.records.filter((record) => record.formalScoreScope === "school-official-only").length === 3955, "NCHU ordinary count drifted");
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 905, "NCHU special-path count drifted");
  assert(payload.records.filter((record) => record.minRankEnd).every((record) => record.rankEvidenceScope === "score-derived-provincial-segment" && record.nativeAdmissionRankUnavailable === true), "NCHU rank evidence semantics drifted");
}
...
function alreadyApplied(core, manifest, releaseDir) {
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(rows.every((record) => record.province === province), `NCHU province drift in ${province}`);
    count += rows.length;
  }
  assert(count === EXPECTED_ADDED_RECORDS, `Already-applied NCHU row count drifted: ${count}`);
...
function main() {
  ...
  assert(byProvince.size === 29 && !byProvince.has("西藏") && !byProvince.has("宁夏"), "NCHU province coverage drifted");
  const tempDir = path.join(releaseDir, `.v3306-${process.pid}`);
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(!shard.records.some((record) => record.sourceId === SOURCE_ID), `${province} already contains NCHU rows`);
    ...
    shardStats[province] = { recordsAdded: additions.length, recordsAfter: shard.records.length, derivedRanksAdded: additions.filter((record) => record.rankDerivedFromScore).length };
  }
  ...
  layer.currentFinding = "新增南昌航空大学2021-2025年29省4860条官方分专业最低/平均/最高分和录取人数；4804条位次按最低分对应各省一分一段表换算，明确不是学校录取最低位次。普通school-official-only与专项/艺体等special-path-only保持隔离，学校单校证据最高只到A-候选复核。";
  ...
  coverage.schools = sortedUnique([...(coverage.schools || []), "南昌航空大学"]);
  coverage.cities = sortedUnique([...(coverage.cities || []), "南昌"]);
  coverage.schoolTags = sortedUnique([...(coverage.schoolTags || []), "公办", "理工", "航空特色"]);
  ...
  for (const [yearText, count] of Object.entries(payload.audit.byYear)) {
    ...
    if (!existingSchoolNames.has("南昌航空大学")) row.schools = Number(row.schools || 0) + 1;
  }
  ...
  const runtimeManifest = {
    dataset: "official-national-school-admission-2021-2025-v3306-nchu-runtime",
    ...
    after: {
      ...
      ordinaryRecords: 3955,
      specialPathRecords: 905,
      scoreDerivedRankRecords: EXPECTED_DERIVED_RANKS,
      nativeAdmissionRankRecords: 0,
      rankUnavailableRecords: 56,
