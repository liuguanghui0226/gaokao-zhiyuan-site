...
const BASE_VERSION = "local-deterministic-v3.314-ningxia-official-rank2025-aligned-868426records";
const NEXT_VERSION = "local-deterministic-v3.315-hebei-official-rank2025-aligned-868426records";
const SOURCE_ID = "official-hebei-rank-2025-v3315";
const PROVINCE = "河北";
const YEAR = 2025;
const BASE_RANKS = 117615;
const ADDED_RANKS = 1087;
const NEXT_RANKS = 118702;
const RECORDS = 868426;
const LINKED_RECORDS = 59020;
const EXCLUDED_SPECIAL_RECORDS = 157;
const LINKED_SOURCE_NOTES = 119;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-hebei-rank-conversion-2025-v3315-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-hebei-rank-conversion-2025-v3315-runtime-manifest.json",
  };
...
function verifyImport(payload) {
  assert(payload.dataset === "official-hebei-rank-conversion-2025-v3315-import", `Unexpected dataset ${payload.dataset}`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "Hebei source note mismatch");
  assert(payload.rankConversions?.length === ADDED_RANKS, `Expected ${ADDED_RANKS} rank rows`);
  assert(payload.audit?.parsedRecords === ADDED_RANKS && payload.audit?.duplicateIds === 0, "Hebei import audit drifted");
  assert(payload.audit?.fullTableComparisons === ADDED_RANKS && payload.audit?.fullTableDifferences === 0, "Hebei full-table evidence comparison drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "历史类").length === 533, "History row count drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "物理类").length === 554, "Physics row count drifted");
...
function mapAdmissionRecord(record, rankIndex, topBySubject) {
  ...
  const replacementCaution = "该位次由河北2025官方一分一档表按最低分换算，非原录取/投档表直接公布。";
...
function main() {
  ...
  const item = manifest.shards[PROVINCE];
  assert(item, "Hebei runtime shard is missing");
  ...
  assert(shard.records.length === 69443 && shard.records.length === item.records, "Hebei record count drifted before merge");
  assert(shard.rankConversions.length === 1094 && shard.rankConversions.length === item.rankConversions, "Hebei rank count drifted before merge");
  assert(!shard.rankConversions.some((row) => row.sourceId === SOURCE_ID), "Hebei v3.315 ranks already exist");
  ...
  assert(officialLinked === 47235 && thirdPartyLinked === 11785, "Official/third-party linked split drifted");
  assert(schoolOfficialScopeLinked === 775, "School-official-only linked count drifted");
  assert(topBucketLinked === 2, `Expected 2 top-bucket linked rows, got ${topBucketLinked}`);
  assert(linkedBySource.size === LINKED_SOURCE_NOTES, `Expected ${LINKED_SOURCE_NOTES} linked source IDs, got ${linkedBySource.size}`);
  assert(linkedByType.get("major-admission") === 27135, "Major linked count drifted");
  assert(linkedByType.get("institution-admission") === 11258, "Institution linked count drifted");
  assert(linkedByType.get("vocational-admission") === 20615, "Vocational linked count drifted");
  ...
  assert(shard.rankConversions.length === 2181, "Hebei rank count drifted after merge");
  ...
  for (const [sourceId, count] of linkedBySource) {
    ...
    note.hebei2025ScoreDerivedRankRecords = count;
    if (Number.isFinite(Number(note.rankUnavailableRecords))) {
      assert(Number(note.rankUnavailableRecords) >= count, `${sourceId} rank-unavailable source count is smaller than the Hebei linked count`);
    ...
    if (Number.isFinite(Number(note.scoreOnlyRecords))) {
      assert(Number(note.scoreOnlyRecords) >= count, `${sourceId} score-only source count is smaller than the Hebei linked count`);
    ...
    note.rankAlignmentBoundary = `仅河北2025年${count}条历史/物理类整数最低分记录按同年官方一分一档表换算省级位次区间；其他省份、年份和特殊路径保持原证据状态。`;
  }
  ...
  layer.currentFinding = "新增河北省教育考试院2025年物理类554档、历史类533档官方一分一档记录；考试院18页PDF与独立完整表的1087个位次单元逐项一致。由此为59020条河北2025历史/物理类整数最低分记录补充省级位次区间，其中47235条来自官方来源、11785条来自待复核第三方来源；157条特殊招生路径继续隔离，不进入普通类自动推荐。";
  layer.downgradeReason = `当前数据层 ${NEXT_VERSION} 严格区分院校原生最低位次与最低分换算的省级分数段位次；河北2025只对历史/物理类整数高考总分建立换算，艺术、体育、综合分、科类不明和特殊路径不混入普通类推荐，最高分合并档不生成伪精确名次。`;
  ...
  const tempDir = path.join(releaseDir, `.v3315-${process.pid}`);
  ...
  const runtimeManifest = {
    dataset: "official-hebei-rank-conversion-2025-v3315-runtime",
    generatedAt: payload.generatedAt,
    sourceId: SOURCE_ID,
    importFile: args.importFile,
    releaseDir: args.releaseDir,
    before: { modelVersion: BASE_VERSION, records: RECORDS, rankConversions: BASE_RANKS, provinceRankConversions: 1094 },
    after: {
