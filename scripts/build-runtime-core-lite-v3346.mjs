...
const defaultReleaseDir = "site/data/release-v3.275";
const defaultManifest = "data/admissions/runtime-core-lite-v3346-manifest.json";
...
function parseArgs(argv) {
  const args = { releaseDir: defaultReleaseDir, manifest: defaultManifest };
...
if (projectRoot.startsWith("/Volumes/")) {
...
if (fullCore.modelVersion !== expectedVersion) {
  throw new Error(`Unexpected model version ${fullCore.modelVersion}`);
}
if (runtimeManifest.modelVersion !== fullCore.modelVersion) {
  throw new Error("Core and runtime manifest model versions differ");
}
...
if (reduction < 0.75) {
  throw new Error(`Core-lite raw reduction is only ${(reduction * 100).toFixed(2)}%`);
}
...
const audit = {
  dataset: "runtime-core-lite-v3346",
  generatedAt: new Date().toISOString(),
  releaseDir: path.relative(projectRoot, releaseDir),
  modelVersion: fullCore.modelVersion,
  profile: "core-lite-v1",
  sourceNotes: { full: fullNotes.length, lite: compactNotes.length, fields: sourceNoteFields },
  fullCore: {
