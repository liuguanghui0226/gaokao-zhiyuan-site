...
assert.equal(core.admissionScoreLayer.rankCoverage.records, 133640);
assert.equal(core.admissionScoreLayer.rankSourceCoverage.sources, 221);
assert.equal(core.admissionScoreLayer.rankSourceCoverage.parsedSources, 155);
assert.equal(core.admissionScoreLayer.rankSourceCoverage.queuedSources, 62);
...
assert.deepEqual([year2025.sources, year2025.parsedSources, year2025.queuedSources, year2025.parsedRecords], [87, 63, 20, 29987]);
...
console.log(JSON.stringify({ ok: true, modelVersion, ranks: 1896, linkedAdmissionRecords: 8222, linkedByScope: runtimeManifest.after.linkedByInstitutionScope, sourceNotes: 5132 }, null, 2));
