...
const audit = JSON.parse(fs.readFileSync(
  path.join(projectRoot, "data/admissions/runtime-core-lite-v3328-manifest.json"),
  "utf8",
));

assert.equal(lite.modelVersion, "local-deterministic-v3.328-shanghai-official-rank2025-policy-bonus-inclusive-undergraduate-floor-aligned-868426records");
assert.equal(lite.modelVersion, full.modelVersion);
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 129194);
...
const shanghai = lite.admissionScoreLayer.sourceNotes.find((note) => note.id === "official-shanghai-rank-2025-v3328");
assert.equal(shanghai.scoreBasis, "gaokao-total-including-policy-bonus");
assert.equal(shanghai.rankPolicyBonusIncluded, true);
...
assert.equal(manifest.coreLite.sourceNotes, 5132);
assert.equal(manifest.runtimeProfile.version, "v3.328");
assert.equal(audit.dataset, "runtime-core-lite-v3328");
...
console.log("Runtime core-lite v3.328 tests passed");
