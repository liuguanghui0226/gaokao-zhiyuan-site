...
assert.equal(manifest.version, "v3.347");
assert.equal(manifest.applicationPlanReadiness.currentYear, 2026);
assert.equal(manifest.applicationPlanReadiness.currentPlanConfirmedGroups, 2142);
assert.equal(manifest.applicationPlanReadiness.currentPlanCoverageRate, 0.0055);
assert.equal(manifest.applicationPlanReadiness.provincesWithCurrentYearMatches, 21);
...
assert.equal(rows[0].recentPlanCoverageRate, 0);
assert.equal(rows[0].priorityLabel, "优先补数");
