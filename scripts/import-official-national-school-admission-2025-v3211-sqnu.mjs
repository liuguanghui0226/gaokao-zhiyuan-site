...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2025-v3211-sqnu-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2025-v3211-sqnu";
...
const PAGES = [
  {
    key: "henan-major-2025",
    title: "商丘师范学院2025年本科批各专业录取情况",
    url: `${BASE_URL}/info/1005/3549.htm`,
    rawBase: "2025-henan-major",
    parser: "henanRankedMajor",
  },
  {
    key: "henan-coop-joint-software-art-single-2025",
    title: "商丘师范学院2025年中外合作办学、联合办学、软件学院及艺术单列专业录取情况",
    url: `${BASE_URL}/info/1005/3550.htm`,
    rawBase: "2025-henan-coop-joint-software-art-single",
    parser: "henanRankedMajor",
  },
  {
    key: "henan-directed-xizang-2025",
    title: "商丘师范学院2025年河南省定向西藏生录取情况",
    url: `${BASE_URL}/info/1005/3551.htm`,
    rawBase: "2025-henan-directed-xizang",
    parser: "henanRankedMajor",
  },
  {
    key: "outside-ordinary-sports-art-2025",
    title: "商丘师范学院2025年外省（普通本科，体育，艺术类本科）录取情况",
    url: `${BASE_URL}/info/1005/3552.htm`,
    rawBase: "2025-outside-ordinary-sports-art",
    parser: "outside",
  },
  {
    key: "henan-public-funded-teacher-2025",
    title: "商丘师范学院2025年公费师范生录取情况",
    url: `${BASE_URL}/info/1005/3548.htm`,
    rawBase: "2025-henan-public-funded-teacher",
    parser: "henanPublicTeacher",
  },
  {
    key: "henan-art-sports-2025",
    title: "商丘师范学院2025年河南省艺术及体育本科录取情况",
    url: `${BASE_URL}/info/1005/3547.htm`,
    rawBase: "2025-henan-art-sports",
    parser: "henanArtSports",
  },
];

const SOURCE = {
  id: "official-sqnu-national-2025-school-major-admission",
  quality: "official-school-sqnu-2025-national-pdf-score-rank",
  schoolCode: "10483",
  schoolName: "商丘师范学院",
  city: "商丘",
...
const SUBJECT_RE = "(?:综合改革|普通类|历史类|物理类|文史|理工|艺术类|体育类|书法类统考|书法类|播音与主持类统考|播音与主持类|美术与设计类统考|美术与设计类|舞蹈类统考|舞蹈类|音乐表演类（器乐）统考|音乐表演类（声乐）统考|体育（历史科目组合）|体育（物理科目组合）|体育\\(不分科目类\\)|体育（不分科目类）)";
const BATCH_RE = "(?:艺术第二批\\(统考本科批\\)\\(B段\\)|体育类第一批\\(统考本科批\\)|本科提前批艺术类本科第二批|艺术类本科批书法类|艺术类本科批播音与主持类|艺术类本科批美术与设计类|艺术类本科批舞蹈类|艺术类本科批音乐类|艺术体育统考本科批|艺术本科统考批|艺术类本科批|艺术本科2小批|艺术本科批|体育类本科批|体育本科批|体育本科|本科提前批|提前批公费师范生|普通本科批|普通类平行|本科二批|本科批B段|本科批|专科批|常规批)";
const NUM_RE = "-?\\d+(?:\\.\\d+)?|-";

const ART_PATTERN = /艺术|美术|音乐|舞蹈|播音|编导|表演|戏剧|影视|动画|视觉传达|环境设计|书法|摄影|器乐|声乐|设计/;
const SPORTS_PATTERN = /体育|社会体育|运动训练/;
const SPECIAL_PATTERN = /公费师范|定向|西藏|专项|预科|单列|地方公费|优师|对口|民族|内高班|南疆|哈密/;
const COOP_PATTERN = /中外合作|合作办学|联合办学|软件学院|嵌入式|商丘职业技术学院|联办/;

function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2025-v3211-sqnu.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2025-v3211-sqnu.mjs --use-cache",
    "",
    "Imports 商丘师范学院招生信息网 official 2025 PDF admission score tables.",
  ].join("\n");
}
...
function parseTail(line, count) {
  ...
  return { left: clean(match[1]), nums: match.slice(2).map(clean) };
}

function shouldSkipTextLine(line) {
  const text = clean(line);
  if (!text) return true;
  return /省份|最高分|分数情况|录取情况|录取数|最低分位次|最低分\s+位次|注：|高考文化|统考成绩|绩×/.test(text);
}

function startsWithProvince(line) {
  return new RegExp(`^\\s*${PROVINCE_RE}`).test(line);
}
...
function makeRecord({
  ...
  const record = {
    id: `2025-sqnu-${dataType.replace(/-admission$/, "")}-${stableId([
      page.key,
      province,
      sourceSubjectRaw,
      sourceBatchRaw,
    ...
    rawRow: {
      source: "sqnu-2025-official-pdf-pdftotext-line",
      pageKey: page.key,
      rowIndex,
      sourceLine,
    },
    cautions: [
      "本记录来自商丘师范学院招生信息网官方 2025 年录取分数 PDF，是单校分省/分专业录取边界，不是省级教育考试院全量投档/录取分数表。",
      rankUnavailable
...
async function main() {
  ...
  const sourceNote = {
    id: SOURCE.id,
    title: "商丘师范学院招生信息网：2025年录取分数官方 PDF 表",
    publisher: "商丘师范学院招生信息网",
    url: INDEX_URL,
    quality: SOURCE.quality,
    usage: "从商丘师范学院招生信息网官方“录取分数”栏目下载 2025 年河南普通本科、合作/联办/软件/编导、定向西藏、公费师范、河南艺体和外省普通/艺体本科 PDF 表，使用 pdftotext -layout 抽取单校分省分专业最高分、最低分、平均分、录取人数、最低位次或省控线。学校官网单校数据不关闭任何省级正式投档/录取分数表缺口。",
    ...
    boundaryNotes: [
      ...
      "河南省定向西藏生记录是河南考生定向就业路径，不是西藏自治区省级投档表，不能关闭西藏正式省级缺口。",
      "外省 PDF 多数未列最低分位次，运行层标记 rankUnavailable=true，不生成假位次。",
      "艺术、体育、公费师范、定向西藏等按 special-path-only 隔离，不与普通高考文化分概率混算。",
    ],
  };
  ...
  const payload = {
    ...
    parserVersion: "v3211-sqnu-pdf-pdftotext-2025-01",
  };
