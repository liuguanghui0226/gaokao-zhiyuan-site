...
const DEFAULT_OUTPUT = "site/data/release-v3.275/admission-plan-supplement-v353.json.gz";
const BASE_INPUT = "site/data/release-v3.275/admission-plan-supplement-v352.json.gz";
const DLMU_INPUT = "data/admissions/official-national-school-plan-2026-v353-dlmu-import.json";
const VERSION = "v3.353";
const RECORD_FIELDS = [
...
function main() {
  ...
  if (args.help) { console.log(`Usage:\n  node scripts/build-admission-plan-supplement-v353.mjs --out ${DEFAULT_OUTPUT}`); return; }
  const basePath = path.resolve(PROJECT_ROOT, BASE_INPUT);
  const dlmuPath = path.resolve(PROJECT_ROOT, DLMU_INPUT);
  const base = JSON.parse(zlib.gunzipSync(fs.readFileSync(basePath), { to: "string" }));
  const dlmu = JSON.parse(fs.readFileSync(dlmuPath, "utf8"));
  const dlmuSource = dlmu.sourceNotes?.[0];
  if (base.version !== "v3.352" || !Array.isArray(base.records)) throw new Error("Invalid v3.352 base supplement");
  if (!dlmuSource?.id || !Array.isArray(dlmu.records) || dlmu.records.length !== 836) throw new Error("Invalid DLMU import");
  const records = [...base.records, ...dlmu.records].map(compactRecord);
  ...
  const dlmuSummary = {
    id: dlmuSource.id,
    title: dlmuSource.title,
    publisher: dlmuSource.publisher,
    url: dlmuSource.url,
    quality: dlmuSource.quality,
    schoolCode: dlmu.records[0]?.schoolCode || "",
    schoolName: dlmu.records[0]?.schoolName || "",
    records: dlmu.records.length,
    ordinaryRecords: countBy(dlmu.records, (record) => record.formalScoreScope === "school-official-only"),
    specialPathRecords: countBy(dlmu.records, (record) => record.formalScoreScope === "special-path-only"),
    provinces: new Set(dlmu.records.map((record) => record.province).filter(Boolean)).size,
    planCount: dlmu.records.reduce((sum, record) => sum + Number(record.planCount || 0), 0),
    headlinePlanCount: dlmuSource.headlinePlanCount,
    unattributedDelta: dlmuSource.unattributedDelta,
    summaryGroups: dlmuSource.summaryGroups,
    usage: dlmuSource.usage,
  };
  const payload = {
    version: VERSION,
    ...
    source: {
      ...
      title: "官方院校 2026 分省分专业计划补充（苏州大学、暨南大学、茅台学院、厦门大学、辽宁石油化工大学、大连海事大学）",
      publisher: "苏州大学、暨南大学、茅台学院、厦门大学、辽宁石油化工大学、大连海事大学招生办公室",
      url: dlmuSource.url,
      quality: "official-school-national-plan-html-api-2026-multi-source",
      usage: "运行时独立补充层，汇总六所高校官网 2026 年分省分专业计划；只用于当年专业池、计划数、科类、选科和路径约束，不替代省级考试院计划、投档线或录取概率。",
    },
    sources: [...(base.sources || []).map((source) => ({ ...source })), dlmuSummary],
    summary: {
    ...
    cautions: [...new Set([...(base.cautions || []), ...(dlmuSource.cautions || [])])],
    inputs: [
      { path: BASE_INPUT, sha256: sha256(fs.readFileSync(basePath)), records: base.records.length },
      { path: DLMU_INPUT, sha256: sha256(fs.readFileSync(dlmuPath)), records: dlmu.records.length },
    ],
