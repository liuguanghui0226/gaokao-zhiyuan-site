...
assert.ok(index.includes("./assets/app.js?v=3.343.0"));
assert.ok(app.includes("位次口径含政策加分"));
assert.ok(app.includes("Number((score - minScore).toFixed(3))"));

console.log(JSON.stringify({ ok: true, plainLanguage: true, clickableDisciplineTiles: true, minimumExplicitTextSizePx: 14, assetVersion: "3.343.0" }, null, 2));
