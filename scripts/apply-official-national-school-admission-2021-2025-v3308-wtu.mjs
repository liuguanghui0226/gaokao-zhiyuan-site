...
const BASE_VERSION = "local-deterministic-v3.307-national-school-official-jxust2023-2025-native-rank-855003records";
const NEXT_VERSION = "local-deterministic-v3.308-national-school-official-wtu2021-2025-native-rank-857225records";
const SOURCE_ID = "official-wtu-national-2021-2025-school-major-admission";
const EXPECTED_ADDED_RECORDS = 2222;
const EXPECTED_NATIVE_RANKS = 1921;
const EXPECTED_BASE_RECORDS = 855003;
const EXPECTED_NEXT_RECORDS = 857225;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-national-school-admission-2021-2025-v3308-wtu-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-national-school-admission-2021-2025-v3308-wtu-runtime-manifest.json",
  };
...
function verifyImport(payload) {
  assert(payload.dataset === "official-national-school-admission-2021-2025-v3308-wtu", `Unexpected import dataset ${payload.dataset}`);
  assert(payload.records?.length === EXPECTED_ADDED_RECORDS, `Expected ${EXPECTED_ADDED_RECORDS} records`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "WTU source note mismatch");
  assert(new Set(payload.records.map((record) => record.id)).size === EXPECTED_ADDED_RECORDS, "WTU import contains duplicate ids");
  assert(payload.records.every((record) => record.sourceId === SOURCE_ID && record.schoolName === "武汉纺织大学" && record.dataType === "major-admission"), "WTU import contains out-of-scope rows");
  assert(payload.audit.skippedRows.length === 0 && payload.audit.summaryMismatches.length === 0, "WTU source reconciliation drifted");
  assert(payload.records.filter((record) => record.minRankEnd).length === EXPECTED_NATIVE_RANKS, "WTU native-rank count drifted");
  assert(payload.records.filter((record) => record.rankDerivedFromScore).length === 0, "WTU must not contain derived ranks");
  assert(payload.records.filter((record) => record.formalScoreScope === "school-official-only").length === 1633, "WTU ordinary count drifted");
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 589, "WTU special-path count drifted");
  assert(payload.records.filter((record) => record.minRankEnd).every((record) => record.rankEvidenceScope === "school-recorded-min-score-rank" && record.nativeAdmissionRankUnavailable === false), "WTU rank evidence semantics drifted");
}
...
function alreadyApplied(core, manifest, releaseDir) {
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(rows.every((record) => record.province === province), `WTU province drift in ${province}`);
    count += rows.length;
  }
  assert(count === EXPECTED_ADDED_RECORDS, `Already-applied WTU row count drifted: ${count}`);
...
function main() {
  ...
  assert(byProvince.size === 31 && byProvince.has("西藏") && byProvince.has("宁夏"), "WTU province coverage drifted");
  const tempDir = path.join(releaseDir, `.v3308-${process.pid}`);
  ...
  for (const [province, item] of Object.entries(manifest.shards)) {
    ...
    assert(!shard.records.some((record) => record.sourceId === SOURCE_ID), `${province} already contains WTU rows`);
  ...
  layer.currentFinding = "新增武汉纺织大学2021-2025年31省2222条官方分专业录取记录；1921条直接保留官网最低位次，301条位次未公开记录保持rankUnavailable。155个分省分年查询页的专业录取人数与481个官网汇总行按官网原科类、计划类型、批次和选科备注全部一致；1633条普通school-official-only与589条专项、艺体、中外合作、预科、定向等special-path-only继续隔离。";
  ...
  coverage.schools = sortedUnique([...(coverage.schools || []), "武汉纺织大学"]);
  coverage.cities = sortedUnique([...(coverage.cities || []), "武汉"]);
  coverage.schoolTags = sortedUnique([...(coverage.schoolTags || []), "公办", "理工", "湖北省属"]);
  ...
  for (const [yearText, count] of Object.entries(payload.audit.yearCounts)) {
    ...
    if (!existingSchoolNames.has("武汉纺织大学")) row.schools = Number(row.schools || 0) + 1;
  }
  ...
  const runtimeManifest = {
    dataset: "official-national-school-admission-2021-2025-v3308-wtu-runtime",
