...
const BASE_VERSION = "local-deterministic-v3.299-jiangsu-first-stage-control-lines2026-pending-vocational-and-rank-provenance-847079records";
const NEXT_VERSION = "local-deterministic-v3.300-liaoning-control-lines2026-and-rank-provenance-847095records";
...
const EXPECTED_RECORDS = 16;
const EXPECTED_RANK_ROWS = 1076;
const EXPECTED_NEW_RECORD_COUNT = 847095;
const EXPECTED_NEW_SHARD_RECORDS = 33811;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-liaoning-control-lines-2026-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-liaoning-control-lines-2026-v3300-runtime-manifest.json",
  };
...
function refreshReadiness(container, records, rankConversions) {
  const row = container?.rows?.find((item) => item.province === "辽宁");
  assert(row, "Liaoning province-readiness row is missing");
...
function rankUrlFor(record) {
  ...
  throw new Error(`Unexpected Liaoning rank subject: ${record.subjectType}`);
}
...
function patchRankSourceNote(core, sourceEvidence, sourceRankRows) {
  ...
  assert(note, "Official Liaoning rank source note is missing");
  assert(note.parsedRecords === EXPECTED_RANK_ROWS, `Unexpected Liaoning parsed rank rows: ${note.parsedRecords}`);
  assert(note.url === ORIGINAL_RANK_PAGE_URL, `Unexpected Liaoning rank page URL: ${note.url}`);
  assert(note.quality === "official-liaoning-rank-conversion-pdf", `Liaoning rank quality drifted: ${note.quality}`);
  assert(note.attachmentUrls?.includes(RANK_HISTORY_URL) && note.attachmentUrls?.includes(RANK_PHYSICS_URL), "Liaoning official PDF URLs drifted");
  assert(note.subjects?.find((row) => row.subjectType === "历史类")?.records === 518, "Liaoning history rank inventory drifted");
  assert(note.subjects?.find((row) => row.subjectType === "物理类")?.records === 558, "Liaoning physics rank inventory drifted");
  assert(note.omittedScoreGaps === 4, "Liaoning zero-person score-gap count drifted");
  ...
  for (const [subject, checkpoints] of Object.entries(crossCheck)) {
    ...
    for (const key of ["vocational", "bachelor", "special"]) {
      ...
      assert(rankAt(sourceRankRows, subjectType, score)?.rankEnd === rankEnd, `Liaoning ${subjectType}/${score} rank cross-check drifted`);
    }
  }
  ...
  note.provenanceRevision = {
    verifiedAt: "2026-07-17",
    finding: "辽宁招生考试之窗页面和两份正式PDF本轮直连超时；辽宁省教育厅正式转载索引重新下载并确认历史、物理附件URL。复核运行层1076条记录的科类库存、端点、名次宽度、逐行连续性和本专科/特殊类型边界，保留官方表4个零人数分数缺口，位次数值零改动。本轮未重新解析PDF全文，不把库存连续性复核表述为重新逐行抽取。",
    ...
    valueChanges: 0,
    directOfficialRedownloadStatus: "timed-out-current-session-government-mirror-link-identity-confirmed",
    verificationScope: "government mirror index rehashed; runtime inventory, endpoints, rank widths, continuity and control-boundary checkpoints verified; no fresh full-PDF row extraction",
    governmentMirror: {
      url: sourceEvidence.rankEvidence.governmentMirrorIndexUrl,
      sha256: sourceEvidence.evidence.governmentRankIndex.sha256,
    },
    officialFiles: {
      history: { url: RANK_HISTORY_URL, rows: 518 },
      physics: { url: RANK_PHYSICS_URL, rows: 558 },
    },
  };
}
...
function verifyAlreadyApplied({ core, manifest, shard }) {
  ...
  assert(sourceRecords.length === EXPECTED_RECORDS, `Expected ${EXPECTED_RECORDS} Liaoning control records`);
  assert(sourceRankRows.length === EXPECTED_RANK_ROWS, `Expected ${EXPECTED_RANK_ROWS} Liaoning rank rows`);
  assert(sourceRankRows.every((record) => record.sourceUrl === rankUrlFor(record)), "Liaoning rank source URL repair drifted");
  assert(rankNote?.provenanceRevision?.rankRowsLinked === EXPECTED_RANK_ROWS, "Liaoning rank provenance drifted");
...
function main() {
  ...
  const shardFile = path.join(releaseDir, "liaoning.json.gz");
  ...
  assert(payload.dataset === "official-liaoning-control-lines-2026-import", `Unexpected import dataset: ${payload.dataset}`);
  ...
  assert(payload.records.every((record) => record.province === "辽宁" && record.year === 2026 && record.dataType === "control-line" && record.sourceId === SOURCE_ID), "Import contains an out-of-scope record");
  assert(new Set(payload.records.map((record) => record.id)).size === EXPECTED_RECORDS, "Import contains duplicate ids");
  assert(payload.records.filter((record) => record.formalScoreScope === "control-line-only").length === 4, "Expected four ordinary Liaoning records");
  assert(payload.records.filter((record) => record.formalScoreScope === "special-path-only").length === 12, "Expected 12 Liaoning special-path records");
  assert(payload.records.filter((record) => Number.isFinite(record.professionalMinScore)).length === 0, "Liaoning must not invent numeric professional thresholds");
  assert(payload.records.filter((record) => record.professionalQualification).length === 10, "Expected ten Liaoning sports/art qualification rows");
  assert(payload.records.every((record) => record.scoreMaximum === 750), "Liaoning control records must retain the 750-point score scale");
  ...
  assert(manifest.recordCount === 847079, `Unexpected base record count ${manifest.recordCount}`);
  assert(manifest.rankConversionCount === 116656, `Unexpected rank-conversion count ${manifest.rankConversionCount}`);
  assert(manifest.shards?.["辽宁"]?.records === 33795, `Unexpected Liaoning base count ${manifest.shards?.["辽宁"]?.records}`);
  assert(manifest.shards?.["辽宁"]?.rankConversions === EXPECTED_RANK_ROWS, "Unexpected Liaoning rank-conversion manifest count");
  assert(shard.rankConversions?.length === EXPECTED_RANK_ROWS, `Unexpected Liaoning rank-conversion count ${shard.rankConversions?.length}`);
  ...
  const sourceRankRows = rankRows(shard);
  assert(sourceRankRows.length === EXPECTED_RANK_ROWS, `Expected ${EXPECTED_RANK_ROWS} official Liaoning rank rows`);
  assert(sourceRankRows.every((record) => !record.sourceUrl), "Expected all Liaoning rank rows to need URL repair");
  ...
  const before = {
    ...
    liaoningRecords: shard.records.length,
    ...
    liaoningSha256: sha256(zlib.gunzipSync(fs.readFileSync(shardFile))),
  };
  ...
  assert(JSON.stringify(beforeRankValues) === JSON.stringify(sourceRankRows.map((record) => [record.id, record.score, record.scoreRange, record.rankStart, record.rankEnd, record.sameRankScore, record.sourceQuality])), "Liaoning rank values changed during provenance repair");
  ...
  layer.currentFinding = "辽宁2026普通历史本科/专科442/150分、物理本科/专科344/150分进入普通资格路由；特殊类型、体育和艺术12条保持特殊路径。10条体育/艺术记录只保存文化线与专业资格要求，不补造专业分。1076条普通类位次补齐科类正式PDF URL，数值零改动；原站本轮直连超时，由省教育厅正式转载索引确认附件身份。";
  ...
  const provinceBreakdown = coverage.provinceBreakdown.find((row) => row.province === "辽宁");
  assert(provinceBreakdown, "Liaoning province coverage row is missing");
  ...
  manifest.shards["辽宁"].records = shard.records.length;
  manifest.shards["辽宁"].bytes = shardBytes.byteLength;
  manifest.shards["辽宁"].sha256 = sha256(shardBytes);
  ...
  const runtimeManifest = {
    dataset: "official-liaoning-control-lines-2026-v3300-runtime",
    ...
    after: {
      ...
      liaoningBytes: shardBytes.byteLength,
      liaoningSha256: sha256(shardBytes),
      manifestBytes: manifestBytes.byteLength,
      manifestSha256: sha256(manifestBytes),
    },
  };
  assert(runtimeManifest.after.liaoningRecords === EXPECTED_NEW_SHARD_RECORDS, `Unexpected Liaoning merged count ${runtimeManifest.after.liaoningRecords}`);
