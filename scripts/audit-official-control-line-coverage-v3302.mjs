...
const outFile = path.join(projectRoot, "data/admissions/official-control-line-coverage-2026-v3302.json");
const expectedCovered = ["安徽", "北京", "重庆", "福建", "甘肃", "广东", "广西", "贵州", "海南", "河北", "河南", "黑龙江", "湖北", "湖南", "吉林", "江苏", "江西", "辽宁", "内蒙古", "宁夏", "青海", "山东", "陕西", "上海", "四川", "天津", "西藏", "新疆", "浙江"];
...
assert(manifest.modelVersion === "local-deterministic-v3.302-qinghai-control-lines2026-language-thresholds-and-rank-provenance-847152records", "Unexpected v3.302 model version");
assert(manifest.recordCount === 847152, "Unexpected v3.302 record count");
assert(core.admissionScoreLayer.coverage.dataTypes["control-line"] === 1506, "Unexpected v3.302 control-line count");
...
assert(coveredProvinces.length === 29 && missingProvinces.length === 2, "Expected 29 covered and 2 missing provinces after Qinghai v3.302");
assert(JSON.stringify([...missingProvinces].sort()) === JSON.stringify(["山西", "云南"].sort()), `Unexpected missing provinces: ${missingProvinces.join("、")}`);
...
for (const expected of [
  ...
  assert(qinghai.boundaries.some((row) => row.subjectType === expected[0] && row.routeKind === expected[1] && row.minScore === expected[2] && row.scoreBasis === "gaokao-total"), `Qinghai boundary drifted: ${expected.join("/")}`);
}
assert(!qinghai.boundaries.some((row) => [427, 417, 356, 348, 342, 340, 326, 290, 282, 280, 260, 258].includes(row.minScore)), "Qinghai special, minority-language, art or sports lines leaked into ordinary coverage");
...
const report = {
  dataset: "official-control-line-coverage-2026-v3302",
