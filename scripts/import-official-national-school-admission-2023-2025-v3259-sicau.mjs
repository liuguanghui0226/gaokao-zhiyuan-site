...
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2023-2025-v3259-sicau-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2023-2025-v3259-sicau";
const OFFICIAL_HOME_URL = "https://zsdata.sicau.edu.cn/zsdata/lqxx/";
const INDEX_URL = "https://zsdata.sicau.edu.cn/zsdata/lqxx/";
const SPA_URL = "https://zsdata.sicau.edu.cn/zsdata/lqxx/#/lnfs";
const BASE_API_URL = "https://zsdata.sicau.edu.cn/lqxx/s";
...
const SOURCE = {
  id: "official-sicau-national-2023-2025-school-admission",
  quality: "official-school-sicau-2023-2025-national-api-score-rank",
  schoolCode: "10626",
  schoolName: "四川农业大学",
  city: "成都",
  publisher: "四川农业大学本科招生网",
  tags: ["四川", "成都", "雅安", "四川农业大学", "农业", "双一流"],
};
...
function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2023-2025-v3259-sicau.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2023-2025-v3259-sicau.mjs --use-cache --concurrency 4",
    "",
    "Imports 四川农业大学 2023-2025 历年分数 official API data.",
  ].join("\n");
}
...
function rawAssetFile(url) {
  return `sicau-${path.basename(new URL(url).pathname)}`;
}
...
function queryRawRel(query, index) {
  ...
  return `sicau-${query.nf}-${slugify(query.sf)}-${slugify(query.klmc)}-${slugify(query.xqmc)}-${slugify(query.zslb)}-${index}-${hash}.json`;
}
...
function buildQueries(typeMap) {
  ...
  for (const key of keys) {
    ...
    if (sf === "西藏" && nf === "2025") {
      skippedAllQueries.push({ key, issue: "skip_existing_sicau_xizang_2025_source" });
      continue;
    }
...
function buildRecord(row, rawRel, query, pageIndex, rowIndex, dataType) {
  ...
  const idPrefix = dataType === "institution-admission" ? "sicau-summary" : "sicau";
  const record = {
    ...
    sourcePageKey: `sicau-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
function parseQuery(raw, rawRel, query, pageIndex) {
  ...
  if (payload.code !== 200 && payload.code !== "200") {
    return {
      records,
      summary: {
        pageKey: `sicau-${query.nf}-${query.sf}-${query.klmc}-${query.xqmc}-${query.zslb}`,
...
async function main() {
  ...
  const officialHomeRawFile = "sicau-lqxx-spa-page.html";
  ...
  try {
    ...
    const warningRawFile = "sicau-lqxx-spa-fetch-warning.json";
    ...
    fs.writeFileSync(path.join(rawRoot, warningRawFile), `${JSON.stringify({
      ...
      boundary: "The importer does not infer data from this unavailable shell page; records are sourced from the official zsdata.sicau.edu.cn SPA/API responses below.",
    }, null, 2)}\n`);
  }
  const officialHomeHtml = fs.existsSync(path.join(rawRoot, officialHomeRawFile))
    ...
    : "";
  const indexHtml = await getTextRaw(rawRoot, "sicau-lqxx-index.html", INDEX_URL, args.useCache, "text/html,*/*;q=0.9");
  const jsFiles = [];
  ...
  if (officialHomeHtml && !/lqxx|lqcx|历年|分数|录取|招生/.test(officialHomeHtml)) {
    throw new Error("SICAU historical-score SPA shell no longer matches the expected admission-query surface; refusing to import without that evidence.");
  }

  const getTypeRaw = await postJsonRaw(rawRoot, "sicau-getType-lnfs.json", GET_TYPE_URL, { type: "lnfs" }, args.useCache);
  const themeCfgRaw = await postJsonRaw(rawRoot, "sicau-getTheme.json", THEME_CFG_URL, {}, args.useCache);
  const displayCfgRaw = await postJsonRaw(rawRoot, "sicau-getlqxsgz-lnfs.json", DISPLAY_CFG_URL, { type: "lnfs" }, args.useCache);
  const campusCfgRaw = await postJsonRaw(rawRoot, "sicau-xqlxXsCfg.json", CAMPUS_CFG_URL, {}, args.useCache);
  ...
  const rawFiles = [
    ...
    `${RAW_DIR}/sicau-lqxx-index.html`,
    ...jsFiles,
    `${RAW_DIR}/sicau-getType-lnfs.json`,
    `${RAW_DIR}/sicau-getTheme.json`,
    `${RAW_DIR}/sicau-getlqxsgz-lnfs.json`,
    `${RAW_DIR}/sicau-xqlxXsCfg.json`,
    ...pageSummaries.map((summary) => summary.rawFile),
  ];

  const configSha256 = {};
  for (const rawFile of rawFiles.filter((file) => /sicau-(lqxx-spa-page|lqxx-spa-fetch-warning|lqxx-index|getType|getTheme|getlqxsgz|xqlxXsCfg|.*\.js)/.test(file))) {
  ...
  const sourceNotes = [
    {
      ...
      title: "四川农业大学历年分数 API（2023-2025）",
      url: SPA_URL,
      officialHomeUrl: OFFICIAL_HOME_URL,
      quality: SOURCE.quality,
      usage:
        "学校官网单校分专业录取最高分/平均分/最低分/最低位次边界；可用于四川农业大学候选边界复核、四川/农业院校方向分数段趋势和全国单校分数加厚，不替代任何省级教育考试院全量投档/录取表。源表公开最低位次的行保留最低位次；缺失位次的行保持 rankUnavailable=true 且不生成假位次。",
      rawDir: RAW_DIR,
      rawFiles,
      parsedRecords: records.length,
      ...
      boundaryNotes: [
        ...
        "艺术体育、专项、协作计划、乡村振兴、帮扶计划、中外高水平大学学生交流计划、中外合作、预科、八协计划、内高班、西藏班、新疆班、藏线、内地西藏/新疆、单列、南疆、定向/援疆/新疆协作、运动队等特殊路径按 special-path-only 隔离。",
        "2025 年西藏行已由 official-sicau-xizang-2025-school-admission 窄源导入，本轮全国扩展显式跳过，避免同校同年同省语义重复。",
  ...
  const output = {
    dataset: "official-national-school-admission-2023-2025-v3259-sicau",
