...
const DEFAULT_OUTPUT = "site/data/release-v3.275/admission-plan-supplement-v359.json.gz";
const INPUT = "data/admissions/official-national-school-plan-2026-v359-ncbcjxau-import.json";
const VERSION = "v3.359";
const RECORD_FIELDS = [
  ...
  "sourceQuality", "sourceId", "sourceSubjectRaw", "formalScoreScope", "admissionType", "admissionSubtype", "schoolOfficialScope", "planRemark",
...
function main() {
  ...
  if (args.help) { console.log(`Usage:\n  node scripts/build-admission-plan-supplement-v359.mjs --out ${DEFAULT_OUTPUT}`); return; }
  const inputPath = path.resolve(PROJECT_ROOT, INPUT);
  const input = JSON.parse(fs.readFileSync(inputPath, "utf8"));
  const source = input.sourceNotes?.[0];
  if (input.dataset !== "official-ncbcjxau-national-plan-2026-v3.359" || !source || source.id !== "official-ncbcjxau-national-plan-2026") throw new Error("Invalid NCBJXAU v3.359 import");
  if (!Array.isArray(input.records) || input.records.length !== 443) throw new Error("NCBJXAU record count changed");
  if (source.rawRows !== 443 || source.duplicateRows !== 0 || source.rawPlanCount !== 3400 || source.planCount !== 3400 || source.provinces !== 28) throw new Error("NCBJXAU source totals changed");
  const records = input.records.map(compactRecord);
  if (new Set(records.map((record) => record.id)).size !== records.length) throw new Error("NCBJXAU supplement contains duplicate record IDs");
