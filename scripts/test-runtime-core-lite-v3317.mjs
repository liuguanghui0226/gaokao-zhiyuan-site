...
const auditFile = path.join(projectRoot, "data/admissions/runtime-core-lite-v3317-manifest.json");
...
assert.equal(lite.modelVersion, "local-deterministic-v3.317-liaoning-official-mirror-rank2025-aligned-868426records");
...
assert.equal(lite.admissionScoreLayer.rankConversionRecords, 120750);
assert.equal(liteNotes.length, fullNotes.length);
assert.equal(liteNotes.length, 5121);
...
const rankSource = liteNotes.find((note) => note.id === "official-liaoning-rank-2025-v3317");
assert.ok(rankSource);
assert.equal(rankSource.province, "辽宁");
assert.equal(rankSource.year, 2025);
assert.equal(rankSource.quality, "official-content-mirror-chsi-liaoning-exam-office-pdf-text-eol-cross-verified");
assert.equal(rankSource.url, "https://gaokao.chsi.com.cn/gkxx/zc/ss/202506/20250625/2293390731.html");
...
assert.equal(manifest.coreLite.sourceNotes, 5121);
...
assert.equal(audit.dataset, "runtime-core-lite-v3317");
