...
const YEAR = 2020;
const DEFAULT_OUT = "data/admissions/official-national-school-admission-2020-v3216-sqnu-import.json";
const RAW_DIR = "data/admissions/raw/official-national-school-admission-2020-v3216-sqnu";
const BASE_URL = "https://zhaoban.sqnu.edu.cn";
const INDEX_URL = `${BASE_URL}/lqfs/6.htm`;
const PDFTOTEXT_BIN = process.env.PDFTOTEXT_BIN || "/opt/homebrew/bin/pdftotext";

const PAGES = [
  {
    key: "henan-sports-2020",
    title: "商丘师范学院2020年河南省体育本科录取情况",
    url: `${BASE_URL}/info/1005/2663.htm`,
    rawBase: "2020-henan-sports",
    parser: "henanArtSports2023",
  },
  {
    key: "henan-joint-sqvtc-2020",
    title: "商丘师范学院2020年与商丘职业技术学院联合办学录取情况（6058）",
    url: `${BASE_URL}/info/1005/2563.htm`,
    rawBase: "2020-henan-joint-sqvtc",
    parser: "henanUnrankedMajor",
  },
  {
    key: "henan-preparatory-2020",
    title: "商丘师范学院2020年本科预科班录取情况",
    url: `${BASE_URL}/info/1005/2461.htm`,
    rawBase: "2020-henan-preparatory",
    parser: "henanSportsMinOnly",
  },
  {
    key: "henan-major-2020",
    title: "商丘师范学院2020年河南省普通本科二批录取情况",
    url: `${BASE_URL}/info/1005/2460.htm`,
    rawBase: "2020-henan-major",
    parser: "henanUnrankedMajor",
  },
  {
    key: "henan-coop-undergraduate-2020",
    title: "商丘师范学院2020年普通本科二批(中外合作办学）录取情况",
    url: `${BASE_URL}/info/1005/2457.htm`,
    rawBase: "2020-henan-coop-undergraduate",
    parser: "henanUnrankedMajorNoAvg",
  },
  {
    key: "outside-ordinary-sports-art-2020",
    title: "商丘师范学院2020年外省(普通本科，体育、艺术类本科）录取情况",
    url: `${BASE_URL}/info/1005/2456.htm`,
    rawBase: "2020-outside-ordinary-sports-art",
    parser: "outsideUnranked2020Loose",
  },
  {
    key: "henan-primary-teacher-2020",
    title: "商丘师范学院2020年小学教育（师范）（全科教师）录取情况",
    url: `${BASE_URL}/info/1005/2455.htm`,
    rawBase: "2020-henan-primary-teacher",
    parser: "henanPrimaryTeacher2020",
  },
  {
    key: "henan-coop-vocational-2020",
    title: "商丘师范学院2020年专科（中外合作办学）录取情况",
    url: `${BASE_URL}/info/1005/2454.htm`,
    rawBase: "2020-henan-coop-vocational",
    parser: "henanUnrankedMajor",
  },
  {
    key: "henan-upgrade-2020",
    title: "商丘师范学院2020年专升本录取情况",
    url: `${BASE_URL}/info/1005/2453.htm`,
    rawBase: "2020-henan-upgrade",
    parser: "henanUpgrade",
  },
];

const SOURCE = {
  id: "official-sqnu-national-2020-school-major-admission",
  quality: "official-school-sqnu-2020-national-pdf-score",
  schoolCode: "10483",
  schoolName: "商丘师范学院",
  city: "商丘",
...
const SUBJECT_RE = "(?:综合改革|普通类|历史类|物理类|历史科目组|物理科目组|文史|理工|文科综合|理科综合|艺术类|艺术文|艺术理|艺术|体育类|体育|美术|音乐|默认|文艺|文理综合|专升本|书法学校际联考|书法类统考|书法类|播音与主持类统考|播音与主持类|美术与设计类统考|美术与设计类|美术统考|舞蹈类统考|舞蹈类|舞蹈统考|器乐统考|声乐统考|音乐表演类（器乐）统考|音乐表演类（声乐）统考|体育（历史科目组合）|体育（物理科目组合）|体育\\(不分文理\\)|体育\\(不分科目类\\)|体育（不分科目类）|艺术\\(不分文理\\)|艺术（不分文理）|艺术\\(历史类\\)|艺术\\(不分科目类\\)|体育类（历史等科目类）|美术（历史等科目类）|声乐（历史等科目类）|器乐（历史等科目类）|美术与设计（历史等科目类）|体育类（历史科目组）|体育类（不分历史物理）|艺术类（不分历史物理）)";
const BATCH_RE = "(?:艺术第二批\\(统考本科批\\)\\(B段\\)|体育类第一批\\(统考本科批\\)|本科提前批艺术类本科第二批|艺术类体育类本科一批|普通类本科批A阶段|本科批\\(普通\\)|艺术类本科批书法类|艺术类本科批播音与主持类|艺术类本科批美术与设计类|艺术类本科批舞蹈类|艺术类本科批音乐类|艺术类本科批统考|艺术体育统考本科批|艺术二批\\(B段\\)|艺术类本科二批|本科征集志愿批|体育艺术本一U段|本提前艺本二批|本科A电编类|本科A美术类|本科A音乐类|艺术本科B段|艺术本科B|艺术本科统考批|艺术类本科批|艺术本科2小批|艺术本科A段|艺术本科批|艺术专科批|提前批体育本科|体育类本科批|体育本科批|体育本科|本科提前批|提前批公费师范生|地方公费师范生|专升本批|普通本科批|普通类本科批|普通类平行|本科第二批B类|本科二批普通类|本科第二批|本科一批|本科二批|本科批B段|本科批|高职高专批|专科批|普通批|普通类|常规批|二本)";
const NUM_RE = "-?\\d+(?:\\.\\d+)?|-";

const ART_PATTERN = /艺术|美术|音乐|舞蹈|播音|编导|表演|戏剧|影视|动画|视觉传达|环境设计|书法|摄影|器乐|声乐|设计|绘画/;
...
function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-national-school-admission-2020-v3216-sqnu.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-national-school-admission-2020-v3216-sqnu.mjs --use-cache",
    "",
    "Imports 商丘师范学院招生信息网 official 2020 PDF admission score tables.",
  ].join("\n");
}
...
async function downloadHtml(page, rawRoot, useCache) {
  ...
  if (!/showVsbpdfIframe\("[^"]+?\.pdf/i.test(html) && !/href="[^"]+?\.pdf[^"]*"/i.test(html)) {
    throw new Error(`No official PDF link found in ${page.url}`);
  }
...
function normalizeSubject(raw, context = "", province = "") {
  ...
  if (/文理综合[（(]文[）)]|普通文科|文科普通类/.test(text)) return "历史类";
  if (/文理综合[（(]理[）)]|普通理科|理科普通类/.test(text)) return "物理类";
...
function parseHenanUnrankedMajorNoAvg(page, text, rawPdfRel, rawTxtRel) {
  const records = [];
  const skippedRows = [];
  const leftRegex = new RegExp(`^(${PROVINCE_RE})\\s+(${BATCH_RE})\\s+(${SUBJECT_RE})\\s+(.+)$`);
  text.split(/\r?\n/).forEach((line, lineIndex) => {
    ...
    const [, sourceProvinceRaw, sourceBatchRaw, sourceSubjectRaw, majorNameRaw] = left;
    const [admissionCountRaw, maxScoreRaw, minScoreRaw] = tail.nums;
    const province = normalizeProvince(sourceProvinceRaw);
    const majorName = clean(majorNameRaw);
    const minScore = parseNumber(minScoreRaw);
    if (!majorName || !Number.isFinite(minScore)) {
...
function parseHenanPreparatoryNoAvg(page, text, rawPdfRel, rawTxtRel) {
  const records = [];
  const skippedRows = [];
  const leftRegex = new RegExp(`^(${PROVINCE_RE})\\s+(${BATCH_RE})\\s+(${SUBJECT_RE})$`);
  text.split(/\r?\n/).forEach((line, lineIndex) => {
...
function parseHenanRankedMajorSeparatedRank2021(page, text, rawPdfRel, rawTxtRel) {
  const records = [];
  const skippedRows = [];
  const rankValues = [];
  let inRankBlock = false;
  for (const line of text.split(/\r?\n/)) {
    const rowText = clean(line).replace(/\f/g, "");
    if (/最低分位次/.test(rowText)) {
      inRankBlock = true;
      continue;
    }
    if (!inRankBlock) continue;
    if (startsWithProvince(rowText)) break;
    if (/^\d+$/.test(rowText)) rankValues.push(rowText);
  }
  const leftRegex = new RegExp(`^(${PROVINCE_RE})\\s+(${BATCH_RE})\\s+(${SUBJECT_RE})\\s+(.+)$`);
  text.split(/\r?\n/).forEach((line, lineIndex) => {
    const rowIndex = lineIndex + 1;
    ...
    const [, sourceProvinceRaw, sourceBatchRaw, sourceSubjectRaw, majorNameRaw] = left;
    const [admissionCountRaw, maxScoreRaw, minScoreRaw, avgScoreRaw] = tail.nums;
    const minRankRaw = rankValues[records.length] || "";
    const province = normalizeProvince(sourceProvinceRaw);
    const majorName = clean(majorNameRaw);
    const minScore = parseNumber(minScoreRaw);
    if (!majorName || !Number.isFinite(minScore)) {
  ...
  if (rankValues.length && rankValues.length !== records.length) {
    skippedRows.push({
      reason: "rank-count-mismatch",
      page: page.key,
      parsedRecords: records.length,
      rankValues: rankValues.length,
    });
  }
  return { records, skippedRows };
}
...
function parseOutsideLeft2021(leftRaw) {
  const leftRegex = new RegExp(`^(${PROVINCE_RE})\\s+(${BATCH_RE})\\s+(${SUBJECT_RE})\\s+(.+)$`);
  const prepared = clean(leftRaw)
    .replace(new RegExp(`^(${PROVINCE_RE})(?=\\S)`), "$1 ")
    .replace(/(体育\\(历史类\\)|体育\\(物理类\\)|艺术\\(文\\)|艺术\\(理\\)|艺术\\(不分文理\\))(?=\\S)/g, "$1 ");
  const left = prepared.match(leftRegex);
  if (!left) return null;
  const [, sourceProvinceRaw, sourceBatchRaw, sourceSubjectRaw, middleRaw] = left;
  return { sourceProvinceRaw, sourceSubjectRaw, sourceBatchRaw, middleRaw };
}

function parseOutsideLeft2020Loose(leftRaw) {
  const prepared = clean(leftRaw)
    .replace(new RegExp(`^(${PROVINCE_RE})(?=\\S)`), "$1 ");
  const parts = prepared.split(/\s+/).filter(Boolean);
  if (parts.length < 4) return null;
  const [sourceProvinceRaw, sourceBatchRaw, sourceSubjectRaw, ...majorParts] = parts;
  if (!new RegExp(`^${PROVINCE_RE}$`).test(sourceProvinceRaw)) return null;
  return {
    sourceProvinceRaw,
    sourceSubjectRaw,
    sourceBatchRaw,
    middleRaw: majorParts.join(" "),
  };
}

function parseOutsideUnranked2020Loose(page, text, rawPdfRel, rawTxtRel) {
  const records = [];
  const skippedRows = [];
  logicalOutsideLines(text, 4).forEach(({ line, rowIndex }) => {
    if (shouldSkipTextLine(line)) return;
    ...
    const left = parseOutsideLeft2020Loose(tail.left);
    if (!left) {
      skippedRows.push({ reason: "left-parse-failed", page: page.key, rowIndex, left: tail.left, line });
      return;
    }
...
function parseOutsideUnranked2021(page, text, rawPdfRel, rawTxtRel) {
  const records = [];
  const skippedRows = [];
  logicalOutsideLines(text, 4).forEach(({ line, rowIndex }) => {
    if (shouldSkipTextLine(line)) return;
    ...
    const left = parseOutsideLeft2021(tail.left);
    if (!left) {
      skippedRows.push({ reason: "left-parse-failed", page: page.key, rowIndex, left: tail.left, line });
      return;
    }
...
function splitPrimaryTeacherMajor2020(majorNameRaw) {
  const majorRaw = clean(majorNameRaw);
  const match = majorRaw.match(/^(小学教育[（(]师范[）)][（(]全科教师[）)])(.+)$/);
  if (!match) return { majorName: majorRaw, candidateCounty: "" };
  return { majorName: clean(match[1]), candidateCounty: clean(match[2]) };
}

function parseHenanPrimaryTeacher2020(page, text, rawPdfRel, rawTxtRel) {
  const records = [];
  const skippedRows = [];
  const leftRegex = new RegExp(`^(${PROVINCE_RE})\\s+(${BATCH_RE})\\s+(${SUBJECT_RE})\\s+(.+)$`);
  text.split(/\r?\n/).forEach((line, lineIndex) => {
    ...
    const [admissionCountRaw, maxScoreRaw, minScoreRaw, avgScoreRaw] = tail.nums;
    const province = normalizeProvince(sourceProvinceRaw);
    const { majorName, candidateCounty } = splitPrimaryTeacherMajor2020(majorNameRaw);
    const minScore = parseNumber(minScoreRaw);
    if (!majorName || !Number.isFinite(minScore)) {
      skippedRows.push({ reason: "missing-required-field", page: page.key, rowIndex, line });
      return;
    }
    const classification = classifyAdmission(page, sourceSubjectRaw, sourceBatchRaw, majorName, `${candidateCounty} 地方公费师范生`);
    records.push(makeRecord({
      page,
      rawPdfRel,
      rawTxtRel,
      ...
      classification,
      sourceLine: clean(line),
      extra: { candidateCategory: "小学全科公费师范生" },
    }));
  });
  return { records, skippedRows };
}
...
async function main() {
  ...
  for (const page of PAGES) {
    ...
    } else if (page.parser === "henanUnrankedMajorNoAvg") {
      parsed = parseHenanUnrankedMajorNoAvg(page, text, rawPdfRel, rawTxtRel);
    } else if (page.parser === "henanPreparatoryNoAvg") {
      parsed = parseHenanPreparatoryNoAvg(page, text, rawPdfRel, rawTxtRel);
    } else if (page.parser === "henanRankedMajorSeparatedRank2021") {
      parsed = parseHenanRankedMajorSeparatedRank2021(page, text, rawPdfRel, rawTxtRel);
    } else if (page.parser === "outsideUnranked2022") {
      parsed = parseOutsideUnranked2022(page, text, rawPdfRel, rawTxtRel);
    } else if (page.parser === "outsideUnranked2020Loose") {
      parsed = parseOutsideUnranked2020Loose(page, text, rawPdfRel, rawTxtRel);
    } else if (page.parser === "outsideUnranked2021") {
      parsed = parseOutsideUnranked2021(page, text, rawPdfRel, rawTxtRel);
    ...
    } else if (page.parser === "henanPrimaryTeacher2020") {
      parsed = parseHenanPrimaryTeacher2020(page, text, rawPdfRel, rawTxtRel);
  ...
  const sourceNote = {
    id: SOURCE.id,
    title: "商丘师范学院招生信息网：2020年录取分数官方 PDF 表",
    publisher: "商丘师范学院招生信息网",
    url: INDEX_URL,
    quality: SOURCE.quality,
    usage: "从商丘师范学院招生信息网官方“录取分数”栏目下载 2020 年河南体育本科、联办、本科预科、普通本科二批、中外合作本科、小学全科教师、专科中外合作、专升本和外省普通/艺体本科单年 PDF 表，使用 pdftotext -layout 抽取单校分省分专业最高分、最低分、平均分和录取人数。学校官网单校数据不关闭任何省级正式投档/录取分数表缺口。",
    parsedRecords: records.length,
    ...
    boundaryNotes: [
      ...
      "河南省预科、小学全科教师和专升本记录是河南考生特殊路径，不是省级教育考试院全量投档/录取表；不关闭西藏正式省级缺口。",
      "2020 年本包导入单年 PDF 表；2017-2020 艺术体育对比表未作为本轮源，避免与单年体育表重复并避免混年列解析误差。",
      "2020 年本轮源表未公开最低分位次；所有行标记 rankUnavailable=true，不生成假位次。",
  ...
  const payload = {
    ...
    parserVersion: "v3216-sqnu-pdf-pdftotext-2020-01",
  };
