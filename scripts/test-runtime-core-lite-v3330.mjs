...
const audit = JSON.parse(fs.readFileSync(
  path.join(projectRoot, "data/admissions/runtime-core-lite-v3330-manifest.json"),
  "utf8",
));
...
const jiangxi = lite.admissionScoreLayer.sourceNotes.find((note) => note.id === "official-jiangxi-rank-2025-v3330");
assert.equal(jiangxi.scoreBasis, "gaokao-total-including-policy-bonus");
assert.equal(jiangxi.rankPolicyBonusIncluded, true);
assert.equal(jiangxi.automaticAdmissionScoreAlignmentAllowed, true);
...
console.log("Runtime core-lite v3.330 tests passed");
