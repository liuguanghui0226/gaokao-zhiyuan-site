...
const DEFAULT_OUTPUT = "site/data/release-v3.275/admission-plan-supplement-v361.json.gz";
const INPUT = "data/admissions/official-national-school-plan-2026-v361-gzu-import.json";
const VERSION = "v3.361";
const RECORD_FIELDS = [
  ...
  "tuition", "sourceTuitionRaw", "programDuration", "sourceQuality", "sourceId", "sourceTypeRaw", "sourceSubjectRaw", "sourceDepartmentRaw", "sourceCampusRaw", "sourceNoteRaw",
  "formalScoreScope", "admissionType", "admissionSubtype", "schoolOfficialScope", "planRemark", "sourceUrl", "sourcePageUrl", "sourceIndexUrl", "officialEvidencePath", "officialCharterEvidencePath",
];
...
function main() {
  ...
  if (args.help) { console.log(`Usage:\n  node scripts/build-admission-plan-supplement-v361.mjs --out ${DEFAULT_OUTPUT}`); return; }
  const inputPath = path.resolve(PROJECT_ROOT, INPUT);
  const input = JSON.parse(fs.readFileSync(inputPath, "utf8"));
  const source = input.sourceNotes?.[0];
  if (input.dataset !== "official-gzu-national-plan-2026-v3.361" || !source || source.id !== "official-gzu-national-plan-2026") throw new Error("Invalid GZU v3.361 import");
  if (!Array.isArray(input.records) || input.records.length !== 1180) throw new Error("GZU record count changed");
  if (source.rawRecords !== 1180 || source.invalidRows !== 0 || source.duplicateRows !== 0 || source.rawPlanCount !== 8840 || source.planCount !== 8840 || source.provinces !== 31) throw new Error("GZU source totals changed");
  const records = input.records.map(compactRecord);
  if (new Set(records.map((record) => record.id)).size !== records.length) throw new Error("GZU supplement contains duplicate record IDs");
  ...
  const payload = {
    ...
    source: { id: source.id, title: source.title, publisher: source.publisher, url: source.url, charterUrl: source.charterUrl, quality: source.quality, usage: source.usage },
