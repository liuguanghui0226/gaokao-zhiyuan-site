...
assert.equal(metrics.items, 545);
assert.equal(metrics.sources, 239);
assert.equal(metrics.authoredSummaries, 38);
assert.equal(metrics.citationOnlyItems, 507);

context.__geographyTest.renderGeography();
assert.match(view.innerHTML, /data-geography-version="geo-2026\.08\.24\.35"/);
assert.match(view.innerHTML, /资料边界与更新/);
assert.match(view.innerHTML, /资料版本 geo-2026\.08\.24\.35/);
assert.match(view.innerHTML, /引文型方法卡/);
assert.match(view.innerHTML, /全部课程 · 545/);
assert.match(view.innerHTML, /地理必修第一册 · 100/);
assert.match(view.innerHTML, /地理必修第二册 · 110/);
assert.match(view.innerHTML, /选择性必修1 自然地理基础 · 115/);
assert.match(view.innerHTML, /选择性必修2 区域发展 · 108/);
assert.match(view.innerHTML, /选择性必修3 资源、环境与国家安全 · 112/);
