...
assert.equal(metrics.items, 215);
assert.equal(metrics.sources, 88);
assert.equal(metrics.authoredSummaries, 38);
assert.equal(metrics.citationOnlyItems, 177);

context.__gaokaoTest.renderGeography();
assert.match(view.innerHTML, /data-geography-version="geo-2026\.08\.22\.13"/);
assert.match(view.innerHTML, /资料边界与更新/);
assert.match(view.innerHTML, /资料版本 geo-2026\.08\.22\.13/);
assert.match(view.innerHTML, /引文型方法卡/);
assert.match(view.innerHTML, /全部课程 · 215/);
assert.match(view.innerHTML, /地理必修第一册 · 34/);
assert.match(view.innerHTML, /地理必修第二册 · 44/);
assert.match(view.innerHTML, /选择性必修1 自然地理基础 · 49/);
assert.match(view.innerHTML, /选择性必修2 区域发展 · 42/);
assert.match(view.innerHTML, /选择性必修3 资源、环境与国家安全 · 46/);
assert.match(view.innerHTML, /215 条摘要/);
