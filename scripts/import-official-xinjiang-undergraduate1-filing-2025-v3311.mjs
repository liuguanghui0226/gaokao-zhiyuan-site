...
const TMP_ROOT = path.join(os.homedir(), ".codex", "tmp", "gaokao-xinjiang-undergraduate1-2025-v3311");
const DEFAULT_URL = "https://www.xjzk.gov.cn/c/2025-07-25/494639.shtml";
const DEFAULT_OUT = "data/admissions/official-xinjiang-undergraduate1-filing-2025-v3311-import.json";
const DEFAULT_RAW_REL = "data/admissions/raw/official-xinjiang-undergraduate1-filing-2025-v3311";
...
const IMAGE_SUBJECTS = new Map([
  ["29619", { subjectType: "历史类", originalSubject: "文史" }],
  ["29620", { subjectType: "物理类", originalSubject: "理工" }],
  ["29623", { subjectType: "物理类", originalSubject: "理工" }],
]);
const EXPECTED_IMAGE_AUDIT = new Map([
  ["29619", { width: 2829, height: 24042, rows: 200, sha256: "3d5e72f6587f52fd42b875730b1cdc9b4bd0895c361f5ee1fea4079b86ce983a" }],
  ["29620", { width: 2829, height: 19773, rows: 165, sha256: "ac343e28f8e4d10519ee40663a40e132c888e364b8f736195ff77eccb83291e4" }],
  ["29623", { width: 2829, height: 16653, rows: 140, sha256: "b0f9b75007aca5f00d2be4a19e966315331d8964bf40ad3551d35bd5bed8f110" }],
]);
...
function usage() {
  return [
    "Usage:",
    `  node scripts/import-official-xinjiang-undergraduate1-filing-2025-v3311.mjs --out ${DEFAULT_OUT}`,
    "  node scripts/import-official-xinjiang-undergraduate1-filing-2025-v3311.mjs --use-cache",
    "",
    "Imports Xinjiang 2025 ordinary undergraduate first-batch official image filing scores.",
  ].join("\n");
}
...
function invariant(condition, message) {
  if (!condition) throw new Error(`Xinjiang undergraduate1 v3.311 audit failed: ${message}`);
}
...
async function downloadText(url) {
  const response = await fetch(url, {
    headers: {
      "user-agent": "Mozilla/5.0 gaokao-xinjiang-undergraduate1-v3311-importer/1.0",
...
function extractPageMeta(html, pageUrl) {
  ...
  if (!/新疆维吾尔自治区2025年普通高校招生普通类本科一批次投档情况/.test(title)) {
    throw new Error(`Unexpected Xinjiang undergraduate1 page title: ${title}`);
  }
  const imageMeta = [];
  const regex = /<a\s+href=["']([^"']*\/upload\/resources\/image\/2025\/07\/25\/(\d+)\.jpg)["'][^>]*>\s*<img\b([^>]*)>/gi;
...
function schoolTags(record) {
  ...
  const tags = ["新疆官方本科一批投档线", record.subjectType];
  ...
  if (Number(record.minScore) < 400) tags.push("本科一批低分边界");
  return [...new Set(tags)];
}

function columnsFor(width) {
  const scale = width / 2829;
  ...
  return {
    ...
    filed: [s(1390), s(1585)],
    high: [s(1585), s(1785)],
    minTotal: [s(1785), s(1980)],
    tie1: [s(1980), s(2175)],
    comprehensive: [s(2175), s(2370)],
    tie3: [s(2370), s(2565)],
    avg: [s(2565), width + 10],
  };
}
...
function parseImageRows({ image, items, dimensions }) {
  ...
  for (const anchor of anchors) {
    ...
    const minScore = Number.isFinite(correction.minScore) ? correction.minScore : bestNumber(rowItems, ranges.minTotal);
    ...
    const highestScoreOcr = bestNumber(rowItems, ranges.high);
    ...
    const planCount = Number.isFinite(correction.planCount) ? correction.planCount : bestNumber(rowItems, ranges.plan);
    const filingCount = Number.isFinite(correction.filingCount) ? correction.filingCount : bestNumber(rowItems, ranges.filed);
    ...
    const tie1 = bestNumber(rowItems, ranges.tie1);
    const comprehensive = bestNumber(rowItems, ranges.comprehensive);
    const tie3 = bestNumber(rowItems, ranges.tie3);
    ...
    const base = {
      ...
      scoreMetric: "新疆教育考试院普通本科一批院校最低投档排序分",
      rankMetric: "官方投档表未公开最低位次",
      ...
      sourceId: SOURCE_ID,
      sourceQuality: "official-xinjiang-2025-undergraduate1-institution-filing-image-ocr-score-only-v3311",
      ...
      cautions: [
        ...
        "原表为普通类本科一批次院校投档分数情况，只公开投档分和同分排序项，不含最低位次；本导入不生成假位次。",
    ...
    base.id = `${YEAR}-xinjiang-undergrad1-v3311-filing-${hash(idBase, 18)}`;
...
async function main() {
  ...
  invariant(records.length === 505, `expected 505 unique records, got ${records.length}`);
  invariant(records.filter((record) => record.subjectType === "历史类").length === 200, "history row count must be 200");
  invariant(records.filter((record) => record.subjectType === "物理类").length === 305, "physics row count must be 305");
  ...
  invariant(records.every((record) => Number.isFinite(record.minScore) && record.minScore >= 100 && record.minScore <= 750), "invalid minimum score");
  invariant(records.every((record) => Number.isFinite(record.maxScore) && record.maxScore >= record.minScore && record.maxScore <= 750), "invalid maximum score");
  invariant(records.every((record) => Number.isFinite(record.avgScore) && record.avgScore >= record.minScore && record.avgScore <= record.maxScore), "invalid average score");
  invariant(records.every((record) => record.tieBreakScores?.totalScore === record.minScore), "tie-break total does not equal minimum score");
  invariant(records.every((record) => Number.isFinite(record.tieBreakScores?.chinese) && record.tieBreakScores.chinese >= 0 && record.tieBreakScores.chinese <= 150), "invalid Chinese tie-break score");
  invariant(records.every((record) => Number.isFinite(record.tieBreakScores?.math) && record.tieBreakScores.math >= 0 && record.tieBreakScores.math <= 150), "invalid math tie-break score");
  invariant(records.every((record) => Number.isFinite(record.tieBreakScores?.comprehensive) && record.tieBreakScores.comprehensive >= 0 && record.tieBreakScores.comprehensive <= 300), "invalid comprehensive tie-break score");
  ...
  const rawManifest = {
    dataset: "official-xinjiang-undergraduate1-filing-2025-v3311-raw",
    generatedAt,
    sourceUrl: args.url,
  ...
  fs.writeFileSync(outPath, JSON.stringify({
    ...
    sourceNotes: [{
      ...
      quality: "official-xinjiang-2025-undergraduate1-institution-filing-image-ocr-score-only-v3311",
      usage: `官方普通类本科一批次院校投档图片表，经本地 OCR 抽取 ${records.length} 条院校投档线；无最低位次，按 score-only 本科一批进档边界使用。`,
      evidenceBoundary: "province-official ordinary undergraduate first-batch institution filing score, plan/filing count and tie-break components; no major result, minimum rank, elective requirement or admission probability",
