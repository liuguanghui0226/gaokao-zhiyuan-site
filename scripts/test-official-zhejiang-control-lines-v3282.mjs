...
const modelVersion = "local-deterministic-v3.282-zhejiang-control-lines2026-and-segment-routing-846519records";
...
assert.equal(core.admissionScoreLayer.structuredRecords, 846519);
assert.equal(core.admissionScoreLayer.sourceNotes.length, 5088);
assert.equal(core.admissionScoreLayer.coverage.dataTypes["control-line"], 873);
assert.deepEqual(core.admissionScoreLayer.coverage.formalScoreMissingProvinces, ["西藏"]);
assert.equal(manifest.modelVersion, modelVersion);
assert.equal(manifest.recordCount, 846519);
