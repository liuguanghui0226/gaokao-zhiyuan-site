...
const BASE_VERSION = "local-deterministic-v3.315-hebei-official-rank2025-aligned-868426records";
const NEXT_VERSION = "local-deterministic-v3.316-chongqing-authority-linked-rank2025-aligned-868426records";
const SOURCE_ID = "official-chongqing-rank-2025-v3316";
const PROVINCE = "重庆";
const YEAR = 2025;
const BASE_RANKS = 118702;
const ADDED_RANKS = 975;
const NEXT_RANKS = 119677;
const RECORDS = 868426;
const LINKED_RECORDS = 29017;
const EXCLUDED_SPECIAL_RECORDS = 337;
const LINKED_SOURCE_NOTES = 110;
const TOP_BUCKET_LINKED_RECORDS = 50;

function parseArgs(argv) {
  const args = {
    importFile: "data/admissions/official-chongqing-rank-conversion-2025-v3316-import.json",
    releaseDir: "site/data/release-v3.275",
    runtimeManifest: "data/admissions/official-chongqing-rank-conversion-2025-v3316-runtime-manifest.json",
  };
...
function scoreDerivedRankBoundary(note, chongqingCount) {
  const scopes = [];
  const hebeiCount = Number(note.hebei2025ScoreDerivedRankRecords || 0);
  if (hebeiCount > 0) scopes.push(`河北2025年${hebeiCount}条`);
  scopes.push(`重庆2025年${chongqingCount}条`);
...
function verifyImport(payload) {
  assert(payload.dataset === "official-chongqing-rank-conversion-2025-v3316-import", `Unexpected dataset ${payload.dataset}`);
  assert(payload.sourceNotes?.length === 1 && payload.sourceNotes[0].id === SOURCE_ID, "Chongqing source note mismatch");
  assert(payload.rankConversions?.length === ADDED_RANKS, `Expected ${ADDED_RANKS} rank rows`);
  assert(payload.audit?.parsedRecords === ADDED_RANKS && payload.audit?.duplicateIds === 0, "Chongqing import audit drifted");
  assert(payload.audit?.fullTableComparisons === ADDED_RANKS && payload.audit?.fullCellComparisons === 2925 && payload.audit?.fullTableDifferences === 0, "Chongqing full-table evidence comparison drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "历史类").length === 473, "History row count drifted");
  assert(payload.rankConversions.filter((row) => row.subjectType === "物理类").length === 502, "Physics row count drifted");
...
function mapAdmissionRecord(record, rankIndex, topBySubject) {
  ...
  const replacementCaution = "该位次由重庆2025一分一段表按最低分换算，非原录取/投档表直接公布。";
...
function main() {
  ...
  const item = manifest.shards[PROVINCE];
  assert(item, "Chongqing runtime shard is missing");
  ...
  assert(shard.records.length === 62503 && shard.records.length === item.records, "Chongqing record count drifted before merge");
  assert(shard.rankConversions.length === 988 && shard.rankConversions.length === item.rankConversions, "Chongqing rank count drifted before merge");
  assert(!shard.rankConversions.some((row) => row.sourceId === SOURCE_ID), "Chongqing v3.316 ranks already exist");
  ...
  assert(officialLinked === 28481 && thirdPartyLinked === 536, "Official/third-party linked split drifted");
  assert(schoolOfficialScopeLinked === 530, "School-official-only linked count drifted");
  assert(topBucketLinked === TOP_BUCKET_LINKED_RECORDS, `Expected ${TOP_BUCKET_LINKED_RECORDS} top-bucket linked rows, got ${topBucketLinked}`);
  assert(linkedBySource.size === LINKED_SOURCE_NOTES, `Expected ${LINKED_SOURCE_NOTES} linked source IDs, got ${linkedBySource.size}`);
  assert(linkedByType.get("major-admission") === 19800, "Major linked count drifted");
  assert(linkedByType.get("institution-admission") === 115, "Institution linked count drifted");
  assert(linkedByType.get("vocational-admission") === 9094, "Vocational linked count drifted");
  assert(linkedByType.get("major-group-admission") === 2, "Major-group linked count drifted");
  ...
  assert(shard.rankConversions.length === 1963, "Chongqing rank count drifted after merge");
  ...
  for (const [sourceId, count] of linkedBySource) {
    ...
    note.chongqing2025ScoreDerivedRankRecords = count;
    if (Number.isFinite(Number(note.rankUnavailableRecords))) {
      assert(Number(note.rankUnavailableRecords) >= count, `${sourceId} rank-unavailable source count is smaller than the Chongqing linked count`);
    ...
    if (Number.isFinite(Number(note.scoreOnlyRecords))) {
      assert(Number(note.scoreOnlyRecords) >= count, `${sourceId} score-only source count is smaller than the Chongqing linked count`);
  ...
  layer.currentFinding = "新增重庆2025年历史类473档、物理类502档一分一段记录；两份考试院原始链接由两个转载索引共同保留，EOL完整表与独立完整表975行、2925个分数/人数/累计人数单元逐项一致。由此为29017条重庆2025历史/物理类整数最低分记录补充省级位次区间，其中28481条来自官方来源、536条来自待复核第三方来源；337条特殊招生路径继续隔离。";
  layer.downgradeReason = `当前数据层 ${NEXT_VERSION} 严格区分院校原生最低位次与最低分换算的省级分数段位次；重庆2025只对历史/物理类整数高考总分建立换算，艺术、体育、综合分、科类不明和特殊路径不混入普通类推荐，最高分合并档不生成伪精确名次。`;
  ...
  const tempDir = path.join(releaseDir, `.v3316-${process.pid}`);
  ...
  const runtimeManifest = {
    dataset: "official-chongqing-rank-conversion-2025-v3316-runtime",
    generatedAt: payload.generatedAt,
    sourceId: SOURCE_ID,
    importFile: args.importFile,
    releaseDir: args.releaseDir,
    before: { modelVersion: BASE_VERSION, records: RECORDS, rankConversions: BASE_RANKS, provinceRankConversions: 988 },
    after: {
