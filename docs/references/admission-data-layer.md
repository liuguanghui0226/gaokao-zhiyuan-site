...
v3.297 当前运行版本为 `local-deterministic-v3.297-hainan-control-lines2026-and-rank-provenance-847033records`，含 847033 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5103 条来源说明。新增海南 2026 本科控制线 14 条：900 分制本科批普通类 479 分为唯一 `control-line-only`；国家专项、特殊类型、体育和 10 个艺术方向共 13 条为 `special-path-only`。体育文化 421 分、专业 75 分分字段保存；艺术只保存 383/407 文化线和专业合格要求，不生成官方未给出的专业数值。实施办法明确专科批实行先报志愿再划线，普通专科继续 `pending-official-release`，不使用往年线或高职分类考试线替代。海南 547 条官方位次逐条补齐考试局正式 PDF URL，阳光高考转载的 20 页 PDF 重新解析 547 行并与运行数据逐行零差异。控制线、专业线、位次换算、院校/专业组投档线和专业录取最低分继续分层，任何一层都不能替代另一层。
