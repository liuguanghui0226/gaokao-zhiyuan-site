...
v3.296 当前运行版本为 `local-deterministic-v3.296-guizhou-control-lines2026-and-rank-provenance-847019records`，含 847019 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5102 条来源说明。新增贵州 2026 控制线 29 条：普通历史类本科/专科 439/200 分、物理类本科/专科 393/200 分共 4 条为 `control-line-only`；特殊类型、艺术、体育和民汉双语口语 25 条为 `special-path-only`。22 条艺体记录只保存文化线与专业合格条件，官方控制线页没有给出的专业数值不生成；97.70 分民汉双语口语资格线按 100 分独立量纲保存。控制线由阳光高考正式转载、铜仁政府原图和中新网正文逐项核验；贵州考试院原站本轮 TLS 失败，来源保持 `official-content-mirror`。贵州 1201 条官方位次换算逐条补齐历史/物理正式 PDF URL，位次数值变化为 0；两份镜像 PDF 与既有正式 PDF 清单 SHA-256 相同。控制线、专业合格线、位次换算、院校/专业组投档线和专业录取最低分继续分层，任何一层都不能替代另一层。
