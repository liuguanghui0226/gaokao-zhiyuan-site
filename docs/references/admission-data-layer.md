...
v3.328 当前运行版本为 `local-deterministic-v3.328-shanghai-official-rank2025-policy-bonus-inclusive-undergraduate-floor-aligned-868426records`，含 868426 条结构化录取/投档/计划/资格边界、129194 条位次换算记录和 5132 条来源说明。新增上海 2025 本科录取控制分数线上 222 条综合类位次记录；上海市教育考试院 4 页官方扫描 PDF 的 221 行、663 个机器读取数值格经完整表和累计人数算术复核，621 分缺失行及 563 分累计人数误读作为 2 处披露修正保存。运行层为 1964 条上海普通整数最低分记录补充 `score-derived-provincial-segment` 位次区间，其中官方来源 1708 条；14 条特殊路径隔离。政策性加分计入高考成绩，换算记录统一保存 `rankPolicyBonusIncluded=true`。623 分及以上仅保存 1-52 名合并档，401 分及以下不越过本科线外推；换算位次不是院校原表直接公布的录取最低位次。
