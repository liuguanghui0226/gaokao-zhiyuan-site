...
v3.318 当前运行版本为 `local-deterministic-v3.318-hunan-education-department-rank2025-aligned-868426records`，含 868426 条结构化录取/投档/计划/资格边界、121889 条位次换算记录和 5122 条来源说明。新增湖南 2025 历史类 550 档、物理类 589 档一分一段记录；湖南省教育厅来源标注完整表共 1152 行且累计逐行闭合，独立镜像 1139 行和 3417 个分数、同分人数、累计人数单元零差异，阳光高考另存“湖南考试招生”来源原图。运行层为 9376 条湖南 2025 普通历史/物理类整数最低分记录补充 `score-derived-provincial-segment` 位次区间，388 条特殊路径隔离；历史 9 个、物理 2 个零人数分数不生成假行，0-99 分合并档不用于换算。省级档分含优惠加分，可换算同年同科类文化分位次，但不等于院校原表直接公布的录取最低位次，也不用于艺体综合分。
