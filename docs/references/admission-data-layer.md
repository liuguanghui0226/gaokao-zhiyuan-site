...
v3.295 当前运行版本为 `local-deterministic-v3.295-guangxi-control-lines2026-and-rank-provenance-846990records`，含 846990 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5101 条来源说明。新增广西 2026 控制线 50 条：普通历史类本科/专科 398/180 分、物理类本科/专科 368/180 分共 4 条为 `control-line-only`；特殊类型、体育和艺术 46 条为 `special-path-only`。44 条艺体文化课与专业统考双门槛记录分字段保存，专业分不替代文化分。控制线来自广西高校转载的招生考试院原图并由中新网文字镜像逐项交叉核验；考试院原站本轮 TLS 连接失败，来源保持 `official-content-mirror`。广西 1012 条官方位次换算逐条补齐历史/物理正式页面 URL，位次数值变化为 0。控制线、专业统考线、位次换算、院校/专业组投档线和专业录取最低分继续分层，任何一层都不能替代另一层。
