...
v3.333 当前运行版本为 `local-deterministic-v3.333-beijing-official-undergraduate-rank2025-national-bonus-municipal-local-bonus-guard-868426records`，含 868426 条结构化录取/投档/计划/资格边界、133640 条位次换算记录和 5136 条来源说明。北京 2025 官方一分一段明确含全国性照顾加分，本科普通批使用六科 750 分录取总成绩；1271 条非市属高校院校专业组最低分获得 `score-derived-provincial-segment` 位次。126 条市属高校记录因地方 5 分只适用于市属高校、与全国性加分分数分布不完全同口径而保持无位次；580 条专科记录使用语数外三科总分，也保持无位次。换算结果不是院校投档表直接公布的原生最低位次，更不是组内具体专业录取最低位次或录取概率。

v3.331 历史运行版本为 `local-deterministic-v3.331-guangdong-official-rank2025-dual-level-bonus-full-table-aligned-868426records`，含 868426 条结构化录取/投档/计划/资格边界、132497 条位次换算记录和 5135 条来源说明。广东 2025 历史类 573 档、物理类 598 档官方普通类分数段记录按 `rankUsage=undergraduate|vocational` 展开为 2342 条位次；两种口径分别保存本科层次加分和专科层次加分，禁止混用。运行层为 1253 条广东普通整数最低分记录补充 `score-derived-provincial-segment` 位次，其中本科 1251 条、专科 2 条；54 条特殊路径、94 条模糊边界和 3 条非整数计分不自动套表。换算位次是最低分对应的全省分数段区间，不是院校原表直接公布的录取最低位次。

v3.330 历史运行版本为 `local-deterministic-v3.330-jiangxi-official-rank2025-filing-score-policy-bonus-inclusive-full-table-replaced-868426records`，含 868426 条结构化录取/投档/计划/资格边界、130155 条位次换算记录和 5134 条来源说明。江西 2025 历史类 562 条、物理类 575 条官方档案分记录替换同量第三方运行记录，位次总量不变。两科考试院 PDF 与独立镜像逐字节一致，累计人数全部闭合；1134 档数值一致，历史 441 分、375 分和物理 572 分 3 档的第三方累计位次截断已按官方表修正。运行层为 1671 条江西普通整数最低分记录补充 `score-derived-provincial-segment` 位次区间，其中官方院校来源 596 条、待复核第三方来源 1075 条；160 条特殊路径隔离。换算记录保存 `rankPolicyBonusIncluded=true`，档案分口径是总分加经审核公示的政策加分；历史类 661 分及以上仅保存 1-24 名、物理类 676 分及以上仅保存 1-33 名合并档，物理 117 分和 101 分不生成空位次。换算位次是最低分对应的全省分数段区间，不是院校原表直接公布的录取最低位次。

v3.329 历史运行版本为 `local-deterministic-v3.329-anhui-official-rank2025-policy-bonus-inclusive-full-table-aligned-868426records`，含 868426 条结构化录取/投档/计划/资格边界、130155 条位次换算记录和 5133 条来源说明。新增安徽 2025 历史类 469 条、物理类 492 条官方含加分位次记录；6 页完整 PDF 的两份独立副本逐字节一致，961 个逐分档的连续性、同分人数和累计人数算术全部闭合。运行层为 1804 条安徽普通整数最低分记录补充 `score-derived-provincial-segment` 位次区间，其中官方院校来源 658 条、待复核第三方来源 1146 条；179 条特殊路径隔离。换算记录统一保存 `rankPolicyBonusIncluded=true`；历史类 668 分及以上仅保存 1-23 名、物理类 691 分及以上仅保存 1-43 名合并档，199 分及以下不外推，艺术体育综合分不连接文化总分表。换算位次是最低分对应的全省分数段区间，不是院校原表直接公布的录取最低位次。

v3.319 当前运行版本为 `local-deterministic-v3.319-jiangsu-jseea-first-stage-rank2025-aligned-868426records`，含 868426 条结构化录取/投档/计划/资格边界、122287 条位次换算记录和 5123 条来源说明。新增江苏 2025 第一阶段历史类 177 档、物理类 221 档一分一段记录；江苏省教育考试院原图、阳光高考逐字节同图和独立全尺寸镜像共同核验 398 行、1194 个分数、同分人数、累计人数单元，差异为 0。运行层为 7060 条江苏普通整数最低分记录补充 `score-derived-provincial-segment` 位次区间，1496 条低于历史 482 分或物理 463 分的普通记录保持缺位次，123 条特殊路径隔离。第二阶段表只统计仍符合第二阶段志愿填报条件的考生，不是全体累计位次，不与第一阶段拼接。省级位次换算不等于院校原表直接公布的录取最低位次，也不用于艺体综合分。

v3.318 历史运行版本为 `local-deterministic-v3.318-hunan-education-department-rank2025-aligned-868426records`，含 868426 条结构化录取/投档/计划/资格边界、121889 条位次换算记录和 5122 条来源说明。新增湖南 2025 历史类 550 档、物理类 589 档一分一段记录；湖南省教育厅来源标注完整表共 1152 行且累计逐行闭合，独立镜像 1139 行和 3417 个分数、同分人数、累计人数单元零差异，阳光高考另存“湖南考试招生”来源原图。运行层为 9376 条湖南 2025 普通历史/物理类整数最低分记录补充 `score-derived-provincial-segment` 位次区间，388 条特殊路径隔离；历史 9 个、物理 2 个零人数分数不生成假行，0-99 分合并档不用于换算。

v3.309 当前运行版本为 `local-deterministic-v3.309-national-school-official-qlu2021-2025-native-rank-859382records`，含 859382 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5114 条来源说明。新增齐鲁工业大学 2021-2025 年 28 省单校分专业记录 2157 条；2074 条官网表列最低录取位次保存为 `school-recorded-min-score-rank`，83 条未公布位次的行保持 `rankUnavailable=true`。718 份原始响应锁定首页、省份年份字典、科类字典、类型字典和 297 个分数查询，2157 条源行零跳过、零重复。普通 1849 条为 `school-official-only`；艺体、中外合作、地方专项、民族班、校企合作和菏泽校区等 308 条为 `special-path-only`。官网分数接口未列选科要求和录取人数，运行层不补造；西藏、青海、宁夏虽然有省份入口但 2021-2025 年没有分数记录，保持来源空缺。

v3.308 历史运行版本为 `local-deterministic-v3.308-national-school-official-wtu2021-2025-native-rank-857225records`，含 857225 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5113 条来源说明。新增武汉纺织大学 2021-2025 年全国 31 省单校分专业记录 2222 条；1921 条官网表列最低位次保存为 `school-recorded-min-score-rank`，301 条未公布位次的行保持 `rankUnavailable=true`。这与 v3.306 南昌航空大学 4804 条 `score-derived-provincial-segment` 换算位次严格分开：前者是学校表格直接记录的最低分位次，后者由最低分和省一分一段表换算；两者都只是单校边界，不等于省级全量投档表或录取概率。普通 1633 条为 `school-official-only`，专项、艺体、预科、定向和合作办学等 589 条为 `special-path-only`；官网“艺文/艺理”只在检索层归一为艺术类，原科类仍保留用于证据对账。

v3.298 当前运行版本为 `local-deterministic-v3.298-heilongjiang-control-lines2026-and-rank-provenance-847051records`，含 847051 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5104 条来源说明。新增黑龙江 2026 控制线 18 条：普通历史本科/专科 385/150 分、物理本科/专科 340/150 分共 4 条为 `control-line-only`；特殊类型 2 条、体育 4 条和艺术 8 条共 14 条为 `special-path-only`。9 条本科艺体记录分字段保存文化课与专业课/术科门槛；体育和艺术专科公告只公布文化线，3 条记录仅保存专业资格要求，不生成官方未给出的专业数值。历史、物理两份正式 XLS 重新解析 1071 行并与运行数据逐行零差异，全部补齐科类附件 URL；历史 664 分为官方表省略的零人数档，不补假行。控制线、专业线、位次换算、院校/专业组投档线和专业录取最低分继续分层，任何一层都不能替代另一层。

v3.290 当前运行版本为 `local-deterministic-v3.290-hebei-control-lines2026-and-rank-provenance-846822records`，含 846822 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5096 条来源说明。新增河北 2026 控制线 54 条：普通历史类本科/专科 485/200 分、物理类本科/专科 443/200 分共 4 条为 `control-line-only`；特殊类型、艺术、体育和对口 50 条为 `special-path-only`。28 条文化课与专业成绩双门槛记录分字段保存，专业分不替代文化分；艺术校考和学校自行组织的专业测试没有公开统一数值时只保存政策说明。河北 1094 条位次换算逐条补齐正式来源页，位次数值变化为 0。控制线、专业线、位次换算、院校/专业组投档线和专业录取最低分继续分层，任何一层都不能替代另一层。

v3.289 当前运行版本为 `local-deterministic-v3.289-fujian-control-lines2026-and-rank-provenance-846768records`，含 846768 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5095 条来源说明。新增福建 2026 控制线 22 条：普通物理类本科/专科 446/235 分、历史类本科/专科 458/235 分共 4 条为 `control-line-only`；特殊类型、艺术和体育 18 条为 `special-path-only`。16 条文化课与专业统考或省际联考双门槛记录分字段保存，专业分不替代文化分；戏曲省际联考和其他艺术省统考使用不同 `professionalScoreMetric`。福建 927 条位次换算按历史类、物理类逐条补齐正式来源页，位次数值变化为 0。控制线、专业线、位次换算、院校/专业组投档线和专业录取最低分继续分层，任何一层都不能替代另一层。

v3.288 历史运行版本为 `local-deterministic-v3.288-neimenggu-control-lines2026-and-rank-provenance-846746records`，含 846746 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5094 条来源说明。新增内蒙古 2026 控制线 74 条：普通物理类本科/专科 363/160 分、历史类本科/专科 403/160 分共 4 条为 `control-line-only`；特殊类型、艺体、专项和对口招生 70 条为 `special-path-only`。36 条文化课与专业统考双门槛记录分字段保存，专业分不替代文化分。内蒙古 974 条位次换算按历史类、物理类逐条补齐正式来源页，位次数值变化为 0。

v3.287 当前运行版本为 `local-deterministic-v3.287-tianjin-control-lines2026-pending-vocational-and-rank-provenance-846672records`，含 846672 条结构化录取/投档/计划/资格边界、116656 条位次换算记录和 5093 条来源说明。新增天津 2026 控制线 6 条：普通本科 458 分为 `control-line-only`，特殊类型资格参考线 547 分、艺考类本科文化线 343/297/229 分、体育类本科文化线 407 分共 5 条为 `special-path-only`。天津 381 条 2026 位次换算逐条补齐阳光高考正式来源页，2 页 PDF 的 622036 字节和 SHA-256 已锁定，位次数值变化为 0。天津 2026 普通高职专科控制线尚待官方发布，不拿往年线补造；本科线下推荐只保留路径调研，最高 55 分、可信度 C，不展示历史投档命中或可执行清单。

v3.286 当前运行版本为 `local-deterministic-v3.286-beijing-control-lines2026-and-score-basis-846666records`：新增北京教育考试院 2026 控制线 9 条，其中普通本科 429 分按高考总分判断，普通专科 120 分按语数外三科总分判断；特殊类型、艺术、体育和高职单考单招 7 条按 `special-path-only` 隔离。推荐器新增独立 `vocationalScore` 输入与 `chinese-math-foreign-450` 比较口径：三科分缺失时不假定达到专科线、不显示可执行清单；总分低于 120 时只利用“三科小计不高于总分”的确定关系判定线下。北京 341 条 2026 位次换算记录补齐正式来源页，10 页 PDF 哈希复核，位次数值变化为 0。高考总分、专科三科分、位次换算、控制线和院校专业录取边界继续分层。

v3.285 当前运行版本为 `local-deterministic-v3.285-anhui-control-lines2026-and-rank-provenance-846657records`：新增安徽 2026 控制线 52 条，其中普通历史/物理本科与专科 4 条用于资格路由，特殊类型、体育和艺术类 48 条按 `special-path-only` 隔离。42 条艺术记录保留文化课分和省统考专业课分两个字段，不能相加、不能跨维度比较；戏曲类只保留省际联考合格要求，不生成假专业分。安徽 976 条位次换算记录补齐正式来源页，官方 6 页 PDF 哈希复核无漂移。文化控制线、专业统考线、位次换算、院校/专业组投档线和专业录取最低分继续分层，任何一层都不能替代另一层。
