...
v3.317 当前运行版本为 `local-deterministic-v3.317-liaoning-official-mirror-rank2025-aligned-868426records`，含 868426 条结构化录取/投档/计划/资格边界、120750 条位次换算记录和 5121 条来源说明。新增辽宁 2025 历史类 517 档、物理类 556 档成绩统计记录；阳光高考保存的辽宁招考办 PDF 文本层与中国教育在线两科完整表逐项比较，1073 行和 3219 个分数、同分人数、累计人数单元零差异。运行层为 21701 条辽宁 2025 普通历史/物理类整数最低分记录补充 `score-derived-provincial-segment` 位次区间，41 条特殊路径隔离；历史 667、164、162 分和物理 703、153 分是零人数缺口，不生成假行。省级统计表可换算同年同科类文化分位次，但不等于院校原表直接公布的录取最低位次，也不用于艺体综合分。
