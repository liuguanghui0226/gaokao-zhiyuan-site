...
v3.332 当前运行版本为 `local-deterministic-v3.332-shaanxi-official-rank2025-dual-domain-lower-absent-bucket-guard-868426records`，含 868426 条结构化录取/投档/计划/资格边界、133640 条位次换算记录和 5136 条来源说明。陕西 2025 普通历史 549 条、普通物理 594 条官方分数段记录中 1141 条可用于估算，2 条含缺考人数的低分汇总桶只留作证据；2026 既有同类 2 条汇总桶同步加保护。运行层为 992 条普通整数最低分补充 `score-derived-provincial-segment` 位次，其中本科 986 条、明确专科 6 条；289 条特殊路径和 302 条模糊边界不自动套表。换算位次是最低分对应的全省分数段区间，不是院校原表直接公布的录取最低位次；官方页未说明政策加分口径时保持未知。
