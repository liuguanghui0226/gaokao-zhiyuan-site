...
v3.334 当前运行版本为 `local-deterministic-v3.334-xizang-official-category-required-no-public-rank-guard-868426records`，含 868426 条结构化录取/投档/计划/资格边界、133640 条位次换算记录和 5136 条来源说明。西藏 2025 年 22 条官方控制线统一写入 A 类、B 类或部队生源类别；A 类为区内世居两代（含两代）以上少数民族考生，B 类为汉族及区外少数民族考生。推荐器要求西藏考生先确认类别，未选时可信度强制为 C，控制线和单校记录不得跨类别混用。已核验的官方公开渠道未提供可计算一分一段，因此西藏位次换算保持 0，不按分数生成假位次。
